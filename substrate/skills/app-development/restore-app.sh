...
# --- Port collision check + folder-move + unit-install (all under the same
#     lock create-app.sh uses, so a concurrent create can't steal the port
#     between our check and our install) ---

ARCHIVED_APP_UNIT="$APP_DIR/app-$SLUG.service.archived"
(
    exec 200>"$LOCK_FILE"
    flock -x 200
    ...
    if printf '%s\n' "$USED_PORTS" | grep -qx "$ARCHIVED_PORT"; then
        echo "port $ARCHIVED_PORT is now in use by another app on this box — archive that one first, or create a fresh app instead of restoring" >&2
        exit 1
    fi
