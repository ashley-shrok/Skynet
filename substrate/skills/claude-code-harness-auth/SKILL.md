...
- **Notice and resolve are one loop, not two.** The DM that says "click this URL" is the
  same conversation user pastes the code back into; the token that setup-token then
...
- **Agent-driven, not user-driven.** The agent runs `claude setup-token` in a sidecar tmux
  ...
  the printed token, and installs it. user's only manual step is clicking the URL and
...
false-positive DM frequency, same agent-driven UX for user.
...
**Pending-flow state (transient, exists only while awaiting user's paste):**
...
`step` transitions: `awaiting-code` (user clicks URL, DMs code back) →
...
pieces of it.
    ...
    # to fail LOUD here rather than silently ship an incomplete URL to user whose
...
If a future version breaks the matcher, the URL-capture timeout fires and user gets a
plain-error DM instead of a partial success. Coarse-grained failure by design.

⚠️ **Do NOT skip the ANSI strip on any DM sent to user.** pipe-pane captures include
...
### A.4 — DM user the URL and persist state
...
maintainer when user DMs back; Phase B runs then.

## Phase B — continue with code, capture token, install

Runs from the maintainer's reflex when user's DM lands and pending state exists.
...
If user DMed anything other than a plausible OAuth code, it's chat, not the code — leave
...
user didn't come back within 4h assume she's not going to" — anything past that will fail
...
hit a failure mode we don't recognize. Kill sidecar, clear state, DM user, retry next
daily wake.

**Sidecar died mid-flow.** Handled in B.1 — if `tmux has-session` fails, clean up state
and DM user the failure without feeding the code into nothing.
...
- **user gets DMed when nothing is actually wrong.** False-positive DMs — asking for a
...
- **The install marker gets lost or corrupted.** Daily check re-triggers on missing marker
  (bootstrap case). If a working token IS installed but marker is gone, user gets an
...
- **Token-capture regex misses a future format change.** Anthropic could rename the token
  ...
  loose but not future-proof. A miss triggers the coarse-fail path (DM user, retry next
...
- **Subscription lapses.** If Claude Max expires or org membership changes, setup-token
  stops working. Skill has no visibility — user sees 401, fixes subscription first.
- **The flow becomes a place things get stuck.** A sidecar tmux that survives failure and
  blocks a session slot. A state file that never clears. user DMs the code and nothing
