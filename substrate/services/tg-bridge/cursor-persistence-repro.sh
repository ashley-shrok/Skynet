...
#   1. Seed /tmp/repro-state/user.since with a known next_batch token.
...
printf '%s' "s_seed_batch_12345" > "$STATE/user.since"
LOADED=$(cat "$STATE/user.since")
...
SINCE=$(cat "$STATE/user.since" 2>/dev/null || true)
if [ -z "$SINCE" ]; then
...
printf '%s' "$NB" > "$STATE/user.since"
UPDATED=$(cat "$STATE/user.since")
