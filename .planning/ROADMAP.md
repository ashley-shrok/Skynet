...
- [ ] 82-04-PLAN.md — WipBubble rewire: import useBrandingConfig from `@/branding/branding-store` in src/ui/features/pretty-view/WipBubble.tsx; destructure `wipIndicatorPath` from the config; change `<img src="/wip-cube.webp">` to `<img src={wipIndicatorPath}>`; preserve all other JSX props (alt, role="status", aria-label, h-[52px] w-[52px] classes) verbatim per LOCKED "No new sizing field"; append single-line Phase 82 addendum to the existing patch #86/#260/2026-09-05 comment history block. No new WipBubble test file (per Claude's Discretion — one-line src prop wiring); existing PrettyView.plain-dom.test.tsx Test 4 continues to pass (asserts role="status" and mount location, not src). (wave 2, depends on 82-03)
