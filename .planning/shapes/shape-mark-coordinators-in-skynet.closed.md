...
The on-disk cosmetic pipeline that reads role, display name, title, color hue, voice, and avatar from each identity's YAML frontmatter is Phase 66's contribution — code-complete and verified but not yet shipped (user paused mid-ship). This build extends that reader by exactly one more field — the coordinator flag — and adds a rendering pass for it on the three named surfaces. The reader already routes through hostId to reach the correct box's disk; this piggybacks on that routing without introducing a new mechanism.
...
**Deferred:**
- Mobile-specific tuning if the desktop treatment scales awkwardly on mobile. user will iterate after seeing it live.
- Any additional coordinator affordances (bounty routing behaviors, sort-by-coordinator, filter-by-coordinator).

**Tempting-but-no:**
- Folding this into Phase 66 to save a ship gate. Growing Phase 66's scope would trigger re-verification and complicate the ship-pause state user is deliberately holding.
...
**GSD phase, stacked.** Plan and execute independently now; the work commits locally alongside the other unshipped items (Phase 66, and the smaller items sitting on the branch). When user greenlights the next ship, this phase's commits go out in that same bundle. No independent ship gate for this phase.
