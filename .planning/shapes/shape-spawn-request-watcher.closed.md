...
happens once Shape 5 is verified and user greenlights the whole-campaign ship. Shape 4
...
- **Shape: coordinator safety timeout (~few minutes, escalates same as picker failure)** — present · coordinator-instructions.md specifies ~3 minute safety timeout; on timeout escalates to user via picker-failure path
