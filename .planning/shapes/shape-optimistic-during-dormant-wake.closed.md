...
When user sends a message to an agent that is currently asleep, the message-in-flight
...
networks switch — the change-only signal never gets re-sent for the sleeping-state user
...
fine when a failed state was mostly a false positive user had learned to ignore for a
...
user has been hitting this specific bug for a long time. The visible pattern is always
...
failed one seconds later. Sometimes a duplicate real bubble also shows up. user has
...
reconnect. In practice, user's tab spends most of its life connected to sessions that
...
regardless of the truth. So the widened branch almost never triggers, and user sees the
...
drops during the widened wait, does the pending survive reconnect. user considers this
...
"reconnect can happen between when the agent went dormant and when user sends," but it
...
- **If user sends two messages in a row to a sleeping agent and they do not both
...
- Reconnect-during-the-widened-wait behavior. Rare enough scenario that custom mechanism
  for it is scope creep. Follow-up bounty only if it bites user in practice.
