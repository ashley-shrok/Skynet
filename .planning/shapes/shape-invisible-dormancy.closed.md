...
specifically so prior message history would stay readable while asleep. user has
...
- Any escape-hatch UI in pretty view for "session is stuck" — the tmux terminal view
  behind pretty view is the escape hatch for that case, and user doesn't use the
