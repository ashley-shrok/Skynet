...
**Opened:** 2026-09-12 (user + vega, /build)
**Status:** Agreed — /open grill skipped per user's explicit direction ("you already know shape: hide identity rows via sentinel, that's it")
...
The `.hidden` sentinel is identity-scoped (`~/fleet/identities/<name>/.hidden`), not user-scoped, so if one user of this Skynet instance hides an identity row, it's hidden for every user. Mirrors the existing `.pinned` sentinel scoping — same tradeoff, same rationale. user aware, greenlit.
