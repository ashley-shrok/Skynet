...
Skynet's voice-in (mic recording turned into text) and voice-out (assistant messages read aloud) currently terminate at a self-hosted model rig on user's personal PC over the tailnet. That machine is going away as a model host — she's reclaiming it. This shape moves both sides of voice onto external cloud providers so nothing depends on the personal PC anymore.

Chosen providers, based on hands-on quality validation against real Skynet distribution: **Amazon Polly** for voice-out (top-tier generative engine, user confirmed the quality matches or beats her local rig) and **Amazon Transcribe** streaming for voice-in.
...
- **One provider, one tier, no ceremony.** Voice-out is always top-tier generative because user heard the quality difference and picked it flat. No user-facing engine toggle, no per-identity engine pick, no fallback tier. The mental model is: pick your voice, that's it.
