...
**Closed:** 2026-09-17
**Vehicle used:** GSD phase (Phase 113) — backend endpoint + frontend restructure + tests + nginx-parity-already-present
**Overall verdict:** closed-hit

### Shape features (conformance)

- **What this is — extend Edit Skills modal to cover create-new-skill lifecycle** — present · SkillsEditorModal.tsx now has createSkill flow; header add-file affordance moved to tab strip; single-host picker hidden.
- **Shape — "+ New skill" button in header after skill picker** — present · SkillsEditorModal.tsx L557-564 renders the button after the skill `<select>`.
- **Shape — chained name → description prompts** — present · handleNewSkill at L324-371 chains window.prompt for name then description with re-prompt loops.
- **Shape — silent slugify of name** — present · L332 calls slugifyRoleName(rawName.trim()); the raw folder name is machine-managed.
- **Shape — backend mints skill folder + SKILL.md sentinel carrying only frontmatter** — present · POST /skills-editor/skill at skills-editor.ts L1010-1184; composeSkillMdSeed at L262-267 produces frontmatter with empty body.
- **Shape — refetch skills, auto-select new skill, tab strip opens on fresh SKILL.md** — present · handleNewSkill L358-361 refetches + setSelectedSkillName(result.slug); file-list effect auto-selects first file, which is SKILL.md (alphabetical).
- **Shape — old Add-file button leaves header, becomes "+ New file" tab pinned right of tab strip** — present · newFileTabButton fragment at L443-456; pinned as last child at L754; header no longer contains add-file button.
- **Shape — New-file tab behaves as action, never becomes selected** — present · onClick invokes handleAddFile() only, never setActiveTab; styling stays non-selected regardless of activeTab.
- **Shape — New-file tab present the moment a skill is picked, even before files load** — present · Empty-file-list body branch at L652-672 also renders newFileTabButton with empty-state copy pointing at it.
- **Shape — host picker hides when exactly one host** — present · L504 gates `<select>` render on flatHosts.length > 1; auto-select effect still picks sole host.
- **Philosophy — SKILL.md invariant: cannot be deleted through this surface** — present · Backend DELETE /file rejects path === "SKILL.md" at skills-editor.ts L1229-1232; SkillFileTab.tsx L151 hides Trash2 when filename === "SKILL.md". Forward-only: no migration code for existing SKILL.md-less skills.
- **Prior context — path-safety two-layer gate + shell-escape on new endpoint** — present · POST /skill runs SKILL_NAME_RE gate (L1031) + belt-and-suspenders prefix assertion (L1110-1113) + shellEscape on interpolated values.
- **Prior context — nginx config parity in both files** — present · No new route added; /skills-editor block already lives in both docker/nginx.conf L445 and docker/nginx-https.conf L460 from Phase 44.
- **Scope IN — tests for new backend route** — present · skills-editor.test.ts describe('POST /skills-editor/skill') covers happy path, invalid name, path escape via "..", duplicate 409, plus empty/overlength/multi-line description, SSH fail, SFTP-cleanup, YAML escape.
- **Scope IN — frontend tests** — present · SkillsEditorModal.test.tsx covers chained-prompt happy path, cancel-on-name, cancel-on-description, empty-description re-prompts, New-file tab is last child of tab strip, single-host hides picker, multi-host shows picker, SKILL.md tab has no delete affordance, non-SKILL.md keeps it, empty-file-list renders new-file tab.
- **Scope OUT — no rename/clone/template/marketplace/retroactive migration/sibling change** — present · No such code found; GlobalFilesModal.tsx and CreateRoleDialog.tsx untouched by any Phase 113 commit. slugifyRoleName is a pre-existing export.
- **Scope Deferred — window.prompt chain kept (no mini-dialog)** — present · handleNewSkill uses chained window.prompt as agreed; no mini-dialog was introduced.
...
Read both backend (skills-editor.ts + test) and frontend (SkillsEditorModal.tsx + test + SkillFileTab.tsx + skills-api.ts) plus both nginx configs. Every commitment in the shape maps to concrete code; no unsanctioned additions detected. Multi-line and >4096-byte description rejections on the backend are natural hardening consistent with the shape's "usual security-adjacent surface" language, not spec-worthy additions. Verified no Phase 113 commits touched GlobalFilesModal.tsx or CreateRoleDialog.tsx.
