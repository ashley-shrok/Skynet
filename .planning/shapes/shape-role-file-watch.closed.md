...
- **The first-run rule fires anyway.** If an identity's very first `/id` load produces a spurious wake because the "have I seen this file before?" check misfires, we've broken the "silent on cold start" invariant user picked to keep fresh sessions clean.
...
- Follow the standing rule: build + commit locally, do NOT push, do NOT deploy — user greenlights push separately per the tightened deploy-window boundary rule.
...
- Shape's "every identity holding every role" language is superseded by user's coord-exclusion call; if the shape ever gets re-read as source of truth, note the sharpening: "every actor identity holding every role." — accepted-as-drift
