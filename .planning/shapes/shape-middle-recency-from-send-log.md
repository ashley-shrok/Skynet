# Shape: middle-list recency, sourced from user's own sends
...
approach: instead of guessing when user last talked to an identity from
...
- **Skynet keeps its own record of "when did user last send to this
  identity."** The record is stored inside Skynet's own durable state so it
  survives container restarts and every device user uses (phone and
...
- **The record is keyed on identity name.** Not on any plumbing underneath —
  ...
  that carries the message. user cares about the identity; the key is the
  identity.
...
- **Attempts count. Delivery is not required.** If user hits send and the
...
- **No starting-point seeding on first ship.** When the change lands, the
  record is empty. Identities user has talked to before all start at
...
don't have a first-class signal for 'user talked to this agent' so we
...
tightening of that predicate excludes more of the ways user's real
...
recover it downstream. When user presses send in Skynet, Skynet knows.
Record it there.
...
Sessions recycle, boxes reboot, transports change; the identity user is
...
- Not trying to count message activity the identity generates (assistant
  replies, tool output, self-chat). The signal is one-directional: user
  saying something to the identity, not the identity being busy.
- Not trying to capture user's phone Matrix DMs that bypass Skynet's
...
- The current source of the recency number is a remote scan of each
  ...
  scanned for lines that pass a strict "user's real user turn" filter
  ...
  identity sinks to the null-to-bottom zone until user types into that
  new transcript. Concrete case: an identity that stood up a VM yesterday,
  recycled overnight, sits below identities user hasn't touched in
  weeks.
- Additional loss mode: interactions routed through a coordinator, arriving
  ...
  interactions are real — user is orchestrating that identity's work —
...
- **If the ranking waits for a backend round-trip before it visibly
  reorders.** When user hits send, she should see the row jump. Waiting
...
- **If user's two devices show different orderings.** Phone and desktop
...
- A new durable per-identity "when did user last send to this identity"
...
- Client-side responsiveness: the row moves the instant user hits send,
...
- Any capture of user's phone Matrix DMs that bypass Skynet's compose
...
- Multi-user keying — Skynet on this box is single-tenant for user, so
...
- The new store must survive container restarts (durable, not in-memory),
  and both of user's devices (phone and desktop) must read the same
