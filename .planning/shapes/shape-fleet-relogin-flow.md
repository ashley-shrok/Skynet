...
- Alice's fleet has multiple accounts she rotates between during high-usage weeks, which shortens the effective login cycle on any box she's using at that moment. The reminder side accommodates this naturally: any re-authentication event, for any reason, resets the counter — no special handling needed.
...
- **A scheduled wake-up fires into an unauthenticated harness with no prior warning.** The core failure mode this feature exists to prevent. If a maintainer box is stranded and Alice didn't get a card in Skynet warning her ahead of time, the feature has missed its point.
- **Alice gets pinged when nothing is actually wrong.** False-positive warnings — cards appearing when the box is fine — erode trust in the signal and train her to ignore it. Better to under-warn (fall back to the red-expired card) than to over-warn.
...
**Out (of this phase):**
- A redundant direct-message channel that fires when Alice isn't in the app. Explicitly deferred — she said "something to reach out to me, but I think we save that for later."
...
The three vertical layers of this feature — the detection engine, the surfaced cards, and the guided flow — are tightly coupled at the wire. The card is meaningless without the detection engine feeding it; the entry-point button is a lie without the flow behind it. Splitting into two phases (reminder-only first, guided-flow later) would ship a half-feature where clicking the "log in" affordance either does nothing or context-switches to a shell — the exact thing Alice called out as unacceptable when she asked for this to be Skynet-native in the first place. One phase, one coherent vertical slice, verified end-to-end before it lands.
...
- No day-zero safety-net cron was shipped; the fleet is unprotected between now and this feature landing. Not a blocker — Alice explicitly said she's not worried about the interim.
