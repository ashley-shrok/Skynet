...
**The UX-pass campaign runs push-only during dev — no full builds, no `docker cp` fast-path, no deploy motion until every bounty in the campaign has landed and user greenlights the ship gate.** This phase respects that: local scoped tests pass, commits push to origin, but no docker build and no container restart until the campaign-level ship greenlight fires.
...
- Deploy motion for this phase individually. It ships together with the rest of the UX-pass campaign when user greenlights.
...
**Ship constraint:** part of the UX-pass campaign. Commits + push + scoped tests only. No `docker build`, no `docker cp`, no `docker compose up --force-recreate`, no full-suite test run until every remaining campaign bounty lands and user explicitly greenlights the ship gate.
...
- **Shape §4: On-disk data migration** — present · Explicitly scoped out of repo per shape's "Outside the repo" note — user's manual pass post-close; no shape miss
...
- **What would make it wrong: main runbook file staying as `<slug>.md`** — present · Backend reads `runbook.md` sentinel; id-skill contract updated; disk rename is user's out-of-repo pass
