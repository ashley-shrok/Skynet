...
- **Shape — Moment 5: Furnish the box (browser auth touchpoint stays with account owner)** — present · user drives the OAuth touchpoint on the fresh VM; furnishing script then runs the mechanical pass
...
- **Shape — Moment 7: Verify + vault-share** — present · user logs in as the user, confirms box + assistant, packages creds into Zoho Vault + shares via Zoho
