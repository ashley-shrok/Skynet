...
- Nine items were originally sorted into this bounty from user's UX-pass digest 2026-09-07. During /open discussion, verification against the current code revealed:
...
**Tempting-but-no:**
- Adding a per-agent "description" or "notes" field on create-agent to replace the removed Brief. user didn't ask for one; if it turns out to be needed later, it's a separate bounty.
- Renaming the modal from "New agent" to something else. Phase 84 already conformed the title; no re-open.
- Making the checkbox label something more clever than "Just a shell — no agent." user greenlit that specific text.
...
**Execution order** (user's standing UX-pass campaign, reshuffled 2026-09-07 during this /open):
