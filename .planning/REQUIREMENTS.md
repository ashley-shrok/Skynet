...
- [ ] **ASIDE-01**: Aside firing is triggered by the same idle-window signal that drives the ComposeBox WIP indicator's clearing — i.e. an assistant turn has landed AND the WIP indicator's idle threshold has passed with no new activity on that session. No separate "worth-explaining" detector, no model call in the trigger path.
- [ ] **ASIDE-02**: Aside firing scope is per browser-window active-set: for every fleet-identity session with at least one open pretty-view tab in the user's currently-connected browser window(s), each completed turn on that session fires its own aside. Sessions with zero open tabs anywhere do not fire asides. Anonymous / ad-hoc (non-identity) claude sessions are excluded from the feature entirely.
- [ ] **ASIDE-03**: The backend injects a canned `/btw` prompt into the identity's tmux session via `tmux send-keys` over the existing SSH exec channel. Prompt text inlines the `/explain` skill body verbatim: `/btw Re-explain whatever's currently going on to me without using code symbols, in a conceptual model style. Not a metaphor — explain the actual thing, don't recast it as an extended analogy.`
- [ ] **ASIDE-04**: The backend extracts the /btw answer by polling `tmux capture-pane`, detecting the BTW overlay's end-of-answer marker line (`↑/↓ to scroll · f to fork · Esc to close`), and reading the answer text between the echoed `/btw` line and the marker. Multi-line answers exceeding the visible overlay are recovered from tmux scrollback (`capture-pane -S -N`); a fallback path scrolls the overlay via `↑`/`↓` keystrokes if scrollback is insufficient.
- [ ] **ASIDE-05**: The aside renders as a new AsideBubble message-bubble type at the BOTTOM of the pretty-view message stream, in-flow inside the scrollable message list (not as an overlay, popup, or fixed-position element). Scrolling the stream up to re-read history works exactly as before; the aside stays pinned at the bottom of the list. The bubble uses the same identity-hue background gradient as normal assistant bubbles for that session but with a distinct thick + neon-glow border treatment (starting values: 10px solid border in `hsla(var(--pv-id-hue),90%,65%,1)`, plus a three-layer outer neon glow at 12px/32px/64px in the hue at descending alpha).
- [ ] **ASIDE-06**: When an aside is currently displayed for a session, the ComposeBox for that session morphs: the send-message button becomes an X icon (hover tooltip "Resume"); the queue-message, thumbs-up, and reset-session affordances all disable/grey; the textarea remains editable and any partial draft text remains intact.
- [ ] **ASIDE-07**: Clicking the X (Resume) affordance dismisses the aside: the AsideBubble is removed from the message stream, the ComposeBox reverts to normal, the backend sends `Escape` into the identity's tmux to close the underlying BTW overlay, and any partial draft in the textarea is preserved (never cleared by dismiss).
- [ ] **ASIDE-08**: v1 policy for a new turn arriving while an aside is already displayed on a session: the currently-displayed aside is preserved unchanged; the newer turn does NOT get its own aside; the newer turn's arrival is otherwise unaffected. (Revisit if this loses too much translation coverage in practice.)
- [ ] **ASIDE-09**: When a pretty-view tab is closed while its aside is still displayed, the underlying tmux BTW overlay is left in its open state — no cleanup or dismiss is sent. When a pretty-view for that session subsequently mounts (any browser tab), the backend pane-probes the session's tmux, detects the still-open BTW overlay if present, extracts its answer text, and emits it to the mounting client so the aside is re-rendered in the same displayed state.
- [ ] **ASIDE-10**: The backend maintains NO separate aside store (no database row, no in-memory key-value cache, no persistence layer). The tmux BTW overlay itself is the sole source of truth for "is there an aside for this session and what does it say" — the backend is a pure translator that reads the pane, emits events, and forwards dismiss commands. Backend restarts recover state by re-probing panes on next relevant event.
- [ ] **ASIDE-11**: When an aside is dismissed in one browser tab, any other browser tab currently viewing the same session's pretty-view also clears its aside display (cross-tab coherence). Achieved via the backend broadcasting an aside-dismissed event on the session's WebSocket stream to every subscribed client.
