...
- [ ] **PURGE-08**: Skynet tab bar chrome (top-level visible tab strip UI user never sees in Skynet) is deleted from the source tree; invisible tab plumbing (mount/unmount, WebSocket lifecycle, focus routing) stays intact
...
- [ ] **SHAPE-05**: Post-lift, the ready-for-attention dot on rows user has clicked into IS visible in UAT (mobile + desktop). If the lift alone doesn't resolve dot visibility, the four candidates are investigated: (a) Terminal.tsx isIdle null-start (ticker not fired post-recreate?), (b) sessionWorkingKey mismatch (row.targetTmuxSession null vs Terminal publishing real tmuxSessionName), (c) activeSet sessionStorage populate on fresh session, (d) PrettyConversationRowLive Rules-of-Hooks compliance.
...
- [ ] **SHAPE-07**: user UATs the conversation list at parity with the mock on both mobile (iPhone PWA) and desktop viewports; `tsc --noEmit` exits 0; `npx vitest run` all green (or unchanged from Phase 12's baseline); `npm run build` succeeds.
...
- [ ] **TG-07**: The mobile-only bottom navigation bar (whose current entries — host manager, credentials editor, and adjacent admin surfaces — user does not use) is deleted entirely as a surface. It does not appear on any mobile viewport in any state
...
### Plain-Language Translation Asides (Phase 14)

- [x] **ASIDE-01**: Aside firing is triggered by the same idle-window signal that drives the ComposeBox WIP indicator's clearing — i.e. an assistant turn has landed AND the WIP indicator's idle threshold has passed with no new activity on that session. No separate "worth-explaining" detector, no model call in the trigger path.
- [x] **ASIDE-02**: Aside firing scope is per browser-window active-set: for every fleet-identity session with at least one open pretty-view tab in the user's currently-connected browser window(s), each completed turn on that session fires its own aside. Sessions with zero open tabs anywhere do not fire asides. Anonymous / ad-hoc (non-identity) claude sessions are excluded from the feature entirely.
- [x] **ASIDE-03**: The backend injects a canned `/btw` prompt into the identity's tmux session via `tmux send-keys` over the existing SSH exec channel. Prompt text inlines the `/explain` skill body verbatim: `/btw Re-explain whatever's currently going on to me without using code symbols, in a conceptual model style. Not a metaphor — explain the actual thing, don't recast it as an extended analogy.`
- [x] **ASIDE-04**: The backend extracts the /btw answer by polling `tmux capture-pane`, detecting the BTW overlay's end-of-answer marker line (`↑/↓ to scroll · f to fork · Esc to close`), and reading the answer text between the echoed `/btw` line and the marker. Multi-line answers exceeding the visible overlay are recovered from tmux scrollback (`capture-pane -S -N`); a fallback path scrolls the overlay via `↑`/`↓` keystrokes if scrollback is insufficient.
- [x] **ASIDE-05**: The aside renders as a new AsideBubble message-bubble type at the BOTTOM of the pretty-view message stream, in-flow inside the scrollable message list (not as an overlay, popup, or fixed-position element). Scrolling the stream up to re-read history works exactly as before; the aside stays pinned at the bottom of the list. The bubble uses the same identity-hue background gradient as normal assistant bubbles for that session but with a distinct thick + neon-glow border treatment (starting values: 10px solid border in `hsla(var(--pv-id-hue),90%,65%,1)`, plus a three-layer outer neon glow at 12px/32px/64px in the hue at descending alpha).
- [x] **ASIDE-06**: When an aside is currently displayed for a session, the ComposeBox for that session morphs: the send-message button becomes an X icon (hover tooltip "Resume"); the queue-message, thumbs-up, and reset-session affordances all disable/grey; the textarea remains editable and any partial draft text remains intact.
- [x] **ASIDE-07**: Clicking the X (Resume) affordance dismisses the aside: the AsideBubble is removed from the message stream, the ComposeBox reverts to normal, the backend sends `Escape` into the identity's tmux to close the underlying BTW overlay, and any partial draft in the textarea is preserved (never cleared by dismiss).
- [x] **ASIDE-08**: v1 policy for a new turn arriving while an aside is already displayed on a session: the currently-displayed aside is preserved unchanged; the newer turn does NOT get its own aside; the newer turn's arrival is otherwise unaffected. (Revisit if this loses too much translation coverage in practice.)
- [x] **ASIDE-09**: When a pretty-view tab is closed while its aside is still displayed, the underlying tmux BTW overlay is left in its open state — no cleanup or dismiss is sent. When a pretty-view for that session subsequently mounts (any browser tab), the backend pane-probes the session's tmux, detects the still-open BTW overlay if present, extracts its answer text, and emits it to the mounting client so the aside is re-rendered in the same displayed state.
...
- [ ] **PIN-02**: Pinned conversation IDs sync across all of user's devices (desktop ↔ iPhone) via server-side storage keyed to her authenticated user account. Pinning on desktop appears on iPhone (within one poll / next mount), and vice versa.
...
- [x] **RELAYBUB-03**: Inbound bubble sender mxid (`@name:server`) is resolved to the corresponding local identity where possible (via the existing identity registry), and the bubble carries that identity's stored `colorHue` — same hue-chain treatment as normal agent bubbles. Unresolved mxids fall back to a neutral grey hue with the raw mxid visible.
...
| RELAYBUB-03 | Phase 17 | Complete |
| RELAYBUB-04 | Phase 17 | Complete |
| RELAYBUB-05 | Phase 17 | Complete |
| RELAYBUB-06 | Phase 17 | Complete |

| IDMEDIT-01 | Phase 18 | Complete |
| IDMEDIT-02 | Phase 18 | Complete |
| IDMEDIT-03 | Phase 18 | Complete |
| IDMEDIT-04 | Phase 18 | Complete |
| IDMEDIT-05 | Phase 18 | Complete |
| IDMEDIT-06 | Phase 18 | Complete |
| IDMEDIT-07 | Phase 18 | Pending |
| IDMEDIT-08 | Phase 18 | Pending |

| TTSSTR-01 | Phase 19 | Complete |
| TTSSTR-02 | Phase 19 | Complete |
| TTSSTR-03 | Phase 19 | Complete |
| TTSSTR-04 | Phase 19 | Complete |
| TTSSTR-05 | Phase 19 | Complete |
| TTSSTR-06 | Phase 19 | Complete |
| TTSSTR-07 | Phase 19 | Complete |
...
*2026-07-31 — added TTSSTR-01..07 for Phase 19 (streaming TTS output via Chatterbox /tts; replaces buffered path on bubble speak-button with Web Audio API progressive decode; iOS Safari spike passed on user's iPhone PWA; IdentityModal voice-preview keeps calling buffered /voice/speak unchanged)*
