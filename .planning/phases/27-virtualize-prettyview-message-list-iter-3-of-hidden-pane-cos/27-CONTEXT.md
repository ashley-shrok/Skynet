...
**Source:** In-chat design lock with user 2026-08-09 (iter 3 dispatch + ⌘F decision) + parent bounty `hidden-pane-cost-mitigation-empirical-rotation` + iter-3 bounty `pretty-view-message-list-virtualization` (both LOCKED)
...
### ⌘F/find-in-page regression on long conversations — ACCEPTED (user 2026-08-09)
...
### Streaming bubble re-measure — MOOT (design confirmation, user 2026-08-09)
...
### Scroll anchor preservation — LOCKED (existing behavior + user 2026-08-09)
...
- **Custom in-app message search** — user-facing recovery for the ⌘F regression. user explicitly declined this (2026-08-09). If she changes her mind later, spin as a sibling bounty then.
...
- **Streaming-bubble re-measure logic** — moot per user's "we don't do streaming tokens." If we ever add streaming tokens later, that's a different phase.
</deferred>

---

*Phase: 27-virtualize-prettyview-message-list-iter-3-of-hidden-pane-cos*
*Context gathered: 2026-08-09 via direct decision-lock (all decisions were already answered in ROADMAP entry + bounty + user's in-chat responses; discuss-phase skipped to avoid re-eliciting settled context)*
