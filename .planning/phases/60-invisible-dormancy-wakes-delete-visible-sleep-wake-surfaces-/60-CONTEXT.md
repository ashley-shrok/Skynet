...
- Any escape-hatch UI in PrettyView for "session is stuck" (user confirmed she doesn't use the current Wake button as escape).
...
**Ship-gate:** full `npx vitest run` green; scoped tests pass throughout dev; docker build + force-recreate; HTTPS 200; user UAT — invisible wake works, red-bubble fires on real failure.
