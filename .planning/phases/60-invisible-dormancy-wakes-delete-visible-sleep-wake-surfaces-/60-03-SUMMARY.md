...
affects: [Phase 60 fully green — all three plans (01 send-path + wakingSince suppression, 02 widened watchdog, 03 frontend deletion + backend cleanup) shipped. Ready for orchestrator ship-gate (full-suite vitest + docker build + force-recreate + HTTPS 200 + user UAT)]
...
- **T-60-03-03 (tampering) mitigated:** Stale frontend bundles that still send `{type:"wake"}` post-deploy fall through to the outer WS `switch(msg.type)` default (log-and-drop, no side effect). user's fleet is single-user; no third-party clients. Verified by inspection of the switch statement after deleting the wake case.
