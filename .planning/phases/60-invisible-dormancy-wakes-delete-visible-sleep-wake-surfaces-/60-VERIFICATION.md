...
**Ready for orchestrator ship-gate** (full-suite `npx vitest run` + `docker build` + `docker compose up -d --force-recreate skynet` behind deadman + HTTPS 200 + user UAT: invisible wake works, red-bubble fires on real failure).
