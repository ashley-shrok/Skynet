...
- **Plan 45-03 is unblocked.** With Plan 45-01 (backend revert) and Plan 45-02 (frontend wire-type cleanup) both landed on `feat/tab-title-from-tmux`, Plan 45-03 can now:
  ...
  2. Add `paddingBottom: 9` inline style to the bubble wrapper at PrettyView.tsx:2481 (PATTERNS.md § 9 Part (c), user LOCKED value).
...
- **Sibling canaries preserved for 45-03 to reference:**
  ...
  - `appendDedupWithCap` and its 5 live-append call sites in `PrettyView.tsx` (per PATTERNS.md § 9 Part (b), preserved as-is — the drop-oldest cap logic is exactly what user wants and does NOT need rebuilding in 45-03).
