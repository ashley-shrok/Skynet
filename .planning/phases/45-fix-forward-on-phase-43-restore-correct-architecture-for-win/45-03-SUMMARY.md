...
tech-stack:
  ...
  patterns:
    ...
    - "Byte-for-byte preservation of appendDedupWithCap + WORKING_SET_CAP + 5 live-append call sites (Alice UAT-locked pattern; already correct pre-surgery)"
...
decisions:
  ...
  - "Chose inline JSX comment (`{/* Phase 45 Bug #2 ... */}`) above the bubble wrapper instead of a longer multi-line comment. The plan required an user-quote source-comment; the one-line form is loud enough for future archaeology while not disturbing the visual density of the messages.map block."
...
must_haves:
  truths:
    ...
    - "PrettyView.tsx bubble wrapper at plain-DOM messages.map render has style={{ paddingBottom: 9 }} (Alice UAT verbatim value)"
...
- **`PrettyView.tsx` bubble wrapper in `messages.map` → inline `style={{ paddingBottom: 9 }}`** — restored 9px inter-bubble gap (Bug #2 fix, Alice UAT verbatim).
...
- **T-45-03-03 (Tampering, 9px paddingBottom inline style):** ACCEPTED. Inline style with a hardcoded integer literal `9` — no user-controlled data flows into the style value. user-locked constant, not derived from any input. Verified: the only `paddingBottom: 9` occurrence in the file is the hardcoded integer on the bubble wrapper; no template literal, no variable interpolation.
