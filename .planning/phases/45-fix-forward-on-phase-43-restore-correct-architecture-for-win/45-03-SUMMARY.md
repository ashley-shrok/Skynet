...
tech-stack:
  ...
  patterns:
    ...
    - "Byte-for-byte preservation of appendDedupWithCap + WORKING_SET_CAP + 5 live-append call sites (user UAT-locked pattern; already correct pre-surgery)"
...
must_haves:
  truths:
    ...
    - "PrettyView.tsx bubble wrapper at plain-DOM messages.map render has style={{ paddingBottom: 9 }} (user UAT verbatim value)"
...
- **`PrettyView.tsx` bubble wrapper in `messages.map` → inline `style={{ paddingBottom: 9 }}`** — restored 9px inter-bubble gap (Bug #2 fix, user UAT verbatim).
