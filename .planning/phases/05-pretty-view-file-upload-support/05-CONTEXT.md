...
**Source:** Synthesized from shape file `.planning/shapes/shape-pretty-view-file-upload-support.md` — that file is authoritative and every philosophical question was locked during a long `/open` discussion with user on 2026-07-20 (see the shape's session log for provenance). This CONTEXT.md restates the shape as locked planning decisions plus the concrete skynet-fork integration points. **The shape is not to be re-litigated; the planner's job is HOW, not WHAT.**
...
- `.planning/shapes/shape-pretty-view-file-upload-support.md` — the LOCKED shape from user's `/open` discussion. Every decision in this CONTEXT.md traces back to this file. Read it first.
