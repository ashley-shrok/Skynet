...
**Patch number placeholder:** `NNN.` — user fills in at pin time. Current top is patch #103 (jump-pill scroll fix, 2026-07-20), so this will most likely be **patch #104** unless something else lands between now and Phase 5 deploy.
...
```
   NNN. `feat(pretty-view): file upload support — drag/drop/paste/mobile
       ...
       * **Deploy note**: shipped behind the mandatory 15-min
         deadman per user 2026-07-03 with explicit per-deploy
         green light per user 2026-07-12. Zero new npm
         ...
         inside the arm-deadman → deploy → wait-for-user →
