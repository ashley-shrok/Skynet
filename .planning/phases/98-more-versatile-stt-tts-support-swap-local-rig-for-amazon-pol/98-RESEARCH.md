...
| STT disk-bank (fire-and-forget to `/app/stt-recordings/`) | API / Backend | — | Preserve verbatim — user's post-hoc reference; CONTEXT explicit |
...
|----------|-------------|------------------|
| P98-STT-01 | Voice-in end-to-end via Transcribe streaming, transcript quality ≥ Chatterbox on user's voice | Verified in exploration (5 clips, 4.9s–127.4s) — see `samples/transcribe-outputs/` |
...
| P98-TTS-01 | Voice-out end-to-end via Polly generative engine | Verified in exploration (30 synths, 5 voices × 2 engines × 3 messages, user picked generative) |
...
│  2. Disk-bank write (fire-and-forget)          │◄── PRESERVE (user reference)
...
**What goes wrong:** Long message splits into N=3 chunks. Each Polly call takes 200-400ms to first byte. Client hears chunk 1 finish, pauses ~250ms, hears chunk 2 finish, pauses ~250ms, hears chunk 3. user perceives it as "the voice stopped mid-thought."
...
**What goes wrong:** You put the disk-bank write AFTER the ffmpeg transcode step. user's post-hoc reference folder fills with `.ogg` files instead of `.webm` files. Confusing for her, and if a transcode fails the file is missing entirely.
...
**Warning signs:** user opens `/opt/skynet/stt-recordings/` and finds `.ogg` files instead of the `.webm` she recorded.
...
**Recommendation:** Option 3 — cleanest, matches the "clean cutover" spirit of the phase. Adds one task: edit `bridge.sh:217-235` to drop the STT curl, keep the placeholder message. user confirms.
...
- Total per full integration run: <$0.01. user's build-phase budget ($20-30) tolerates hundreds of runs.
...
|--------|----------|-----------|-------------------|-------------|
| P98-STT-01 | Transcribe streaming returns transcript for real user voice clip | integration (real AWS, opt-in) | `AWS_INTEGRATION_TESTS=1 npx vitest run src/backend/voice/transcribe-adapter.integration.test.ts` | ❌ Wave 0 |
...
| P98-TTS-03 | TTFB timing (empirical — measured in UAT) | manual | UAT with user on long message | manual-only |
...
- Migration script trigger: MEDIUM — three viable options; recommended "startup one-shot" is Claude's-discretion pick, user may prefer explicit docker-exec instead
