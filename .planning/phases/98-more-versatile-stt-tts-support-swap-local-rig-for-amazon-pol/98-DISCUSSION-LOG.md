...
- Provider tradeoff surfaced: OpenAI (URL-swap-ish, small catalog), ElevenLabs (huge catalog, expensive), AWS (Polly + Transcribe — cheaper, larger neural catalog, real integration effort). user picked AWS: *"it is the shape that's going to work great for the two instances of Skynet that exist in the world today that are both managed by me."*
- Discussion on Amazon Transcribe: no synchronous "POST audio, get transcript" endpoint at AWS; streaming is the only real-time option, so backend translation is more work than the OpenAI story would have been. user accepted.
- Discussion on Polly 6000/3000-char per-call ceiling. user pushed back on my characterization that it wouldn't bite in daily use: *"It is felt in daily use when you get giant messages from agents very often, so don't tell me what I experienced."* — chunk-and-stitch is a mainline concern, not an edge case.
- user proposed unifying the backend path (rather than short-vs-long branching): *"rather than have one route where the client is doing TTS and one route where the backend is doing it, I wonder if it might just make more sense to have the backend do it and keep it simple."* Agreed — locked as "one route, all backend."
- Discussion on validating quality on real distribution before committing. user asked me to DM Isabella (Aither Infra) for AWS test-drive access. Isabella forwarded to Iris (aither-infra-maintainer, who owns AWS work), who counter-proposed a better mechanism than what I asked for: attach a scoped inline policy to t1000's existing instance role (`termix-ssm-role`) so backend uses ambient IMDS creds instead of static keys. Attached in 30s.

### Beat 3 — Grill
- **Q: engine choice (neural vs generative per-identity, per-play, or global default)?** A (after user heard the A/B): *"yeah, definitely generative, because it sounded way better."* Locked: always generative, no toggle.
- **Q: multi-instance scope (t1000 first, or both t1000 + T800 day-one)?** A: neither — user's standing policy is "we just make sure to include what would need to be done at deploy time or just before it in the repo itself somewhere so that you can do those steps when it deploys over here and Stacy can follow those steps over on her side." Locked: uniform code, per-instance operator config via in-repo doc.

### Real-distribution validation
- Copied 3 real assistant messages user attached + 5 real voice clips from `/opt/skynet/stt-recordings/` (durations 4.9s to 127.4s).
- Ran 30 Polly synths across 5 voices × 2 engines × 3 messages; served over tailnet at `http://100.99.149.8:8899/` (Skynet passthrough URLs had a client-side issue, fell back to plain tailnet HTTP per user's request).
- Ran 5 Amazon Transcribe streaming runs via `amazon-transcribe` Python SDK on FLAC-transcoded clips.
- user listened through, greenlit generative-engine flat: *"I think it's doing a better job than my personal models anyway, so I approve."*
- Exploration spend: $1.14 total (Polly $1.05 + Transcribe $0.09).

### Vehicle decision
- Recommended: single GSD phase (backend adapters + frontend catalog reshape + migration + cross-tree deploy doc = phase-shaped work). user agreed: *"thumbs up."*
