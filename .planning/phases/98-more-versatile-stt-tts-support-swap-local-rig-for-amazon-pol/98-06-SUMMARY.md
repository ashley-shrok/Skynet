...
key-decisions:
  ...
  - "Disk-bank write block PRESERVED VERBATIM at the ordering point BEFORE transcodeForTranscribe (Pitfall 6 — Alice's post-hoc reference folder stays populated with the ORIGINAL .webm bytes, not the transcoded .ogg/.flac). Tested by asserting fs.promises.mkdir invocation order precedes webmToOggOpus invocation order."
