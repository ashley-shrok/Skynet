...
- Wired the migration fire-and-forget from `starter.ts` as a sibling block to `ensureBridgeConfigWritten` — zero operator action, no `docker exec` step, no distributor task. user's Phase-86 preference for ops-invisible operations honored.
