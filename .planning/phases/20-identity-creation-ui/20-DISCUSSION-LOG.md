...
| Option | Description | Selected |
|--------|-------------|----------|
| user drafts/approves each | Blocking design pass on the exact wording per step before ship | |
| Tina drafts defaults, user overrides post-ship if any read wrong | Non-blocking — reasonable defaults ship, iterate on real-world friction | ✓ |

**User's choice:** Tina drafts defaults, user overrides post-ship.
**Notes:** user verbatim: "I don't need to approve for number four." Tina's default blurbs drafted in CONTEXT.md `<decisions>` § "Failure blurbs" — 5 blurbs, one per step, each including the specific manual finish-up commands the user needs to run.
...
**Notes:** user locked homeserver-register OUT of Phase 20 scope. Historical pattern is identity-self-service: fresh identities handle their own relay setup via the /id skill's create-branch or during first-wake onboarding. user also directed Tina to relay a concern back to Nelly: "if the new relay server would require her to do that stuff instead of identities doing it themselves like they have historically, then that's a problem." Followup message sent to Nelly documenting this. Nelly's reply confirmed the server-side supports self-service registration and identified the actual gap: the /id skill's create-new-identity branch stops at writing `<name>.md` and doesn't do the register+write-relay.json step. Nelly offered to close that gap at the id-skill layer if user greenlights — that fix would be independent of Phase 20 and would make Phase 20's "leave relay entirely alone" posture work end-to-end.
...
- **Regen-while-generating behavior** — Tina picked "disable Generate button until in-flight batch resolves." user did not surface this; my call under best-we-can philosophy.
- **Stale-avatar handling** — Tina picked "silently keep picked avatar even if name/title/brief edited afterward, no warning." user did not surface this; my call under best-we-can philosophy.
- **Field persistence on failure/close** — Tina picked "reset on modal close, no draft store." user did not surface this; my call under best-we-can philosophy.
...
- **Voice list gender-splitting + defaults per gender.** user flagged during original bounty capture as an adjacent-but-separate concern.
...
- **Homeserver-register at the /id skill layer** (Nelly's proposed fix). NOT in Phase 20's scope regardless of user's greenlight decision on the id-skill patch — those are independent workstreams.
