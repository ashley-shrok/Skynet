...
**Source:** `/open` session with user 2026-08-03 (shape file locked) → `/gsd:discuss-phase` 2026-08-03 (this file). Nelly's DM 2026-08-03 supplied the bootstrap mechanism spec.
...
- Voice list gender-splitting or defaults-per-gender — user flagged as a separate concern (see `<deferred>`).
...
- Review-before-commit / rollback / retry / cancel-mid-birth — user locked these out during `/open`.
...
- CEO-instance multi-user isolation concerns — this is single-operator scope for user's box.
...
### Failure blurbs (my defaults; user overrides post-ship if any read wrong)
...
- Deploy per Tina's held-queue posture (patches #267-#288 currently held atop container `sha256:07547f6c4185`). Do NOT push / build / recreate without user's explicit greenlight.
...
- `~/.claude/identities/tina/bounties/identity-creation-ui/shape-identity-creation-ui.md` — Shape file from `/open` session with user 2026-08-03. LOCKED per the "What would make it wrong" section. Do NOT re-litigate design.
- `~/.claude/identities/tina/bounties/identity-creation-ui/bounty.json` — Bounty tracker with user's verbatim original ask + shape/vehicle timeline.
...
- **Skynet admin creds:** `~/.claude/identities/tina/skynet-creds.json` (mode 0600) — user's account creds + TOTP, used for admin cookie-authenticated Skynet API calls if the birth flow needs any (probably not — JWT auth on the existing identity routes should cover).
...
- **Deploy discipline.** Held-queue posture — do NOT push / build / recreate without user's explicit greenlight. Held atop container `sha256:07547f6c4185` per Tina's handoff.
...
- **Fresh archetype fleet-rule reminders** (Tina's tina.md): NEVER use git worktrees (user 2026-07-31 fleet rule); NO more UAT check-ins (user 2026-07-27); deploy pre-work (push/build) is NOT authorized by a code-work ask; NEVER mask exit codes with `| tail` / `| head` on build commands; frontend `tsc --noEmit` does NOT catch backend TS errors (use `npm run build:backend`).
...
<specifics>
## Specifics — user's verbatim decisions
...
- **Voice list gender-splitting + defaults per gender.** user flagged during original bounty capture as an adjacent-but-separate concern. Chatterbox voice list has 28 predefined voices; splitting them into male/female presented sets with a default per gender at creation time is its own bounty when she wants it.
...
- **Editable archetype prompt UI.** Prompt stays hidden. If user later wants a "power user" mode where she can tweak the drafted prompt before batch generation, that's its own bounty.
...
- **Homeserver-register / relay.json bootstrap for fresh identities.** user explicitly OUT. Identity self-services via /id skill's create-path or first-wake onboarding. See `<specifics>`.
- **CommandPalette variant of NewSessionDialog gets identity-mode.** OUT of Phase 20. If user later wants birth-from-command-palette, that's a small follow-up.
- **Supervisor "adopt" HTTP endpoint on Nelly's side.** Nelly offered it if we need the supervisor to pick up the new identity immediately without a service restart. Deferred until we know whether that's a real requirement based on user's usage. Ping Nelly if it becomes one.
...
*Context gathered: 2026-08-03 via `/gsd:discuss-phase 20` with user, following `/open identity-creation-ui` design lock earlier in session. Nelly's DM 2026-08-03 supplied the bootstrap-sequence spec cribbed into `<decisions>` step 3-4.*
