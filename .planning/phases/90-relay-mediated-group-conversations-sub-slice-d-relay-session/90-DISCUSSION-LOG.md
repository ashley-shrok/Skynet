...
**User's rationale:** Reuse existing visual language. user verbatim: *"the meter and button already exist in the compose box for the regular harness sessions. And so I was just picturing it as kind of a smaller version of that that is in that area that's popped out from underneath the identity badges."*
...
**User's rationale:** user verbatim: *"this should just be the same as Harness Sessions."*
...
**User's rationale:** user enumerated the entire upper-area contents and concluded none of it applies to relay sessions: reset and context meter move to per-agent badges; queue-a-message has no meaning because relay sessions never show a work-in-progress indicator; stop has no single agent to stop; thumbs-up doesn't apply; recap is redundant because the relay's message history IS the transcript already visible in the pane. Attach hidden entirely for v1 — user verbatim: *"attach could just be hidden or disabled, I don't really care which."* Went with hidden per absence-as-affordance principle.
