...
- D-24 single-deploy motion + on-host append instruction (user appends this to `/opt/skynet/Caddyfile` at ship time — deployed Caddyfile lives on-host, not in-repo)
...
- **Follow-up:** Ship-time docker build (user's `docker compose up -d --build caddy` motion during the D-24 deploy) will re-verify the module list. Not blocking for plan completion — the static acceptance criteria all pass, and the R&D proof stands.
