...
- `~/.claude/roles/box-maintainer/box-maintainer.md` — standing directives section. Especially:
  ...
  - **Deploy boundary at `git push`** — commit + scoped-green ok autonomously; `git push` needs fresh per-push greenlight from user.
...
- **Per-user disambiguation UX for the (rare) case where a user owns two hosts with the same name** — Phase 103 first-match-wins with a warning-logged event (per D-17 with user's Q5 discussion). If it ever bites in practice, add richer disambiguation then.
