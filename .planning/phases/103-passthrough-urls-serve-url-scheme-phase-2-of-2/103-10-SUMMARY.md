...
# Metrics
metrics:
  duration: "~5 min executor time (skeleton write + commit + checkpoint return); UAT itself is post-deploy and owned by user + orchestrator"
...
- [ ] user has greenlit the atomic ship motion for Phase 103 at `git push`.
...
**E. Sign into Skynet at `https://term.example.com`** with user's admin credentials. After sign-in, open devtools → Storage → Cookies and locate `skynet_session`. **Verify the `Domain` field reads `term.example.com`** (NOT blank, NOT `.term.example.com` without the leading dot handled properly — check that the cookie will match on all `*.term.example.com` subdomains per D-02). This validates Plan 02's widened cookie shipped.
...
## Live UAT transcript (executed 2026-09-10 by tabitha + user)
...
```
Verified end-to-end by tabitha + user on 2026-09-10 18:08 UTC. All 4 checks passed
...
4. Returned a `CHECKPOINT REACHED` message to the orchestrator awaiting user's post-deploy UAT run.
