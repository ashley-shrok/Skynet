...
- **CI gate for D-05 is now in place.** Any PR against the main branch that touches proxy-factory.ts (or its dependencies types.ts + header-audit-sampler.ts) automatically runs this test via `npx vitest run` in the ship-gate. The gate is at container-deploy time per box-maintainer's test discipline (user 2026-09-07: scoped during dev, full suite as first step of deploy motion). D-05's "MUST pass to merge" is now structurally enforced.
