...
- **Direct browser fetch of `http://100.x.y.z:PORT`:** Do NOT try. `python3 -m http.server` sets no CORS headers; Skynet is served over HTTPS in production (mixed-content block); the tailnet IP may not be reachable from user's browser (only Skynet is guaranteed to be on the Tailnet). Backend proxy is unavoidable.
...
- **Adding a per-file cache for "this url is known editable":** The Map<eventId, Set<url>> lives inside a hook and dies with unmount — that's correct. Do NOT persist to localStorage / sessionStorage / anywhere. user's re-load re-classifies; that's cheap and correct (the tailnet server may be dead by then anyway).
...
| SFTP / atomic-rename write for the edited file | New write path | **Not needed at all** — the save target is the ComposeBox attachment set, not the file on the agent's box. The return trip is user's next reply, per shape LOCKED. | Zero backend write; zero SSH; zero SFTP. This is the shape's central symmetry. |
...
**Warning signs:** user reports clicking the link no longer downloads/opens the file.
...
| A7 | The extension whitelist and basename set are approximately right for user's usage | Pattern 5 constants | Low — the shape doc explicitly says "grows over time as misses are noticed." Set is intentionally starter, not comprehensive. `[ASSUMED]` — user may want to add/remove entries |
...
| Tailscale CLI on user's device | Not applicable — proxy runs server-side | N/A | — | — |
...
- Extension whitelist (Pattern 5) is a starter set based on the shape doc's examples and common config formats — user may add/remove.
