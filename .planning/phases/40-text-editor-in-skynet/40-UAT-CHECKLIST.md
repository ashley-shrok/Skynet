...
- Ask an agent (e.g. "tanya, please serve me the current 40-CONTEXT.md via the id skill's serve pattern so I can UAT the text editor") — the agent's response will be the assistant-role message that carries the tailnet URL user walks Item 1 with.
...
- (c) affordance DOES appear on binary → sniff heuristic is misclassifying — but per the LOCKED shape decision, "false-positive tolerance is acceptable" (user won't save garbage), so this is a warn-only fail. Note it for a future whitelist-tightening pass, not a phase-blocker.

---

### 3. D-03: Additive-not-replacive (clicking the link still downloads/opens as before)

**Verifies decision D-03 verbatim:** "**Additive, not replacive.** The existing link behavior stays — user can still click through, download, or interact with the link exactly the way she can today. The edit affordance is a NEW action that appears ALONGSIDE the link, never in place of it."
...
**Verifies decision D-04 verbatim:** "**Fresh re-fetch at edit-open time** to get current bytes when user taps edit. **If the re-fetch fails, Skynet errors explicitly.** Do NOT silently fall back to the detection-time cached bytes. Visible failure over silent maybe-stale."
...
- Failure path shows a bare toast but no in-modal error → the error is invisible if the modal is dismissed before user reads it. Escalate — the copy must be in-body.
...
**Steps:**
1. Compare the editor modal chrome visually against the Global Files edit modal that user uses today (open a Global Files modal in a separate tab or by memory — Portal + Overlay + Content, backdrop-filter blur, blue-glass gradient, `inset-4` positioning).
...
**Verifies decision D-07 verbatim:** "Uses Skynet's existing reply-with-attachment path (well-worn, occasional upload bugs but no fundamental flaws). NO new agent-side receive convention — agents already know how to read attachments user sends them. The symmetry is: agent serves a link (their existing pattern), user replies with an attachment (her existing pattern). All novelty lives inside Skynet, in the middle."
...
2. Click **Send**.
3. Observe the message going through: your caption + the file(s) as attachments should appear in the conversation as user's turn.
4. Ask the agent: "can you cat the file I just sent?" (or the equivalent — the agent's identity skill should already know how to read attachments from user's message).
