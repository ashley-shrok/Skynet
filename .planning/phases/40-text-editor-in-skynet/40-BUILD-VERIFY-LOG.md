...
The BLOCKING human-verify checkpoint (Task 4 of this plan) is what closes the phase — user walks the UAT checklist post-deploy on production Skynet, then either replies "APPROVED — hand off to maintainer" (phase complete, patch entry pasted at ship day) or "REVISION — [notes]" (one of the three docs artifacts is revised and the checkpoint re-fires).
