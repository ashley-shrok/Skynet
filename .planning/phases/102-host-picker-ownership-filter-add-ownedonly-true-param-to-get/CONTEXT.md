...
Seeded from live conversation with Ashley 2026-09-10 during role-cosmetics-data-migration UAT.
...
(admin-visibility hosts)`. On Ashley's t1000 instance today this surfaces 6 rows
...
## Solution shape (LOCKED — Ashley 2026-09-10)
...
- **Bundled ship with newline fix** (commit `f5b6c4e0`) — Ashley's directive:
...
- `GET /host/db/host?ownedOnly=true` from an authenticated request returns ONLY
  ...
  against t1000 with Ashley's cookie: expect 3 rows (her thenasty, workstation,
...
- All 6 UI consumers pass `ownedOnly=true` and their host lists no longer show
  non-owned rows.
- Sidebar (`AppShell`) also filtered — verified by Ashley eyeballing t1000 after ship.
