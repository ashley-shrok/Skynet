...
provides:
  ...
  - "compareByRecencyDesc Rule 1 flipped null-to-top → null-to-bottom (user's 2026-08-14 no-history-to-top lock retired per 44-CONTEXT.md § Comparator change)"
...
key-files:
  ...
  modified:
    ...
    - src/ui/AppShell.tsx
decisions:
  - "Rule 1 flip is UNCONDITIONAL — no feature flag, no toggle. user's 2026-08-14 no-history-to-top lock is DELIBERATELY retired per 44-CONTEXT.md § Comparator change. Rationale locked at CONTEXT.md L59-63: null-to-bottom is user's new preferred contract; no-history rows sink to bottom of middle rather than hoisting above real-message rows."
...
- `docker build` + `docker compose up -d --force-recreate skynet` (orchestrator scope — user 2026-08-08 fleet directive).
