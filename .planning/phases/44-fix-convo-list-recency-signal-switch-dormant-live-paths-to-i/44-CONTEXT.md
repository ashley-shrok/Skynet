...
**Source:** Authored from operator ↔ user conversation 2026-08-18 (no separate /gsd:discuss-phase run — decisions locked inline before planning).
...
- Scroll-anchor engineering in the panel. Once the source is correct, the tier-flip churn drops sharply and the browser's default `overflow-anchor: auto` should suffice for the residual case. Revisit only if user reports lingering scroll lurches after this ships.
- Any change to the message-bearing kind filter (still `message`, `image`, `relay_outbound`, `relay_inbound`). user's Phase 42 lock — "activity = message either direction, and only that" — stands.
...
- Broader message-bearing kind expansion. user's lock stands.
...
*Context authored: 2026-08-18 from operator ↔ user conversation (no discuss-phase run — decisions locked inline).*
