...
**Notes:** Presented with my lean toward A. user agreed. Rationale: single wire contract is cleaner from the modal's perspective; the connection-hold time isn't a real concern given typical fleet density.
...
Decisions where user's earlier locked answers (from `/open`) fully covered scope, so no discuss-phase re-ask was needed:
...
- **Downstream deployment (T800 / Stacy) pull-through** — batched for after all 3 chunks land, per user's direction.
