...
## Seed-Comment Approach Compliance (user's mid-flow refinement)
...
|------|------|---------|----------|--------|
| — | — | — | — | None flagged. All seed-comment string constants pass user's negative assertions (no Skynet, no §2/§3/id-skill/SKILL.md). No debt markers (TBD/FIXME/XXX) introduced by Phase 22 code. No hardcoded empty props on any new component. |
...
1. user opens NewSessionDialog on a live fleet host → Role dropdown populates from `~/.claude/roles/` on that host.
2. user creates an identity → verifies new `~/.claude/identities/<name>/<name>.md` on target host contains `role:` frontmatter + seed comment.
3. user right-clicks a conversation row → CloneAgentDialog opens with source pre-fill; submit creates new fleet folder with source's role.
4. user clicks `+ New role` → dialog opens with chain checkbox CHECKED; submit chains to NewSessionDialog with role+host pre-filled but editable.
5. user opens IdentityModal → Role tab is FIRST + DEFAULT; edits + saves; verifies role file updated on disk via SSH cat.
