...
- **`Then create an identity with this role` defaults to TRUE.** user: "obviously going to want an identity to take on the new role otherwise you'll have a role without any identities."
...
- **No no-role fallback branches anywhere.** user confirmed 2026-08-04 no fleet identity lacks `role:` frontmatter post-migration. Any plan that adds "graceful (no role)" fallback branches or empty-state handling is a plan-checker BLOCK (dead code).
...
- **No-role fallback / graceful empty branches.** user confirmed no such identities exist post-migration; adding dead code branches is a plan-checker BLOCK.
...
**What goes wrong:** Dead code — user confirmed 2026-08-04 no fleet identity lacks `role:` frontmatter post-migration. Any `(no role)` empty state renders never (validated by grep across the fleet).
...
1. **How does the birth orchestrator write the `role:` frontmatter into `<name>.md` when the current flow delegates identity-file creation to the /id skill on the box?**
   ...
   - **RESOLVED:** B4b(a) locked. Plan 22-02 Task 2 is a `checkpoint:human-verify` where user confirms the ~40-line birth-orchestrator change before it lands; Task 3 implements the pre-write of `<name>.md` + `wakeups/` + `handoff.md` + relay-register via SSH inside Step 2's completion path (silent — no new SSE event type).
