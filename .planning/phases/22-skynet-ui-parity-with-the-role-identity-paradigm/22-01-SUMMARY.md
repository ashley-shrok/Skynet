...
tech-stack:
  ...
  patterns:
    ...
    - "No-fallback semantics: helper THROWS on missing role/frontmatter (D-CONTEXT LOCKED 2026-08-04 — user: no such identities exist post-migration; graceful fallback = dead code = plan-checker BLOCK)"
...
**Manual UAT gate (deferred to Phase 22 UAT per ROADMAP):** user clicking through IdentityModal → Bounties tab → seeing bounties from the role folder. Cannot be automated because it requires a real fleet identity + role folder pair on a live host.
