...
The plan flagged a possible pixel-tuning escape hatch — adding a `.pv-role-prominent` class if `<strong>` inside `.pv-ai-title` renders too heavy against the base weight (D-03 executor discretion). No such adjustment was needed at implementation time; the `<strong>` inside `.pv-ai-title`'s weight-500 base gives a natural prominence bump that reads correctly. If user's real-browser UAT surfaces a mismatch, a follow-up plan (or an inline-* commit) can add the class without touching the JSX.
...
| T-80-08-03  | accept      | aiTitle drop is a product decision (user greenlit); not a security concern. |
