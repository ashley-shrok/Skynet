...
- **D-01 storage substrate landed.** `docker/pool-defaults/pool.json` ships with 7 PascalCase placeholder names (Willow, Cinder, Aster, Vega, Onyx, Sable, Fig); user's vetted list will overwrite this file in parallel and ship at ship time — no code change required to swap contents.
...
- **Ship-time coordination note (for orchestrator):** user's vetted ~1800→~subset name list can drop into `docker/pool-defaults/pool.json` at any time — pure content edit, no code touch required. The loader's shape guard accepts any non-empty array of non-empty PascalCase strings.
