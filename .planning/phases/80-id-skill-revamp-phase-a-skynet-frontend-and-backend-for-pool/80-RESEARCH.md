...
The "role select bug" is NOT a missing dropdown — the dropdown is fully wired at `NewSessionDialog.tsx:961-1010`. Read carefully: the dropdown is gated on `selectedHost !== null && identityMode`. If the user opens the modal WITHOUT picking a host first, no role selector appears — this is likely what user experienced. Confirm during planning whether the bug is "missing role dropdown" (this) or something else, but the existing role dropdown machinery is intact.
...
| `docker/pool-defaults/pool.json` | Vetted-pool JSON seed. Shape: `{ "names": ["Willow", "Cinder", "Aster", "Vega", "Onyx", "Sable", "Fig", …] }`. user finalizes contents in parallel; Phase 80 ships with whatever is landed. | Mirror `docker/branding-defaults/branding.json` |
...
**Impact:** If the unified modal defaults to `identityMode=true` but no host is picked, no role selector appears. This is likely what user experienced as "role selection isn't offered." The fix isn't "add a role dropdown" (it's already there) — it's to make sure:
...
**During planning:** confirm with user which of these is the actual bug she experienced. The dropdown-not-visible-until-host-picked is definitely one axis; there may be additional axes (e.g., the state resets when identityMode toggles, or the role field submits empty and backend rejects).
...
**Where:** Pool loader memoizes at first read (mirrors `branding-config-loader.ts:118`). If user updates `/app/pool-defaults/pool.json` and reloads the container, the new pool takes effect. But if she hot-edits without restart, the memoized value stays stale.
...
1. Create `docker/pool-defaults/pool.json` with initial contents (e.g., 5-10 placeholder names user provides in parallel).
...
| A4 | "Role select bug" = role dropdown gated on host being picked (line 961). Fix = auto-pick sole host / add "pick host first" hint | Landmines §2 | If the actual bug is different (e.g., state resets on identityMode toggle, form submits with empty role), the fix is different. Confirm with user or reproduce during executor implementation. |
...
| A8 | Pool loader ships with whatever user's vetted list is at ship time (shape file: "Phase 80 ships with whatever vetted list is landed at ship time") | Stream 1 | user has parallel-vetting responsibility. Coordination gate is at ship, not at plan. |
...
3. **A4 (actual "role select bug" repro steps)** — request user reproduce or provide expected behavior. Recommendation: default identity-mode ON + auto-pick sole host + "pick host first" hint.
...
- "Role select bug" root cause: MEDIUM — inferred from code inspection; not confirmed with user
- Identity key vs MXID localpart divergence: MEDIUM — inferred from shape file's phrasing; needs user confirmation
