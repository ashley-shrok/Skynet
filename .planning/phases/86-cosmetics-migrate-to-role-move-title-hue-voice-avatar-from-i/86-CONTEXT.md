...
**Source:** In-session `/build feature-mode` → `/open` shape lock 2026-09-07 (greenlit `thumbs up` same session). Bounty `cosmetics-migrate-to-role` (pinned by user 2026-09-07, item 1b in the reordered UX-pass campaign — upstream dependency carved out of `create-agent-modal-ux-pass` /open discussion). Shape file: `.planning/shapes/shape-cosmetics-migrate-to-role.md`.
...
Applies uniformly to all four fields. All four fields are structurally symmetric (user 2026-09-07: "one shared image" for avatar — no per-identity generation within a role-defined style).
...
user 2026-09-07 verbatim: *"Rolls can't have empty cosmetics with the flows that we have set up, so it's not an issue to solve."*
...
### Test discipline (per campaign constraint — user 2026-09-07)
...
- Per-identity avatar generation within a role-defined style (user picked "one shared image" during /open — role owns one avatar, all inheriting identities show it).
...
user 2026-09-07 verbatim on the campaign constraint:
...
- No coord-room BEFORE/AFTER posts (there is no ship; the pushes go without coord posts per user 2026-09-05 push-only rule).
...
Reordered lineup (user 2026-09-07, tabitha owns the order-keeping per user's delegation):
