...
3. **Section 6 entirely (Register the agent on the Matrix relay):** the
   ...
   heading that avoids "matrix"/"homeserver" (unlikely to satisfy user's
...
- **Fix:** Created both files from scratch. For Task 1 the created doc is
  ...
  homeserver" claim user flagged; the plan's grep-0 gate forced removal
