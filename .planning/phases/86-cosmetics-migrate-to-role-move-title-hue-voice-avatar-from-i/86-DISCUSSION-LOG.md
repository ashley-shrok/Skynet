...
**Participants:** user (visionary), tabitha (builder).
...
**user's answer (verbatim):** *"Yeah, I would leave this alone because I actually plan on breaking out the role level stuff into its own modal later, and that would be a good opportunity to add it."*

**Resolution:** (a) — creation-only for Phase 86. Role-cosmetic-edit-post-creation deferred to the future role-level modal breakout user is planning. Captured in CONTEXT.md `<deferred>` section.
...
- Post-creation role cosmetic edit UI → future role-level modal bounty (user's planned breakout).
