...
key-decisions:
  ...
  - "Extraction poller marker-disappearance detection runs FIRST — before the streaming/stable branches. Preserves cross-tab dismiss coherence when user externally Escapes via SSH: broadcastAsideDismissed fans out (dismiss frame + peer state flip) so all tabs clear their aside."
...
- Frontend receives `{type: "aside_dismissed"}` when any tab dismisses OR when the poller observes the marker disappearing externally (user pressed Escape via SSH, tmux died).
