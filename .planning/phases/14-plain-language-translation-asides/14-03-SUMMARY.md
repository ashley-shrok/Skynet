...
None. Pure code additions to frontend TypeScript + React; no environment variables, no external services, no infrastructure changes. Wave 4 will land the ComposeBox morph body; still no user setup required. Wave 5 (integration + smoke tests) may need user to eyeball the aesthetic in situ once end-to-end wiring is complete — but that's a Wave 5 concern, not a Wave 3 setup step.
...
**Wave 5 (integration + smoke tests) is the natural place for end-to-end aesthetic UAT** — user eyeballs the aside in situ across the full stack (arm emit → backend inject → poller extract → aside_ready → AsideBubble render → X click → dismiss round-trip → cross-tab dismiss).
