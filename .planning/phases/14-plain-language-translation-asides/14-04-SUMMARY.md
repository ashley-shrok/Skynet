...
None. Pure code changes to frontend TypeScript + React; no environment variables, no external services, no infrastructure changes. Wave 5 (integration + smoke tests) may need user to eyeball the full aside cycle in situ once end-to-end wiring is validated — but that's a Wave 5 concern, not a Wave 4 setup step.
