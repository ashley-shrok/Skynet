...
### 11. Locked aesthetic — 10px border + three-layer glow (user aesthetic sign-off from aside-visual-snippet.js)
...
- **Queued #150 A** (pruner fleet-aware — `7f63a4b`, fixes user's live-hit pin-nuke; solo-deploy carveout applied at the time was moot because this bundle absorbs it)
...
- Pin **#150 A** (pruner fleet-aware) — if not already pinned solo. Per quick task `260726-l1p` timeline, deploy was deferred pending user greenlight; the solo-deploy carveout is now moot because #150 A ships with this bundle.
...
**When the deploy actually happens, Tina PRE-WARNS user in the deploy notification** that the first hard-refresh may white-screen and the fix is close+reopen. Do NOT jump to rollback on the first PROTOCOL_ERROR report.
...
2. **Ask user for explicit go-ahead for THIS deploy window** — a distinct greenlight, not carried over from earlier code-work authorization. Every build → deploy transition is a new "may I?" moment.
...
After user's UAT signs off (mobile + desktop both viewports), the bounty at `~/.claude/identities/tina/bounties/plain-language-translation-asides/` closes. The full aside subsystem shipped as designed:
