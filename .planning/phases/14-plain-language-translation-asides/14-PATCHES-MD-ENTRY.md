...
## Draft (paste-ready — matches the multi-commit-under-one-pin convention from patches #104, #105, #128, #138, #139, #140)
   ...
   151. `feat(pretty-view,claude-session): patch #151 — plain-language
        ...
        * **Motivating gap** (user's direct call-outs, 2026-07-26 design
          session, full timeline at `~/.claude/identities/tina/bounties/
          plain-language-translation-asides/bounty.json`):
          user bounces between ~5 active identity sessions doing "answering
          ...
          session with user, full spec locked) + `~/.claude/identities/
          tina/bounties/plain-language-translation-asides/aside-visual-
          snippet.js` (DevTools recipe user signed off on 2026-07-26,
        ...
        * **Fix summary — backend WS subsystem (Wave 2).** Added ~370
          ...
          user externally Escapes via SSH, then streaming/stable branch
        ...
        * **Fix summary — frontend AsideBubble + PrettyView wiring
          ...
          snippet.js prototype user signed off on. whitespace-pre-wrap
          ...
          confirms — user's "pretty-view chat surface interior is
        ...
        * **Fix summary — ComposeBox morph body (Wave 4).** Extended
          ...
          AsideBubble above; user 2026-07-26 "Style change to visually
        ...
        * **Deploy notes.** Ships BUNDLED with queued patches #150 A +
          ...
          `docker compose up -d --force-recreate skynet`; pre-warn user
          ...
          does NOT apply — retired 2026-07-21, replaced by user's SSM-
        ...
        * **Bounty reference.** `~/.claude/identities/tina/bounties/plain-
          language-translation-asides/` — closes on user UAT sign-off.
        ...
        * **Locked design decisions** (per CONTEXT.md, no re-litigation):
          ...
          - **Locked aesthetic** (aside-visual-snippet.js prototype user
        ...
        * **Design source-of-truth** (LOCKED, no re-litigation per
          ...
          - `~/.claude/identities/tina/bounties/plain-language-translation-
            ...
            user, empirical kumquat-test verification of /btw mechanism,
            all LOCKED design decisions in timeline entries 15:52-15:58Z.
          - `~/.claude/identities/tina/bounties/plain-language-translation-
            asides/aside-visual-snippet.js` — DevTools recipe user
        ...
        * **Commits** (all on `feat/tab-title-from-tmux`, in order landed):
          ...
          - **Plan 06 (Wave 6 — deploy checkpoint)**: 2+ commits
            ...
            - `[Plan-06 summary SHA]` docs(14-06): summary — Phase 14 code-complete + deploy staged for user greenlight
        ...
        * **Bounty closeout note**. user's UAT sign-off on this patch
...
4. **Close `/close plain-language-translation-asides`** bounty at `~/.claude/identities/tina/bounties/plain-language-translation-asides/`. This is the load-bearing closeout for the Phase 14 feature. All design decisions + implementation decisions + test coverage + user UAT are complete.
5. **Update `~/.claude/identities/tina/tina.md`** compact overview if warranted. The compose-box + pretty-view sections may want to reflect the new aside subsystem as "shape-complete" for future-me's mental-model anchoring. If user's next bounties are aside-refinement work (worth-explaining Haiku filter revisit, aside stacking / history, alternative dismiss gestures), tina.md's box-map may want a "Phase 14 aside subsystem shipped — see bounty archive for design decisions" pointer.
