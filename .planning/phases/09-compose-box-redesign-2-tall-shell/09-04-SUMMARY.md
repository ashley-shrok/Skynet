# Plan 09-04 — Alice UAT (human-verify checkpoint)
...
2. **UAT round 1 findings** (Alice in-line):
   ...
   - Meter showed three colors at once (per-position banding); Alice
...
- **Submit-bug incident logged** (`bounties/messages-land-in-box-not-submitting/`):
  Alice reported the "message lands in Claude Code composer without
...
- **New URGENT bounty parked** (`bounties/pretty-view-plan-mode-not-rendering/`):
  Vicky was live in plan mode when Alice filed via `/bounty` — plan-mode
