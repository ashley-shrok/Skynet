# Plan 09-04 — user UAT (human-verify checkpoint)
...
2. **UAT round 1 findings** (user in-line):
   ...
   - Meter showed three colors at once (per-position banding); user
...
- **Submit-bug incident logged** (`bounties/messages-land-in-box-not-submitting/`):
  user reported the "message lands in Claude Code composer without
...
- **New URGENT bounty parked** (`bounties/pretty-view-plan-mode-not-rendering/`):
  Vicky was live in plan mode when user filed via `/bounty` — plan-mode
