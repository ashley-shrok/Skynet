...
**Source:** Direct discussion with user 2026-08-19 → 2026-08-20 (verbatim quotes below).

<domain>
## Phase Boundary

user reported that Nelly's most recent outbound relay bubble in PrettyView
rendered only the words "relaying user" — the rest of her ~1000-char body
...
This phase implements user's chosen fix: an upfront sanitize pass over the
...
- **SELF-REFERENTIAL fixture:** a `BODY=$(cat <<'EOF' … EOF)` command whose
  ...
  — but user greenlit accepting the self-referential case since it's rare
...
### user's stated priorities (verbatim, 2026-08-19)
...
Deferred rationale (user greenlit 2026-08-20): the self-referential class
...
fixture. user greenlit the simpler sanitize pass alone; heredoc-first is
...
*Context gathered: 2026-08-20 via direct discussion with user*
