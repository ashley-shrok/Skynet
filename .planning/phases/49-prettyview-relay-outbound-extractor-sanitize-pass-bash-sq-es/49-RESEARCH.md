...
tabulating the delta. user greenlit the approach 2026-08-20. Full prototype
...
(including Nelly) actually use. When the regex hits `BODY='Relaying user'"'"'s reply: …'`,
it captures `Relaying user` and stops at the first bare `'`.

## The fix

user (2026-08-20, verbatim): *"what if we strip the characters from the
...
curl -sS -X PUT "$BASE/rooms/$ROOM/send/m.room.message/$TXID" \\
  -d "$(jq -nc --arg b "$BODY" '{msgtype:"m.text", body:$b}')"`;
    // Current behavior: BODY-sq matches the inner substring, returns 'relaying user'.
    expect(extractOutboundBody(cmd)).toBe("relaying user");
  });
});
```
...
- **Self-referential heredoc-content-bleed:** documented, deferred per user
