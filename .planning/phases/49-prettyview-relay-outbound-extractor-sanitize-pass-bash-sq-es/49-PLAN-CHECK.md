...
| 4 | NELLY-SHAPE fixture expects full apostrophe-bearing body | Task 2 § (A) — fixture cmd contains `BODY='Relaying user'"'"'s reply: hi'` in a valid outbound curl command; expectedBody: `"Relaying user's reply: hi"` (26 chars, apostrophe restored). | PASS |
| 5 | SELF-REFERENTIAL KNOWN-LIMITATION test documents (not asserts fix) | Task 2 § (B) — new `describe("extractOutboundBody — known limitations")` block; `expect(extractOutboundBody(cmd)).toBe("relaying user")` — documents current substring-bleed behavior. Deferred rationale cited from CONTEXT.md § Deferred Ideas. | PASS |
...
| No attempt to fix deferred self-referential heredoc-content-bleed | Task 2 § (B) SELF-REFERENTIAL test explicitly asserts current-behavior `toBe("relaying user")` with in-code comment "documented, not fixed by Phase 49" and cites CONTEXT.md § Deferred Ideas. No reorder of strategy priority in Task 1. No heredoc-content pre-mask logic added. COMPLIANT. |
