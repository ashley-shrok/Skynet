...
**Source:** In-chat design lock with user 2026-08-06 (bounty `conversation-list-needs-desk-count-and-filter-menu`)
...
<decisions>
## Implementation Decisions (locked with user 2026-08-06)
...
- Both halves inherit `--pv-hue` (no dual hues — user did not lock a two-hue scheme).
...
Symmetric with the existing pinned filter — the needs-desk filter also exempts active-set rows (active-set rows always render regardless of filter state). Rationale: active-set is user's "currently paying attention to" set; filters must never hide those.
...
*Context gathered: 2026-08-06 via in-chat design lock with user*
