...
Not executed as part of this executor run — fleet directive is no docker / no playwright in executor scope. Live browser verification is user's `/close`-time gate for this phase: open a relay room, observe the URL fragment updates to include `relay:<encoded>`; refresh the browser; observe the relay tab restores. The unit-test coverage plus the shipped grep gates give high confidence the wiring is correct; live UAT closes the loop.
