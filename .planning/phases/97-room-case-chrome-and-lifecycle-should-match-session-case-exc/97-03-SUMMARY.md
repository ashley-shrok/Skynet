...
decisions:
  ...
  - "Placeholder capital-M `Message room…` locked (D-14 SOFT — matches harness template's `Message ${identityName}…` capitalization convention). user's `message room` shorthand read as informal, not a design directive."
