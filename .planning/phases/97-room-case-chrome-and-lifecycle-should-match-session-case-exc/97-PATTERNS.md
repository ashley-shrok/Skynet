...
**Sequence-front task per D-07:** Add a temporary `[pv-split-drop-diag]` structured log at native `dragover` / `drop` on a relay-showing Pane. Reproduce user's flow:
...
**Verdict paths:**
- **If plain-session Pane behaves correctly after room open/close cycle** → user's "shared-state corruption" is a red herring. The fix reduces to threading `tabId` through MultiBadgeAnchor (above). NO SplitView change ships. Diagnostic log can stay as ambient instrumentation OR be removed.
...
**Recommendation from RESEARCH:** Option B is cleaner (preserves geometry byte-for-byte; user's ask is parity in feel). Option A requires a fine-tune of the `pt-N` value against the QueuePlusTab's absolute offset. Executor picks based on live visual review.
...
**Landmines from RESEARCH:**
- **Do NOT hide QueuePlusTab in relay mode.** user wants the top-edge affordance to work (RESEARCH § Finding 3 landmines).
...
**Analog:** None (there is no session-case analog for multi-badge — the harness case has exactly one badge). The reference is user's judgment "halve the gap."
...
- **`Message` capital M** matches harness template convention. user's shorthand `"message room"` was informality, not a design decision. Lock on `Message room…` (SOFT verify at plan-check per RESEARCH § Open Questions).
...
### Harness case regression floor (D-01 no-accidental-inheritance — user's `/close` yardstick)
