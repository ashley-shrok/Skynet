...
Every one of user's seven UAT findings is a case-branch that Phase 93's architecture required but never made. The findings fall into three shapes:
...
**The message list itself DOES mount underneath** — L3830-3832 gate includes `source.kind === "relay"` which overrides the `status === "streaming"` requirement. So user sees messages painting under a scrim that never dismisses.
...
**The signal user wants is history_batch**, not `session`, because `session` fires immediately on WS auth-pass and predates any actual message-loading work. Choose Option A2 for correctness. `[VERIFIED: use-relay-adapter.ts:430-446]`
...
The relay case's analog is: relay adapter's WS attaches → `session` frame lands (adapter marks itself connected) → `history_batch` frame lands (backend has read the room's history and shipped a batch). `history_batch` is the analog of "the harness backend has started tailing the JSONL" — that's when user genuinely should see the veil come down.
...
- **Do NOT dismiss on `session` frame alone.** That's what `isReady` already tracks; user's finding says the veil should not dismiss until messages have loaded (or an empty-room verdict has landed).
...
**The dragload-corruption claim from user** ("split-view for even plain sessions is disturbed after a room has been opened") is the harder half of this finding. Diagnostic hypotheses in priority order:
...
**The most likely single cause is (a) — the missing drag source.** user's specific complaint is "dragging a room-showing surface into an empty split slot opens the room in that slot" — that's a drag-SOURCE ask, and it's currently impossible because the relay badge has no tabId wired. The "shared-state corruption" claim needs live diagnosis with browser DevTools instrumentation before we can size it.
...
2. **Drop-target sanity.** Verify (during discovery — not a fix, a check) that a relay-showing Pane's native drop-target listener attaches, fires on drops, and cleans up cleanly on Pane unmount / path change. Instrument the `[pv-split-drop]` and `[pv-split-preview]` logs (already present) and reproduce user's page-reload-clears-it observation. If clean, split-out DOES NOT apply — findings 1, 3, 4, 5, 6, 7 (and finding 2 as a simple drag-source-add case-branch) can ship as one arc.
...
- Manually reproduce user's flow: open plain session → open room → close room → try to drag another session onto the plain session's Pane. Confirm what breaks: does the plain-session Pane's dragover still fire? Does the coral overlay still paint? Does the drop still route to `onOpenSessionInTree`?
- If the plain-session Pane behaves correctly after the room open/close cycle → confirms user's "shared-state corruption" is a red herring; the arc's fix reduces to problem (a) alone (wire tabId through MultiBadgeAnchor). Split-out DOES NOT apply.
...
`showPaperclip` is passed from PrettyView.tsx:4264 as `true`. When `mode === "relay"`, the Paperclip itself doesn't render, but the textarea's `pl-11` (44px left padding) is still applied. That 44px is the left gutter user sees — space reserved for a Paperclip that isn't there.
...
Alternative: instead of adding padding, take the Row 1 skeleton down to a `mode === "relay"` invisible spacer that carries the same `mb-[3px]` bottom margin as Row 1. Cleaner because it preserves the vertical geometry byte-for-byte. user's ask is parity in feel; a matching invisible spacer is the least-invasive way to get it.
...
- **Do NOT hide the pebble in relay mode.** user wants the top-edge affordance to work in relay mode (that's the whole point of finding 3's third symptom). The pebble's onClick calls `onAdd` which prepends a new empty queue slot at index 0 — the queue-a-message mechanism is a Row 2 concept, not a Row 1 concept, and it's meant to work in both cases.
...
- **Do NOT let the reflow spill into the harness case.** All new class tokens must be `mode === "relay" && ...` gated OR must be visually equivalent to the harness case's existing behavior. Regression floor is user's harness-mode compose looks byte-identical.
...
There is no session-case analog for a multi-badge row — the harness case has exactly one badge. The reference is user's judgment ("halve the gap").
...
`pvIdentity` is derived from the harness session's identity (search PrettyView.tsx around L1809-1823). In the relay case, `pvIdentity` is undefined (there's no fleet identity for a room), so `identityName` is `undefined`, and the placeholder falls back to `Message Claude…`. That's user's complaint — a room isn't Claude.
...
The `Message ${identityName || "Claude"}…` template at ComposeBox.tsx:2750 renders `Message room…` for relay. user's verbatim ask is `"message room"` (lower-case `m` in `message` was probably shorthand). The current template is `Message …` with capital M. Verify with user (or lock as "Message room…" and let the reviewer flag if it's wrong).
...
- **`Message` capital M** matches the harness case; user's shorthand `"message room"` was almost certainly all-lowercase informality, not a design decision. Confirm with the reviewer — locking on `Message room…` (matching the harness template's capitalization convention) is the safer bet.
...
Round-trip on refresh: the URL fragment doesn't carry any `relay:` param → `consumePendingWorkspace` at tab-url.ts:211-251 has nothing to restore for the relay tab → AppShell's tab-restore path doesn't reopen it → user refreshes and the room is gone.
...
Two SOFT judgments below that are not `[ASSUMED]` claims but might benefit from user confirmation at plan-check time:

- **Placeholder capitalization** (Finding 6): user's verbatim `"message room"` was lower-case, but the existing template uses `Message …` (capital M). I recommend locking on `Message room…` to match the template pattern. If user wants strict lower-case (`"message room…"`), it's a one-word change; verify at plan-check.
...
2. **Does the relay-badge tabId drag-source wire "the entire room tab" or "a per-participant handle"?**
   - What we know: The harness IdentityBadge drag-source drags the whole tab. That's the semantic user wants for relay too (Phase 97 D-05).
