...
must_haves:
  truths:
    - "user's UAT reproduction of 'plain-session split-view broken after opening a room' is either confirmed (structural reshape needed → split-out) or refuted (case-branch fill-in only, ship F-2 fix in Plan 06)."
...
<task type="auto" tdd="false">
  <name>Task 3: Reproduce user's flow + write DISCOVERY-NOTES.md with split-out verdict</name>
  ...
  <action>
    Reproduce user's flow in a live browser (Chrome DevTools console open, filter for `pv-split-drop`), and produce `.planning/phases/97-room-case-chrome-and-lifecycle-should-match-session-case-exc/97-01-DISCOVERY-NOTES.md` documenting:
    ...
    3. **Reproduction steps B (user's "shared-state corruption" claim, per D-06):**
       a. Open the app in a fresh tab.
       b. Open plain session A.
       c. Open a relay room.
       d. Close the relay room's tab.
       e. From the tab bar, drag plain session B (a second session tab) onto plain session A's Pane center zone — expect a split to open (this is the baseline session→session drag-drop flow that user reports is broken).
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    2. Confirm the reproduction steps A and B match user's UAT findings (open plain session → open room → try drag-source; open plain session A → open room → close room → drag plain session B onto A).
