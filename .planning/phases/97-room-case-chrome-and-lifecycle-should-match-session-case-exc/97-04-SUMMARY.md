...
key-decisions:
  ...
  - "Task 2 human-verify APPROVED AS-IS by user (thumbs up) at the code level — byte-for-byte prototype match was accepted without live-browser tuck-depth adjustment. Live rendering deferred to phase-end deploy per standard fleet pattern; if tuck reads differently on real DOM, adjustment happens at `/close` time."
...
- **F-5 shipped:** the appendage below each agent's IdentityBadge in the relay case now reads visually as a drawer peeking from behind the pill, per user's tasting decision (D-12 "simple slotted drawer" variant, 2026-09-10).
...
- **Task 2 (checkpoint:human-verify) APPROVED AS-IS by user** — code-level review of the byte-for-byte prototype match; no adjustment requested.
...
- **Task 2 human-verify:** APPROVED AS-IS by user (thumbs up, resume signal "approved") — user reviewed the byte-for-byte prototype match at the code level and greenlit the current -8px tuck depth without adjustment. Live-browser tuck-depth verification is deferred to phase-end deploy per standard fleet pattern.
...
- **Tuck depth locked at -8px:** the prototype's exact value, dead-center of D-13's 6-10px executor tolerance. No live-browser fine-tune executed because user approved the byte-for-byte match at the code level; if the rendered tuck reads differently once deployed, adjustment happens at `/close`.
...
None. The pill-is-absolute-positioned-inside-its-parent-cell landmine called out in RESEARCH § Finding 5 did not manifest — the `-mt-2` on the drawer wrapper achieved the intended tuck at the code level (user confirmed). The absolute-positioning fallback in the plan was not needed.
