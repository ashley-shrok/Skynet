...
human_verification:
  ...
  - test: "F-2 drag-drop end-to-end — open a plain terminal session in one tab and a relay room in another. Drag any participant badge from the room-showing surface onto the plain session's Pane. Room opens as a split. Then close the room, drag another session onto the same Pane — session split-view still works after room mount/unmount cycle (Ashley's page-reload-clears-it observation)."
  ...
  - test: "F-5 meter drawer tuck geometry — open a relay room with at least one agent participant. The meter appendage should read as 'peeking from behind the pill' (drawer chrome). The pill's drop-shadow should visibly land on the drawer surface."
    ...
    why_human: "Byte-for-byte code match to Variant A prototype confirmed at code level; live rendering on real fonts + browser stacking may differ. Ashley approved code-level match; live-browser fine-tune deferred to /close per Plan 04 Task 2 resolution."
  - test: "F-6 placeholder copy visual — open a relay room. Focus the ComposeBox textarea. Placeholder text reads exactly `Message room…` (capital M, U+2026 ellipsis)."
    expected: "Placeholder text `Message room…`. If Ashley wanted strict lowercase `message room…` (per verbatim shorthand), this ships capital-M per the harness template convention and needs Ashley's call at /close. See WARNING-5 note below."
    why_human: "SOFT judgment — Ashley's verbatim was lowercase `message room` (shorthand); RESEARCH § Open Questions locked capital-M `Message room…` to match harness template pattern. Ashley to confirm at /close whether the capital-M choice reads right."
  ...
  - test: "D-01 regression floor — open a plain harness session (any host). Everything looks and behaves exactly as before Phase 97: Row 1 instrument bar present, Paperclip button renders inside textarea, single IdentityBadge at the top-right corner, veil dismisses on pane_state:active, URL persists as tmux:<host>:<session>."
    expected: "Zero visible or behavioral change from pre-Phase-97 baseline for the harness case. Ashley's reviewer yardstick per D-01: 'no accidental inheritance.'"
...
**Phase Goal:** Room case of the two-source chat surface (relay rooms) should feel identical to session case (harness sessions) except in the specific places we've deliberately case-branched — no accidental inheritance. Address the 7 findings from Ashley's UAT walkthrough of the just-shipped Phase 93.
...
| 6 | **F-6** Placeholder copy `Message room…` in relay mode | VERIFIED | `PrettyView.tsx` L4288 `identityName={source.kind === "relay" ? "room" : pvIdentity?.displayName}`. `ComposeBox.tsx` L2773 template `\`Message ${identityName \|\| "Claude"}…\`` byte-untouched with U+2026 ellipsis (byte sequence `e2 80 a6` confirmed via `od -An -tx1`). Renders `Message room…` for relay. **WARNING-5 (capitalization SOFT judgment):** ships capital-M per harness template convention; Ashley's verbatim shorthand was lowercase `message room` — surfaced to /close for Ashley's call. |
...
| `.planning/phases/97-.../97-01-DISCOVERY-NOTES.md` | Discovery + preliminary Verdict A + Ashley approval | VERIFIED | 156 lines; Resolution section records verbatim `"approved verdict a"` from Ashley |
...
| D-06 Shared-state corruption in scope | RECOGNIZED (Verdict A) | Static analysis concluded no evidence of corruption; H1/H2/H4/H5 all clean under code review; H3 ruled out. Ashley approved on static basis; live confirmation deferred to phase-end |
...
| D-14 Placeholder reads `message room` | RECOGNIZED (with WARNING-5) | Ships `Message room…` (capital-M per template convention); Ashley's verbatim was lowercase. Surfaced for /close |
...
|------|------|---------|----------|--------|
| PrettyView.tsx | L4288 | Ships `Message room…` (capital M) rather than Ashley's verbatim shorthand `message room` (lowercase) | Info (WARNING-5 iter-1 plan-check) | RESEARCH § Open Questions locked capital-M per harness template convention; needs Ashley's confirmation at /close whether the choice reads right |
...
| Drag-drop fix leaves general state corruption unaddressed | ADDRESSED (Verdict A) | Static analysis concluded no evidence of corruption. Verdict A approved by Ashley. Live-browser reproduction of Ashley's page-reload-clears-it observation deferred to phase-end deploy; `[pv-split-drop-diag]` forensic instrumentation shipped for escalation if needed |
...
See `human_verification` in frontmatter. Nine items routed for Ashley's live-browser confirmation at /close, all deriving from runtime paths or visual judgments that scoped tests cannot cover.
...
**One SOFT judgment (WARNING-5) surfaced for /close:** placeholder capitalization ships `Message room…` (capital M per harness template convention) while Ashley's verbatim shorthand was lowercase `message room`. RESEARCH § Open Questions and Plan 03 SUMMARY documented this as a SOFT judgment to lock at /close; not a code bug.

**Live-browser confirmation deferred to phase-end deploy** per standard fleet pattern for the following: F-1 veil dismissal on empty and populated rooms, F-2 drag-drop full round-trip (including Ashley's page-reload-clears-it observation — the Verdict-A escalation gate to Verdict B if reproduction fails), F-3 vertical parity within 1px, F-4 gap-1 visual reading, F-5 drawer tuck geometry on real DOM, F-6 placeholder read, F-7 Chrome window-restore round-trip, and D-01 harness regression floor visual pass.
