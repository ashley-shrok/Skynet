...
**Source:** Direct capture of user ↔ Tina design conversation (2026-08-12); no discuss-phase needed — design was settled interactively before phase was opened.
...
**Implicit inverse (user confirmed):** if the user is scrolled UP reading history, new incoming messages do NOT yank them down. The existing jump-to-bottom pill stays the manual affordance.

Nothing else. user verbatim 2026-08-12: *"one of the goals I have here is to keep it simple, because the old auto-scroll behavior got pretty complicated when I asked for a bunch of different cases to be handled."* And: *"I want this to be done right so if you have to rip out the old stuff and start fresh or adjust things like you know that's my priority."*
...
- Bounty `pv-auto-scroll-redesign` timeline entry `2026-08-12T18:05:00Z` — full design detail from the user ↔ Tina conversation (this CONTEXT.md is the phase-scoped extraction).
...
- Library swap to `react-virtuoso`'s `<VirtuosoMessageList>` — parked from bounty `pv-auto-scroll-redesign`. If this phase's simple design still shows jitter under user UAT, library swap is the fallback (bigger refactor, purpose-built for chat). But we're not doing it this phase.
...
- Additional scroll behaviors user might want later (scroll-into-view for specific messages, scroll-to-mention, etc.). Not in scope.
...
*Context captured: 2026-08-12 directly from user ↔ Tina design conversation. No discuss-phase run — design was settled interactively before phase-open.*

## Post-ship correction (2026-08-13)

### Symptom
user 2026-08-13: "if I try to scroll up, I get a little ways up before it either
...
### Structural fix
user greenlit the structural fix over a narrow threshold-bump. Split the
