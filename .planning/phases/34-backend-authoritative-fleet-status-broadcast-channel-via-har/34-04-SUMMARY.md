...
The `describe('fail-open on missing hook payload file', ...)` block in `ssh-poll-orchestrator.test.ts` pins the user 2026-08-13 LOCKED constraint across 6 scenarios:
...
**user 2026-08-13**: after weighing rigor vs. cost, chose option (A) — trust the 6 dedicated fail-open regression tests + verify fail-open at bundle-deploy time. Full backend dev-mode boot on this box would have collided with production port bindings and risked fleet access.
...
- FOUND: be04011 — test(34-04-03): pin fail-open regression tests for user's 2026-08-13 LOCKED constraint
