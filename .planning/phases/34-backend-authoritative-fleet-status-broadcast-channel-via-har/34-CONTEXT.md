...
**Source:** Live design conversation with user 2026-08-12/13 (captured verbatim in STATE.md § Roadmap Evolution 2026-08-13 entry + bounty `fleet-status-backend-signal/bounty.json` — treat both as source-of-truth alongside this file).
...
- **Delivery mechanism: 2s polling over the existing SSH pool** (poll over event-driven `inotifywait`; user 2026-08-13 chose the polling path for zero-dep operation on any host with a shell). Uses whatever SSH primitives the backend already exposes for its tmux/pane plumbing. One channel per identity-hosting host, multiplexed.
...
### Dot semantics (UNCHANGED — LOCKED by user 2026-07-23; do NOT re-litigate)
...
- Semantics: "harness needs a user decision PrettyView can't render interactively" — visual affordance only, no interactive controls (user must switch to a terminal pane to answer; the bubble tells her that).
...
### Structured logging (standing directive, user 2026-08-11)
...
- **NEVER use worktrees** (user 2026-07-31). `workflow.use_worktrees=false` set on this project.
...
- `.planning/STATE.md` § Roadmap Evolution 2026-08-13 entry — verbatim user quotes from the design conversation + full architecture recap
...
- `~/.claude/roles/box-maintainer/box-maintainer.md` § "Skynet conversation-list dot semantics — one meaning: 'ready for your attention'" (user 2026-07-23 lock)
