...
**Source:** In-session `/build feature-mode` → `/open` shape lock 2026-09-09 (greenlit `thumbs up` same session, 4 tasting rounds on roles-list rows, 2 console-snippet iterations on the identity-modal title-line treatment). Bounty `role-management-modal-split` (pinned by user 2026-09-09, item 8 in the ongoing UX-pass campaign — natural follow-up to Phase 89's identity-modal-tab-restructure and Phase 86's cosmetics-migrate-to-role). Shape file: `.planning/shapes/shape-role-management-modal-split.md`.
...
### D-01: Role-cosmetic-edit UI belongs in this phase (LOCKED — user 2026-09-09, resolves Phase 86 deferral)

user 2026-09-07 (Phase 86 CONTEXT.md § Deferred): *"I would leave this alone [post-creation role cosmetic edit] because I actually plan on breaking out the role level stuff into its own modal later, and that would be a good opportunity to add it."*

user 2026-09-09 (this session, verbatim): *"we are going to do the cosmetics editing on this like it's basically just going to be the same thing that the identity modal has, except for maybe the override stuff that shows up there. So that should be a pretty one to one recreation."*
...
### D-02: Roles-list modal uses the Edit-global-files host-picker pattern (LOCKED — user 2026-09-09)

user 2026-09-09 verbatim: *"for B, I was thinking we could actually set it up the same way as the edit global files modal, where there's a drop down to pick a host. And if you are a user that only has one host, then that just defaults to that one host that you have. So that keeps that simple."*
...
### D-03: Role modal is a global modal — portals to `document.body` (LOCKED — user 2026-09-09)

user 2026-09-09 verbatim: *"the roles modal and the edit role modal have nothing to do with sessions that you have open or not. like they are sort of global modals the same way that like editing skills and edit global files modals are."*
...
### D-04: Identity title-line clickable treatment (LOCKED — user 2026-09-09, from console-snippet tasting)
...
### D-06: Role modal chrome wears the role's own hue (LOCKED — user 2026-09-09)
...
### D-07: Three-dots menu after this phase (LOCKED — user 2026-09-09)
...
- Peer identities (tina, tiffany, tanya, taylor) may push concurrently; rebase handles it. No coord post required for push-only sessions per user 2026-09-05 rule.
...
- No coord-room posts for push-only sessions (user 2026-09-05).
...
- Enriching roles-list rows with state (bounty counts, identity counts, activity glances, last-touched timestamps). Directory-not-dashboard stance is deliberate; add later once the routing shape settles and user wants the extra signal.
- Search / filter inside the roles-list modal. Small fleet doesn't warrant it; revisit if fleet grows past ~20 roles.
- Cross-fleet role aggregation (merge roles from every managed host into one list). Kept per-selected-host per user 2026-09-09 (Edit-global-files pattern).
...
Campaign constraint continues (user 2026-09-07):
...
- No coord-room BEFORE/AFTER posts (push-only rule, user 2026-09-05).
- Every push runs `git pull --rebase origin feat/tab-title-from-tmux` first.
- Ride-along ships: a peer's future `--force-recreate` may pick up my commits from origin. user knows and has explicitly authorized this shape.

Post-phase items pending in the campaign string:
- Whatever campaign items remain after Phase 90 lands (user tracks the ordering; tabitha keeps the order per her delegation).

user 2026-09-07 delegation (verbatim): *"after each build for this plan, you're going to reset yourself and then invoke the next build on the next bounty at the start of the next session"* and *"you're in charge of making sure that we continue with the plan and these bounties go in the right order."*
