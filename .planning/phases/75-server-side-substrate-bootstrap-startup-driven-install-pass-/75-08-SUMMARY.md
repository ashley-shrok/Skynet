...
One-shot operator migration script that transitions existing substrate-host credentials from per-user-DEK-only to CSKEK-wrapped. Ships the deliverables for decisions D-12, D-13, D-14, D-15, and D-19 — unblocking user's post-ship operational step.
...
**Pre-check note:** No inline-credential substrate hosts found in developer's local fixture DB (empty store in tests — correct for a fresh dev environment). Production check happens when user runs the migration.
