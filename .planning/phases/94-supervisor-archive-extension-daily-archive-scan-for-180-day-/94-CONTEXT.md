...
- `.planning/shapes/shape-supervisor-archive.md` — user's locked shape from the /open pass 2026-09-09. All D-01..D-17 above are derived from it. Read this first.
...
- **180-day threshold** — user's number from the campaign shape, thumbs-up during Shape 2 /open pass. Not a knob exposed to configuration in this phase (planner does not add a `--threshold-days` CLI flag or environment variable).
- **Daily cadence** — user thumbs-up during /open, chosen over 6-hourly (campaign shape's placeholder), once-per-supervisor-start, and once-per-tick. Reasoning: threshold is measured in days; being off by hours or a day is invisible.
- **Move-first ordering** — user thumbs-up during /open. The campaign shape had kill → deactivate → move; this phase's shape flipped it after grill discussion of failure modes.
- **Fully silent retirement** — user 2026-09-09 verbatim: *"No announcement of any kind."*
- **Un-archive out of scope** — user 2026-09-09 verbatim: *"unarchiving will be a concept that we get into formally later."*
- **Coord exemption** — user 2026-09-09 verbatim: *"Coordinators are not expected to be no dormancy, that's just not a thing, and I think we should not archive coordinators."*
- **Cursor mtime as freshness signal** — user corrected a mid-grill misread on my part: mtime advances on every ~30s successful sync (verified against `recv.sh` line 235), not only on inbound messages. That correction is what enabled the "one signal, no supplementary heuristics needed" cleanness of the mechanism.
...
- **Announcement wire for routine retirements** — history log entry, coord DM, user ping. Excluded per D-15 (silent by design); routine retirements should feel unremarkable under the task-scoped paradigm.
