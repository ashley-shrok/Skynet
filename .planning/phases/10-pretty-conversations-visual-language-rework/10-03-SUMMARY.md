...
dependency_graph:
  ...
  affects:
    ...
    - user's small-window use case: sidebar toggle now always reachable at
...
persistent top-left 32x32 chevron toggle user asked for — the fix for the
...
- **Persistent top-left toggle renders UNCONDITIONALLY at all widths.**
  ...
  affordance user always knows where to find. Position: fixed anchors to
...
code-complete on `feat/tab-title-from-tmux` — user reviews at her next
