...
The `variant` prop is the ONLY new wiring. user's shape lock says variant tracks `isMobile` (the narrow-viewport predicate) rather than `isTouchDevice` (the hardware predicate) — the row's mobile swipe wants to fire on narrow desktop windows too where the touch device may still be a laptop (per Wave 1 handoff §"What Wave 2 DOES need to build" bullet 5).
