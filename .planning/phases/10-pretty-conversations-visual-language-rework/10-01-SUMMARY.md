...
in isolation. Every visual decision user signed off on 2026-07-22 —
...
| File | LOC | Purpose |
|---|---|---|
| `tokens.ts` | 50 | 5 named constants for values reused 3+ times (row heights, swipe geometry) per user naming rule |
...
- Panel-level effect: when the selected conversation changes, force all rows closed (user: "changing conversations should snap any swiped row shut")
