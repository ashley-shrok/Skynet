...
- `~/fleet/roles/box-maintainer/box-maintainer.md` § "Standing directives" — especially the container-mutation serialization rule (the user 2026-09-12), the executor-scope test discipline (the user 2026-09-07), the deploy-boundary-at-push rule (the user 2026-08-29), and the no-worktrees rule (the user 2026-07-31). Planner and executor must honour these.
...
- Fleet no-streaming rule (the user 2026-08-29) — Skynet has NO message streaming ever, anywhere. No skeleton tiles, no "connecting…" spinner, no shimmer. Tiles render atomically on frame arrival.
- Deploy hold for the whole first-class-apps campaign (the user 2026-09-18) — Phase 119's commits stack on Phase 118's held commits (which stack on Phase 115's shape-1 held commits) on `~/skynet-vision`'s `feat/tab-title-from-tmux` branch. `git push` + `docker build` + `docker compose up --force-recreate` all held until the campaign-close greenlight.
...
- **Container-mutation serialization** (the user 2026-09-12) — applies to the eventual deploy motion, NOT to Phase 119's planning or executor phases.
...
- **Section label copy: exactly "Apps"** (title-case source, rendered uppercase via CSS matching the Archived pattern). the user 2026-09-18: *"the header itself would just be icon plus title which would just say app"* — plural per convention.
- **Empty-state copy: exactly "Ask an agent to make an app for you."** the user 2026-09-18 confirmed "seems fine" with no better suggestion; I offered no alternative.
...
- **Tile visual is variant A from the round-1 tasting** with two refinements from the user: (1) icon slot is rounded-square not round, (2) title-only, no secondary line. Locked from /open. All app tiles use default `--pv-hue: 216`.
...
- **Per-app colour / theming / hue** — the user 2026-09-18 verbatim: *"i would say we don't try to color them for version one"*. Deferred; the tile treatment leaves room for it without a structural rewrite.
