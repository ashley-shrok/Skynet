...
1. **Should the OIDC user-create branch (`users.ts:981-1008`) ALSO carry a mandatory-avatar gate?**
   ...
   - Recommendation: Treat OIDC create as OUT of Phase 87's mandatoriness scope (matches literal reading of D-09). Add an explicit code comment at the OIDC create site noting "OIDC users created without avatar per Phase 87 scope; downstream mechanism populates via D-10". If user wants OIDC users to also have mandatory avatars, that becomes a Phase 87.1 scope-add — the D-10 change endpoint can be invoked from a first-login flow later.
