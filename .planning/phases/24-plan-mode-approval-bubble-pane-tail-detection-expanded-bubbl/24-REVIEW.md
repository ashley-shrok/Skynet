...
2. **Same-slug regenerate (the CONTEXT-called-out "user picks feedback →
...
Ink is currently prompting on. user approves based on stale content.
...
issue but silent — user won't know the truncation lost bytes vs
...
fails silently — bubble never mounts, user never sees the actionable UI.
...
for user's local ops posture but slightly leaky for the "small dim line"
