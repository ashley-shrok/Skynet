...
| `src/backend/claude-session/plan-pending-parser.test.ts` | MODIFY | vitest pure-helper suite | synthetic-in/assert-out | `src/backend/claude-session/context-pct-parser.test.ts` (user's fingerprint style) | exact |
...
**Analog:** itself. Copy the `recycleActive` prop shape verbatim (user 2026-08-04 CONTEXT lock: "match recycleActive treatment").
