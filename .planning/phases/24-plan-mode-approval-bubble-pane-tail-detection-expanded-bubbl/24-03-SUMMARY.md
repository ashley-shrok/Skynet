...
- Fail-closed stale-fetch guard: after the pending window closes mid-fetch (sentinel === "null"), any late-arriving .then() drops its result silently. Prevents retro-emit of stale content onto a window that user already resolved via the pane keyboard directly (T-24-03-03).
