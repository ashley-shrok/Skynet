...
key-decisions:
  - "Fingerprint SHAPE (bottom-slice marker AND header-anywhere) preserved verbatim; only the two anchor strings and the count of header variants changed — single-variant per user's pinned-fleet lock 2026-08-04"
...
## Accomplishments
- `isPlanPending` now actually detects the live fleet Ink plan-approval prompt (was silently returning `false` on every pane since quick 260802-rps landed 2 days ago — user's own pane wouldn't have triggered PlanPendingBubble regardless of how many plans she was reviewing).
