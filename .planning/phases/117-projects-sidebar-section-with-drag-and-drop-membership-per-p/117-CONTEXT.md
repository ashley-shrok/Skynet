...
### Membership
- **D-05:** Membership lives WITH the thing that's being grouped — no parallel index, no lookup table anywhere else. The exact carrier depends on conversation kind:
  - **Identity-associated conversation** → `project: <slug>` in the identity file's YAML frontmatter (`~/fleet/identities/<name>/<name>.md`). Same file that already carries `role:`, `task:`, `displayName:`. Moving between projects is a one-file frontmatter edit.
  - **Relay-room conversation** → an `m.tag` account_data event on the Matrix room with tag ID `u.project.<slug>` (Matrix's canonical user-defined-tag namespace). Uses the built-in Matrix tag mechanism — per-user, per-room — reached via account_data GET/PUT on the room.
  - **Terminal session** → excluded from projects entirely in v1 (see D-08).
- **D-05a:** `m.tag` write discipline (read-modify-write): when assigning a relay room to project X, the writer must (1) GET the room's existing `m.tag` account_data blob, (2) preserve all non-project tags (favorites, low-priority, any other `u.*` tags), (3) strip any existing `u.project.*` tags to enforce single-project-per-conversation (D-06), (4) add `u.project.<slug>` with value `{}`, (5) PUT the updated blob. Never blindly overwrite. Clearing a room's project membership is the same operation with step 4 omitted.
- **D-06:** One project per conversation (single-valued). Multi-project membership is explicitly out of scope for v1. For identity conversations, the `project:` field is a single scalar; for relay rooms, D-05a's read-modify-write enforces at most one `u.project.*` tag at a time.
- **D-07:** Missing / dangling / archived project references degrade gracefully — the conversation just falls into the flat middle. No crash, no error banner. Applies both to identity `project:` fields pointing at a nonexistent slug and to relay-room `u.project.*` tags pointing at same.
- **D-08:** Terminal sessions are excluded from projects entirely in v1. DnD drop targets refuse them; no metadata carrier for terminals in v1. If terminals ever need projects (unlikely — per grill), separate phase.
...
### id-skill amendment
- **D-32:** On `/id <name>` load, after reading the role file and any handoff, the id-skill checks the identity file's frontmatter for a `project:` field (same file it just read the identity's `role:` from — no additional file read). If present, it reads `~/fleet/projects/<slug>/project.md` into context AND enumerates the project directory's top-level contents silently (mirrors current runbooks enumeration — knows the names, contents load on demand via a normal Read tool call). Note: relay-room conversations (which don't have identity files — their membership lives in `m.tag` per D-05) do not trigger this id-skill path — the amendment is scoped to identity-loaded sessions only.
...
### Backend surface
- **D-36:** Identity artifact reader (`src/backend/claude-session/identity-artifact-reader.ts`) grows methods for: reading an identity file's `project:` frontmatter field, writing/clearing that field (a frontmatter round-trip write that preserves all other fields — see also D-38 pitfall), listing all projects (enumerate `~/fleet/projects/` directory, excluding `archive/`), reading a project's `project.md` file, creating a project (mint directory + bare file with duplicate-slug rejection), archiving a project (move `~/fleet/projects/<slug>/` → `~/fleet/projects/archive/<slug>/` + cascade archive to member conversations via the existing identity-archive mechanism from Phase 115). Separately, a small Matrix account_data layer (extends `src/backend/matrix/` or a new sibling module) exposes: read a room's `m.tag` blob, write it back with a `u.project.<slug>` tag added (or cleared), preserving all non-project tags per D-05a.
- **D-36a:** Request-body posture for v1 project endpoints — JSON body throughout. Phase 115's `identity-archive` endpoint precedent (JSON body, per `identity-archive.ts:76-79`) governs. The multipart-with-`data`-field rule from `box-map.md § Operating` applies only to write endpoints that carry file uploads (avatar upload, roles-create with attached body); v1's project endpoints (create-project with just a name, archive-project as a sentinel-drop, session-project-field write, relay-room-project-tag write) carry no files. Multipart is reserved for v2 endpoints that grow file-upload payloads (e.g., a future "upload a design doc into the project directory" affordance).
...
### Matrix account_data (relay-room membership carrier — per D-05)
- `src/backend/matrix/matrix-admin-client.ts` — existing Matrix HTTP client. The account_data read/write extends this (or a sibling module).
- Matrix Client-Server spec § 13.3 "Client Config" — account_data GET/PUT endpoints (`/user/{userId}/rooms/{roomId}/account_data/{type}`). Type will be `m.tag`.
- Matrix Client-Server spec § 13.4 "Client Behaviour — Room Tagging" — the `m.tag` event's shape (`{tags: {"u.project.<slug>": {}}}`) and the `u.` user-defined-tag namespace convention.
