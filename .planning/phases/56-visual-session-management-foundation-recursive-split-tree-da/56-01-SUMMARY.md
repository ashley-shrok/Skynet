...
`SplitNode` carries only `kind`, `direction`, `children` (for splits) or `kind`, `tabId` (for leaves). Constant-ratio 50/50 is a rendering choice made downstream in Plan 56-02, not a data-model concern. This satisfies user's 2026-08-28 locked decision "no draggable pane dividers".
