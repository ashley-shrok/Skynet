...
Delete dead UI subtrees user never uses + retire every dangling import, switch-case, and integration exposed by these deletions.
...
2. `src/ui/features/docker/` — 8 files, ~2,500 LOC. user uses `docker` command directly.
3. `src/ui/features/tunnel/` — 6 files, ~900 LOC. user's fleet is on Tailscale.
4. `src/ui/features/server-stats/widgets/` — 11 files, ~880 LOC. user checks health via SSH.
