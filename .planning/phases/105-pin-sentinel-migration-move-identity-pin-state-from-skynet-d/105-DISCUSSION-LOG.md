...
**Notes:** user thumbs-up on the argument that Phase 69 killed the DB identities table and Phase 80/86 established the disk-read-at-request-time pattern; mirror cache would reintroduce the two-sources-of-truth risk the whole move is designed to eliminate.
...
**Notes:** user: *"Okay, then we can use it then."* Rejected alternatives: new parallel wire (duplicates auth + lifecycle + error handling), terminal channel (stateful, wrong use case), distributor sweep (scheduled sweeps ≠ transactional one-shots).
...
**Notes:** user verbatim: *"Yeah, synchronous. Keep it simple."* Rejected alternative would reintroduce the drift risk the shape's "what would make it wrong" explicitly closes.
...
**Notes:** user verbatim: *"This isn't changing, so however it feels now is how it's going to feel after this."* This is a plumbing move, not a UX pass.
...
**Notes:** user verbatim: *"We are not complicating this, so however it would have failed already is how it will fail today, even if it's more likely now than before."*
...
**User's choice:** Drop.
**Notes:** user verbatim: *"Drop it."* Dormant table invites future accidental re-consultation; matches the "no shadow source of truth" invariant.
...
**Notes:** user thumbs-up on the argument that any encoded metadata reopens the drift question (two things to keep in sync per identity — file existence and its contents); any metadata that becomes desired later belongs elsewhere if it belongs anywhere.
