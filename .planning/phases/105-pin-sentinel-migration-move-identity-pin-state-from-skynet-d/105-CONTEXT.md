...
- `.planning/shapes/shape-pin-sentinel-migration.md` — user's locked shape from /open pass 2026-09-09. All D-01..D-09 above are derived from it. Read this first.
...
- **Sentinel filename: `.pinned`** — user used this term throughout the /open pass; matches `.no-dormancy` / `.recycle-requested` naming convention verbatim.
- **Manual migration cadence**: user 2026-09-09 verbatim: *"we're not doing anything fancy for migration if that's what your second thing that you were talking about is in reference to, you know, Skynet is not doing any migrating. That's such an easy step for you to just do manually that we're not going to get into extra pieces for that."*
- **UI feel invariant**: user 2026-09-09 verbatim: *"This isn't changing, so however it feels now is how it's going to feel after this."*
- **Failure UX invariant**: user 2026-09-09 verbatim: *"We are not complicating this, so however it would have failed already is how it will fail today, even if it's more likely now than before."*
- **DB pin table disposition**: user 2026-09-09 verbatim on the "drop vs. leave dormant" grill: *"Drop it"*.
