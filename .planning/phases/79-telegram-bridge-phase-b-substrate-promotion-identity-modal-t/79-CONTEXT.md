...
An earlier revision of this file (and the earlier DISCUSSION-LOG) attributed decisions to user she didn't actually make. Course-corrected 2026-09-06:

- **"user 2026-08-30 normalized substrate services as user-scope"** — INVENTED by me (Tina). user never said this; I pattern-matched from `agent-supervisor` being USER-scope and packaged the inference as her decision. user verbatim on 2026-09-06: *"decisions get attributed to me that I didn't actually make. Because I don't even know why I would care about user scoping."* The systemd USER-scope decision is REMOVED — the bridge is a Docker Compose service (see § 4C-revised below), which drops the systemd question entirely.
- **"user decision #1 = wire an SSH credential to host id 6"** — was user's greenlight of my recommendation, but was BASED ON my erroneous assumption that the Skynet host is guaranteed to be self-registered in Skynet's own host DB. user 2026-09-06 corrected: *"the host machine that the app is running on will actually be a host that is registered into Skynet"* → NOT guaranteed. Decision #1 is REMOVED. The Skynet-container-to-host install pathway becomes irrelevant because the bridge no longer runs on the host (see § 4C-revised below).

**Standing directive for the rest of this file and for downstream agents (researcher / planner / executor):** if a decision is attributed to user, it must be traceable to her literal words in the conversation transcript. Inferences and pattern-matches are labeled *"my inference"* or *"mirrors existing X pattern"* — never *"user decided."* This is the fleet directive from Tina's role file: *"Capture the user's words verbatim — don't paraphrase into attribution."*
...
- Bridge restart is <5s. Matrix-cursor persistence (this phase's core reliability deliverable) means no messages drop during restart. user 2026-09-06: *"as long as we're not ever dropping messages, I don't care that the restart happens."*
...
1. **Where does Skynet's STT config live?** user confirmed it exists (she uses voice input). Find the config key, the source file, and the read mechanism. The bridge reads STT URL from the SAME source. This will inform the shared TS constant module design (§ user-locked #3).
...
- `.planning/shapes/shape-telegram-bridge.md` — shape file, locked; user's discuss-phase overrides in § Shape overrides above.
...
Fleet rule (user 2026-07-31). All Phase B work happens in the main tree on `feat/tab-title-from-tmux`. gsd-executor's worktree mode is not selected for any wave.
