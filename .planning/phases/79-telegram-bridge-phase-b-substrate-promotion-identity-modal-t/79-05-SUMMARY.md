...
**Confirmed.** `grep -cE '100\.113\.23\.63|100\.80\.122\.111' substrate/services/tg-bridge/bridge.sh` returns `0`. Bridge sources both MATRIX_ROOT and STT_URL from `/state/config.env` via `. "$CONFIG_FILE"` — no hardcoded Tailscale endpoints anywhere in the script. This is user's non-negotiable Phase-B reliability floor.
