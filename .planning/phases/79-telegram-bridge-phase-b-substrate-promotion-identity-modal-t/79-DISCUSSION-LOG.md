...
**Date:** 2026-09-06
**Participants:** user (visionary), Tina (builder)
...
- **Shared-config invariant:** bridge reads Matrix homeserver URL + STT URL (+ any other shared endpoint) from Skynet's own config. Hardcoded IPs in `bridge.sh` (`http://100.113.23.63:8008`, `http://100.80.122.111:8000`) get removed and replaced with reads from Skynet config. user: *"if we come out of this and the bridge is not using the config values for where to get speech to text from that Skynet also uses, then that will have been a mistake."*
...
**user's answer (verbatim):** *"I agree with area two."*
...
- **3B — Actionable errors inline, one modification from builder's original:**
  ...
  - Bridge restart fails → tab shows the error + Retry button. **NO automatic DM notification to any maintainer** (user's carve-out). Tab surface is the only escalation surface.
  - Network / registry-write failure → "Couldn't save — try again" + Retry.
- **3C — Do nothing special for restart-interrupts-others.** Cursor persistence makes drops impossible; no confirm dialog, no rate-limiting. user: *"as long as we're not ever dropping messages, I don't care that the restart happens."*
...
**user's answer (verbatim):** *"Okay, I agree with your recommendations."*
...
Research completed cleanly (11/11 questions answered, high confidence across the board). Pattern-mapper completed (22 files classified). Planner ran and produced 5 PLAN.md files. **Before plan-checker fired, Tabitha DM'd me with an observation from user that surfaced two material errors in my planning framing.** Course-corrected — the 5 PLAN.md files were built on wrong assumptions and are deleted; planner will re-run against updated CONTEXT.md.

### What was wrong

1. **False attribution — "user 2026-08-30 normalized substrate services as user-scope."** I recorded this as fact when I recommended USER-scope systemd for the bridge. user never said this. I pattern-matched from agent-supervisor being USER-scope and packaged the inference as her decision. user 2026-09-06 verbatim: *"decisions get attributed to me that I didn't actually make. Because I don't even know why I would care about user scoping."* Standing rule going forward: no attributions without her literal words.

2. **False architectural assumption — "wire an SSH credential to host id 6" as install pathway.** I assumed the Skynet host is guaranteed to be self-registered in Skynet's own host DB, so distributor SSHs to it like any other host. user 2026-09-06 corrected: *"the host machine that the app is running on will actually be a host that is registered into Skynet"* → NOT guaranteed. That framing is coincidental to t1000 today, not the general case.
...
- **Zero operator burden per deployment** — `docker compose up` brings the bridge alongside the other services. user: *"do we need to put any burden on the operator? [...] can we just standardize that and not require that to be filled in?"* → yes.
...
### Standing directive burned into role file (proposed to user)

Attribution discipline: never record "user decided X" without her literal words in the transcript. If inference, label it *"my inference"* or *"mirrors existing X pattern"*. This is the fleet directive *"Capture the user's words verbatim — don't paraphrase into attribution"* — I violated it and need to bake it into pre-write reflex.
