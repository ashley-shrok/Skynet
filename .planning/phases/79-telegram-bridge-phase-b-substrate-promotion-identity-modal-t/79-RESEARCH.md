...
- **3C Restart interrupts others:** do nothing special. Restart is <5s. Matrix-cursor persistence means no messages drop. user: *"as long as we're not ever dropping messages, I don't care that the restart happens."*
...
3. **STT_URL hardcode — Option 1 (config table) or Option 2 (shared TS constant)?**
   ...
   - Recommendation (from Pitfall 5): **Option 2 (shared TS const)** for Phase B speed; Option 1 (proper config table) as a follow-up if runtime configurability is ever needed. user to confirm.
