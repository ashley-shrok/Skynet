...
key-decisions:
  ...
  - "Delete-file next-tab heuristic simplified from plan's 'next-right / previous / none' to 'first-remaining-or-null' — plan's heuristic requires caching the pre-delete file list, but user's fast-path bias says any-remaining-tab is acceptable, and this matches the initial skill-load auto-select-first behavior"
