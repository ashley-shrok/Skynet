...
- **Issue:** Test QB-1 at `src/ui/features/pretty-view/ComposeBox.test.tsx:860` asserted `paperclip.className` matched `/rgba\(240,235,224,0\.3\)/`. Fix commit `1503c40c` (`fix(pretty-view): kill overlap-doubling on composebox icons`, 2026-08-18 19:55 UTC — ~6 hours before Phase 46 execution) intentionally switched from the semi-transparent rgba stroke to the Tailwind `opacity-30` utility to avoid additive alpha compositing on overlapping lucide SVG paths. Test assertion was not updated in that patch. Test was drifting green→red between STATE.md 2026-08-18 evening (Alice's 2434 pass green baseline) and Phase 46 execution.
