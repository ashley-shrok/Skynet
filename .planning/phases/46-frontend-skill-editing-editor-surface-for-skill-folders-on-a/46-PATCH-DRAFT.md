...
5. Live-fire HTTPS check:
   ...
   HOST_ID=<pick-a-real-hostId-from-user's-fleet>
...
7. Post to coord-room announcing patch #TBD landed; drop the UAT checklist link so user can walk it when she's ready.
...
Reference the box-maintainer's bounty tree at `~/.claude/roles/box-maintainer/bounties/frontend-skill-editing/` (maintainer's local path — orchestrator confirms and closes the bounty at PIN time). Bounty ships when user signs off on the UAT checklist (`.planning/phases/44-.../46-UAT-CHECKLIST.md`).
