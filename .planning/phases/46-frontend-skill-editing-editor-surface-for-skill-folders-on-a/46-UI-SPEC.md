...
**Fast-path priority (shape file):** three selections max — menu → host → skill — then a tab click. This is user's trigger; the whole UX is subordinate to it.
...
**Menu-order rationale:** `Edit global files…` above, `Edit skills…` below — both live in the same section as file-editor tools. The two entries are visually indistinguishable (same font/size/color/hover); differentiation is by label only. Per user's fleet UX preference, no icon, no keyboard shortcut hint, no "beta" badge.
...
Tab-bar treatment for non-text file tabs: **identical to text files** — same FileText icon, same label, same selected-pill treatment. Do NOT visually mark them as non-editable in the tab bar (per D-08 "appear as tabs (not hidden)"; user wants zero pre-flight ceremony; the "can't edit" reveal happens on click).
