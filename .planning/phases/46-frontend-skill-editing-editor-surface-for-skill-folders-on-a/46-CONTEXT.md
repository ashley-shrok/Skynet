...
- **Non-text file placeholder** — where the editor pane would be, render a short message that the file isn't text and cannot be edited. user did not specify exact copy; UI-phase decides.
- **Delete-file confirm** — the ONE confirmation user explicitly asked for. All other actions (open, edit, save, add file, delete skill) do NOT need extra confirmation beyond whatever global-files already does.
