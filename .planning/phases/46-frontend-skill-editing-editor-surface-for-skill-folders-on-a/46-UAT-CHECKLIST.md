...
**Fast-path user cares about first:** Steps 1 → 6 (menu → host → skill → tab → edit → save). If those pass, the feature is minimally usable even if a later step fails. Steps 7-14 cover branches user won't hit on every session but the maintainer needs verified before considering the phase closed.
...
**D-10:** the ONE confirmation user explicitly asked for.
...
**D-16 / T-46-02 defense verification.** This is a maintainer-assisted check — user doesn't have to type curl.
...
- [ ] Steps 1-14 all `Pass`. Feature usable end-to-end for user's fast-path.
