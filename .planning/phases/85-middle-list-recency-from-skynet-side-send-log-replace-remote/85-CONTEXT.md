...
identity's newest JSONL for lines matching a strict "user's real user turn"
...
below identities user hasn't touched in weeks. This phase records the
...
- **D-01:** New table in the `skynet-data` SQLite database (Drizzle schema
  ...
  and the record holds "when was user's last send to this identity, in
...
- **D-03:** Universal rule — anything that fires a message-shaped payload
  ...
  user 2026-09-07 verbatim: *"if anything within the compose box sends a
...
- **D-05:** Attempts count. If the send fails because the target is
  unreachable, the stamp still fires. user 2026-09-07 verbatim: *"Attempts
...
- **D-06:** The row moves the instant user hits send — optimistic
...
- **D-09:** No backfill. On ship day the store is empty; every identity
  starts at "never" and rises naturally as user sends to them. user
...
- **D-10:** Both of user's devices (phone + desktop) read from the same
...
- `src/backend/fleet-status/ssh-poll-orchestrator.ts:418-476` —
  `isRealUserTurn`, the "user's real user turn" predicate (locked
...
- user's mental model of the list is messaging-app: "who did I last
  talk to." One-directional signal only (user → identity). The bug
...
- **Capture user's phone Matrix DMs that bypass Skynet's compose
