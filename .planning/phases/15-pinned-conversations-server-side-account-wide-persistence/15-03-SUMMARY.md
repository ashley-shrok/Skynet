...
dependency_graph:
  ...
  affects:
    - "End-to-end pin persistence: user's pin now survives tab/PWA close and syncs across devices on next mount (pending deploy)"
tech_stack:
  added: []
  patterns:
...
Landed the read-side plumbing for Phase 15: on `PrettyConversationsPanel` mount, an async `useEffect` fires `getPinnedIds()` (Wave 2 api-client) and, on success, calls `hydratePinnedIdsFromServer(ids)` (Wave 2 store setter). This completes the end-to-end round-trip — Wave 1 backend + Wave 2 store + Wave 3 panel = user's pins now survive tab/PWA close and sync across devices on next mount (pending deploy). One new integration test (Test 21) asserts the mount → fetch → hydrate wiring.

**Human-verify checkpoint (Task 2) — pending.** Per plan §autonomous:false, the executor lands the code + integration test and stops. user (or the dev operator) runs the 4-step UAT below to unblock the phase completion.
...
- **Tasks:** 1 code task complete (Task 1); Task 2 is a human-verify checkpoint awaiting user
- **Files created:** 0
- **Files modified:** 2
...
This phase's patches batch with the pending **f9v + BTW + gm3 deploy queue** on `feat/tab-title-from-tmux` (currently three-deep per 15-CONTEXT.md § Bundle-mate discipline) OR ship as a solo deploy IF the pin-bug severity triggers #3 ("actively broken in production user is hitting") — decision made at deploy-recommendation time by user.
...
## UAT steps for user (Task 2 human-verify — VERBATIM from plan)

Verify in a running dev Skynet instance (localhost or user's dev environment — user picks). The deploy to term.example.com is intentionally NOT part of this phase — deploy is a separate step batched into the pending f9v + BTW + gm3 deploy queue per CONTEXT.md § Bundle-mate discipline.
...
**PENDING.** Task 2 (checkpoint:human-verify, gate="blocking") awaits user (or dev operator) to run the 4-step UAT above and reply with "approved" or a wave-attributed failure description.

The executor has landed the code + integration test per plan §autonomous:false and returns control to the orchestrator for user's verification step.

---
*Phase: 15-pinned-conversations-server-side-account-wide-persistence*
*Wave 3 code landed: 2026-07-27*
*Human-verify checkpoint: pending user approval*
