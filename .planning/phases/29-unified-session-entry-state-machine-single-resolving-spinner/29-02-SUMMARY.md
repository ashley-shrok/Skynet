...
4. `PrettyViewErrorOverlay — iOS Safari backdrop-filter hardening (patch #333)`
   - Test 5: scrim className carries all three load-bearing tokens (`isolate`, `[transform:translateZ(0)]`, `[-webkit-backdrop-filter:blur(12px)]`). Non-negotiable per patch #333 — user uses this fork primarily on her iPhone PWA.
...
**"Connection failed — retry"** — D-08 default. Em-dash is U+2014. Final copy remains open to UAT tweak with user, per D-08 note.
