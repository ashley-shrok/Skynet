...
**User's choice:** Reuse as-is (Claude recommendation, user confirmed via "let's go").
...
**User's choice:** Option (ii) (Claude recommendation, user confirmed via "let's go").
**Notes:** Inline banner (option i) is easy to miss on mobile PWA. Full-surface card is consistent with the existing warm-red treatment on `SessionHoldingOverlay` error variant, giving one warm-red palette across the app. Retry button UX shape parallels `DormancyOverlay`'s Wake button. Copy "Connection failed — retry" is a planner default, UAT-locked with user.
...
**User's choice:** Option (b) (Claude recommendation, user confirmed via "let's go").
