...
**User's choice:** Reuse as-is (Claude recommendation, Alice confirmed via "let's go").
...
**User's choice:** Option (ii) (Claude recommendation, Alice confirmed via "let's go").
**Notes:** Inline banner (option i) is easy to miss on mobile PWA. Full-surface card is consistent with the existing warm-red treatment on `SessionHoldingOverlay` error variant, giving one warm-red palette across the app. Retry button UX shape parallels `DormancyOverlay`'s Wake button. Copy "Connection failed — retry" is a planner default, UAT-locked with Alice.
...
**User's choice:** Option (b) (Claude recommendation, Alice confirmed via "let's go").
...
- Two open user-questions from session 9 on `pretty-view-conversation-pick-loading-feedback` bounty (revert-#351 timing; app-root-overlay-vs-anchored) — separately owned
