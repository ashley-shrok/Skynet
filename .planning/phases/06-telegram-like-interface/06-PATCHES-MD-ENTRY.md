...
**Instructions for user at pin time:**
...
```
   105. `feat(navigation): telegram-like interface — sidebar conversation
        ...
        * **Motivation** (from shape file). Skynet has been drifting
          ...
          sidebar, mobile-bottom-nav gate). user uses it like a chat
        ...
        * **New conversation-store (in-memory).** Module-scoped store
          ...
          works is fine" — user 2026-07-21). Pins are per-session;
        ...
        * **What we DIDN'T do** (deferred-to-v2 per shape lock):
          - Any activity/unread signal (dots, badges, counts, motion,
            sound) — user 2026-07-21 "we can save it for version two"
          - Cross-conversation search — user "I don't need it" — OUT
            entirely, not v2
          ...
          - A per-session view toggle to flip pretty view to raw
            terminal on mobile — user "I don't need something on
          ...
          - Keyboard tab-nav refinement (Ctrl+Shift+[/] still cycles
            ...
            bounty per plan-check NOTE-06; user rarely uses this per
        ...
        * **Deploy note**: shipped behind the mandatory 15-min deadman
          per user 2026-07-03 with explicit per-deploy green light
          per user 2026-07-12 (see `deploy-runbook.md` under Tina's
          ...
          **substantial user-visible change** — the tab strip and
          ...
          disarm on user's UAT green, let fire on failure). Zero
