...
- **`useMobileScreen()` hook** — Plan 06-04 can read the current screen if it wants to gate NewSessionButton's post-create behavior on "am I on the list or view?" (probably not needed — new-session is always initiated from the list, and user expects to land in the new session's view). Signature: `() => "list" | "view"`.
