...
- **D-12: One-time backfill on rollout.** Existing agent and human accounts predate the registry rooms. On rollout / first-boot of the code that expects the rooms to be populated, run a one-time idempotent script that:
  1. Enumerates all agents Skynet knows about (via the fleet-status roster — already maintained, live in-memory in the fleet-status subsystem).
  2. Enumerates all humans (from the `users` table, filter for rows with populated `mxid` per Phase 88 D-06 guarantee).
  3. Joins each account to the appropriate registry room via admin `joinRoom`.
  Idempotent — safe to re-run. Skip accounts already in the room.
