...
- Bounty: `~/.claude/roles/box-maintainer/bounties/relay-mediated-group-conversations-humans-agents-in-rooms/` (pinned by user).
...
- `.planning/PROJECT.md` — Skynet's core value ("user never loses access to her fleet"); every change preserves reliable browser SSH+RDP.
...
- **Registry rooms + ignore-list are the load-bearing primitives** — see D-09..D-13. Two big departures from earlier design instincts landed here:
  1. NOT naming-convention-based (user: account format is changing).
