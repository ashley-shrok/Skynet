...
affects:
  - "ComposeBox voice-first slash-invocation UX — user says 'slash gsd status' → tmux receives '/gsd status' (was: 'bounty bounty add X' → tmux receives '/bounty add X'; new registry is unbounded, server-fetched per-invocation)"
tech-stack:
  added: []
  patterns:
...
user says **"slash gsd status"** into the mic in a pretty-view pane. The full path now runs:
