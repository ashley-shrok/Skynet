...
**Source:** In-session design conversation with user + Explore-agent codebase scan
...
- **`dormant`** — real supervisor-dormancy signal (Option C, user's pick over
...
**Retire the shadcn PopoverContent tokens** (`bg-popover text-popover-foreground border rounded-md shadow`) that are currently in play — those are the Skynet-general theme tokens, not the pv-* palette. The mismatch is what user named ("doesn't fit").
...
- **Inline SVG, NOT a unicode glyph** — a served `✓` character was mis-decoded
  as `âœ"` on user's iPhone PWA when the earlier snippet omitted charset;
  inline SVG dodges the class entirely.

### Labels + wording — short forms

- "Ready" (new — was going to be "Awaiting" / "Focus"; user picked Ready
...
- **Filter state persistence.** Currently none — filters reset on remount.
  Not part of this phase (parity with existing behavior). If user wants
...
- **Filter behavior on the RDP group.** Ready toggle explicitly SKIPS RDP-group
  ...
  user later wants Ready to hide the RDP zone, that's a separate change.
...
- **Ready-dot semantics locked** (user 2026-07-23, per box-maintainer role
...
*Context gathered: 2026-08-20 by tina, from in-session user design conversation + Explore-agent codebase scan*
