...
None — no new network endpoints, auth paths, file access patterns, or schema changes. All changes are CSS + React state + JSX markup. The inline SVG approach for the check glyph directly mitigates T-52-02-01 (unicode encoding trap on user's iPhone PWA when charset header absent).
