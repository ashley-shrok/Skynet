...
decisions:
  - "SSH-topology LOCKED as Option 3 (per-WS): each WS runs its own sweep exec per tick. Matches existing per-WS connectOneShot topology, delivers measurable 4x exec reduction per WS per tick, zero new coordination surface. user owns the override if she wants Option 1 (per-host connection pool) in a future phase."
