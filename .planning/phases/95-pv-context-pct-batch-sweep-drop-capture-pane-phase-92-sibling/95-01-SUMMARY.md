...
**Rollout mechanism:** Skynet container restart post-merge re-fires `installStopHook` on every fleet-substrate peer as new SSH clients acquire. Steady-state channel churn (dead-channel reap + reconnect) also re-fires it. user's deploy discipline (container restart post-merge) is the reliable Wave 1 rollout trigger — no new plumbing required. The helper's idempotency guarantees no duplicate entries on re-fire.
...
## Wave 1 UAT Gate (Task 2) — awaiting user
...
**Wave 2 authorization signal:** NOT YET — awaiting user UAT reply.
