...
**Source:** Bounty premise (`~/.claude/roles/box-maintainer/bounties/pv-context-pct-batch-sweep-drop-capture-pane/bounty.json`). Design agreed with user 2026-09-09 in-conversation, immediately after Phase 92 UAT revealed the SFTP file-fetch bug wasn't fully resolved by the fleet-status batch collapse alone.

<domain>
## Phase Boundary

Sibling to Phase 92 for the SECOND wasteful-exec pattern hitting the shared Skynet-host SSH connection. Three-part scope (re-scoped 2026-09-09 late — user chose to kill plan mode at the source rather than migrate the plan-pending detector to an FS marker):
...
Delete alongside: the `{ type: "plan_pending", pending }` WS frame emit path, the plan-content cache, the `fetchPlanFile` SFTP side-channel (`src/backend/ssh/plan-file-fetch.ts`), and the frontend `plan_pending` frame handlers. user 2026-09-09 verbatim on the direction: *"tmux capture-pane shouldn't be in there. get rid of it."* — with the follow-up *"remove the plan capture and adjust whatever gets distributed by the substrate distributor to disallow use of plan mode in the first place."*
...
**Related sibling phase (deferred):** Phase 96 (or next available slot) — unify all remaining un-semaphored SSH connections under a fleet-wide semaphore. Phase 95 addresses the specific measured pain (PV context-pct exec pressure); Phase 96 is the concept-level architectural safety net user asked for during Phase 95 discussion. Kept separate: batch REDUCES work (Phase 95), semaphore SERIALIZES work (Phase 96) — different problems, different solutions.
...
- **Deploy is orchestrator-owned** — phase's "done" state is: PR-ready, tests green, UAT checkpoint prepared for user. She owns the ship motion.
...
*Context gathered: 2026-09-09 — seeded from bounty premise (design agreed in-conversation with user same day, immediately after Phase 92 UAT revealed the second consumer)*
