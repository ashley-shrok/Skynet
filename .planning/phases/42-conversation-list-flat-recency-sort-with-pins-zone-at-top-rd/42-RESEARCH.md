...
**What goes wrong:** Wiring the middle sort to `SessionState.updatedAt` from the fleet-status channel produces a sort that bumps on every status flip (busy→idle, background-task start/stop). user's "activity = message either direction, and only that" would break — every background monitor task and every context switch would float the row.
...
| Search-input flooding causing render thrash | Denial of Service (local) | Debounce filter input at 100-150ms via `useDeferredValue` or `setTimeout` if per-keystroke re-renders become perceptible on large lists. Not urgent for user's ~20-session fleet. |
...
user checkpoint recommended before implementation because the backend scope significantly changes the phase's ship shape (frontend-only vs. full-stack).
...
**What's unclear:** Does user still want an "active" cluster at the top even when every row is visually uniform, OR do activeSet rows just render inline in the middle at their recency-determined position?
...
**Recommendation:** Include hidden rows in filter matches — user searching for a specific session by label should find it whether or not they hid it, because the intent to find > the intent to hide. user may disagree; low-stakes.

### 4. Search input focus behavior on mobile

**What we know:** user works from her phone almost exclusively.
...
- Recency signal: MEDIUM — architecture is fully understood, but the RIGHT choice among three options depends on an user call; the mechanics of each option are HIGH confidence.
