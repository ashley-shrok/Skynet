...
5. Post-UAT deploy runbook (only if everything's green AND user greenlights the batch) — cites `~/.claude/identities/tina/deploy-runbook.md` as authoritative per Phase 11 checker B-2 fix precedent
...
### 8. No gear icon anywhere (user's "no settings" lock — carried from Phase 11, tightened by Phase 12 UserProfilePanel deletion)
...
### 15. No SettingsRow at the bottom of the pretty-conversations list (user's "no settings" lock — Phase 11 mobile enforcement carried forward)
...
- [ ] 🚨 **Remove Skynet from the iPhone home screen** (long-press → Remove App → Delete from Home Screen). Then in Mobile Safari, navigate to `https://term.example.com`, tap Share → Add to Home Screen. Reopen from the fresh install. Expected: the top safe-area seam (above the status bar) and the bottom safe-area seam (above the home indicator) render as `#0a0b12` gray — no white flash, no color mismatch.
...
### FLEET-STANDING BATCHING RULE (user 2026-07-23) — patches #138 + #139 SHOULD NOT auto-deploy
...
- **Something is actively broken in production requiring an emergency patch ship** (Phases 11 + 12 don't fix anything broken in prod — they remove UI surfaces user never uses — so this scenario does not apply).

**The default answer is HOLD.** Do not treat "code-complete-clean" as "deploy-ready." Patches #138 + #139 sit in the batch queue; the deploy notification to user bundles all Phase 11 + Phase 12 changes into ONE UAT + ONE recreate + ONE (or TWO) pin(s). If Phase 13 lands within user's typical batching window (~days), extend further.
