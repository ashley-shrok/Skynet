...
**Paste timing:** Only after user greenlights the batched Phase 11 + Phase 12 purge cluster deploy AND UAT passes on the 23 non-negotiable items in `12-UAT-CHECKLIST.md`. Post-deadman-retirement flow per current `~/.claude/identities/tina/deploy-runbook.md` (the 15-min deadman regime was retired 2026-07-21).
**Batch context:** Patch #139 is the SECOND Phase-12-cluster patch (patch #138 = Phase 11 first slice; #139 = this Phase 12 second slice). Per the fleet-standing "batch patches into meaningful deploys" rule (user 2026-07-23), it does NOT ship standalone. Batches with patch #138 (Phase 11) and any subsequent Phase 13+ backend-route purge patches into a single grouped-semantic-unit deploy ("the Skynet-client-surface purge cluster") unless user explicitly greenlights an earlier standalone Phase 12 deploy.
...
## Draft (paste-ready — matches the multi-commit-under-one-pin convention from patches #104, #105, #128, and #138)
   ...
   139. `feat(sidebar,dashboard,shell,locales): patch #139 — Skynet transformation:
        ...
        backend-route purge patches per user 2026-07-23 fleet-standing
        "batch patches into meaningful deploys" rule; not deployed standalone
        unless user explicitly greenlights).
        ...
        * **Fix summary — Skynet tab bar chrome deletion** (PURGE-08). Deleted
        ...
        * **Preserved verbatim** (PURGE-05 carryover + Phase 12 scope-fence
          ...
          wc -l` = 0, build-verify G70). The encrypted-SQLite `skynet-data`
        ...
        * **Bundle-size headline — Ship-of-Theseus purge landed in bytes,
          ...
          **Cumulative Phase 11 + Phase 12 delta vs Phase 10 tip**:
          ...
          user's Skynet client now downloads ~125 kB less across first-load
          ...
          deleted the Skynet client surfaces" story.
        ...
        * **Design source-of-truth** (LOCKED, no re-litigation per CONTEXT.md):
          ...
          - `~/.claude/identities/tina/bounties/skynet-transformation-purge-dead-surfaces/`
            — the bounty premise, user's UAT quote (same quote motivating
        ...
        * **Commits** (all on `feat/tab-title-from-tmux`):
          ...
          - **Plan 05 (Wave 1 — shell/Tab.tsx delete)**: 2 commits
            - `5357279` `chore(shell): delete retired Tab.tsx (Phase 12
              PURGE-08 — Skynet tab bar chrome retired implicitly by Phase
              11 landing swap)`
        ...
        * **Deploy status**. Code-complete on `feat/tab-title-from-tmux` at
          ...
          patches per user 2026-07-23 fleet-standing "batch patches into
...
1. **Update the count line** near the top of the file. Current state depends
   ...
   - Verify current count first with `grep "numbered patches"
     ~/.claude/identities/tina/skynet-patches.md | head -3`; if an
...
4. **Update `~/.claude/identities/tina/tina.md`** compact overview if
   ...
   tina.md's box-map may need a Skynet operational-context adjustment
   ...
   Skynet section to reflect the smaller API surface).
