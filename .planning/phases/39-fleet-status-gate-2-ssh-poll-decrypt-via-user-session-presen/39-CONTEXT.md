...
**Source:** Pre-authored from Gate 2 diagnosis bounty (`fleet-status-ssh-poll-decrypt-and-lazy-lifecycle`) + user design decision LOCKED in-session 2026-08-13. Not run through `/gsd-discuss-phase` — design decisions already captured verbatim below.
...
user verbatim on the design rationale: *"nobody needs to know if something is idle or not, or anything else that's going on here, if no user is present to want to know the information."*
...
### user's principle worth surfacing to the planner
...
Skynet on this box is single-tenant (only user), but the fleet-status pipeline is user-scoped by design (per-connection auth, per-user host ownership). Consider forward-compat:
...
- user has not spoken to this specifically; planner picks the cleanest shape.
...
- No hard-fail-if-hook-missing test — the fail-open contract for missing `last-stop-payload.json` per host stays (locked by user in Phase 34).
...
*Context authored 2026-08-13 from Gate 2 diagnosis bounty + user Path C decision in-session*
