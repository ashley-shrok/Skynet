...
Push is NOT authorized as part of slice 2's execution. Executor motion stops at commits + scoped-tests green. Full-suite green happens at the orchestrator ship-gate before push. The push itself waits for user's explicit greenlight per the standing push-boundary rule. Deploy (docker build + force-recreate + verify) is orchestrator-owned and happens after greenlight, per the standing rule that executors don't do deploys.
