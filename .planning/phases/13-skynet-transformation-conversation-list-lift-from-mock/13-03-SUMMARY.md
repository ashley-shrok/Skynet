...
decisions:
  ...
  - "Added `:hover` glow boost (drop-shadow from 4px/0.55 to 6px/0.75) and `:focus-visible` hue-tinted outline (a11y — the retired Termix chrome had `hover:bg-white/[0.06]` as the only affordance signal; the new bare-icon needs its own hover + focus signals). Both are palette-consistent additions the mock didn't cover but that Ashley's fork requires for keyboard-nav accessibility."
...
**PinAction desktop branch rebased to the mock v4 bare-icon-with-hue-drop-shadow treatment — `color: hsla(var(--pv-hue), 80%, 70%, 0.95)` + `filter: drop-shadow(0 0 4px hsla(var(--pv-hue), 80%, 60%, 0.55))` — retiring the last 2 Termix theme-class hits in the conversation-list subtree and delivering SHAPE-03 verbatim per Ashley's "totally obnoxious" callout.**
...
- `src/ui/features/pretty-conversations/PinAction.tsx` (124 → 113 lines):
  - Retired: `iconColorStyle` derivation (JS-computed hue color), `iconClassBase` derivation (JS-computed Termix theme-class string), Tailwind chrome classes (`w-6 h-6 rounded-md bg-transparent border-0 hover:bg-white/[0.06] transition-colors duration-100 cursor-pointer`), inline `style={iconColorStyle}` prop on the button, icon size classes (`w-3.5 h-3.5` on <Pin>/<PinOff>).
...
- **Added `:hover` glow boost + `:focus-visible` outline:** Not in the mock (which is a static HTML mock without hover/focus signals). The retired Termix chrome had `hover:bg-white/[0.06]` as the only hover affordance signal; the new bare-icon needs its own hover + focus signals for accessibility. Both use `hsla(var(--pv-hue), ...)` to stay palette-consistent.
...
- **T-13-03-03 (Repudiation — which layer purged the last Termix theme-class hit):** Task 1's final grep gate returned 0 non-comment hits. Purge is definitively this plan's work — PinAction.tsx lines 97/98/101 in the pre-plan file were the only remaining hits, and they are gone from the post-plan file.
...
- **Wave 5 (13-05) — Build-verify + UAT checklist + patch draft:** All conversation-list-surface Termix theme classes purged. The subtree is 100% `--color-pv-*`/`.pv-*`-scoped. Build-verify + upstream-rebase-diff can proceed cleanly.
...
- **Files verified exist and are modified as declared:**
  - FOUND: `src/ui/features/pretty-conversations/PinAction.tsx` (113 lines, references `pv-pin-action-desktop`, no Termix theme classes)
...
- **Termix theme-class purge verified:** `grep -rE 'text-muted-foreground|hover:text-foreground|bg-background|bg-card|text-foreground|border-border|muted-foreground' src/ui/features/pretty-conversations/` returns 0 non-comment hits.
