...
**Source:** Direct user conversation (user 2026-07-23, this session) — no discuss-phase needed because the mock IS the source of truth and user has been telling me this framing across MANY sessions; the failure pattern up to now was not context-gathering, it was fragmenting the movement into sibling bounties instead of trusting the mock as the target.
...
**Strictly OUT of scope (verified with user 2026-07-23 verbatim):**
...
- `src/ui/components/*.tsx` — shadcn primitives (input, skeleton, sidebar, card, sheet, sonner, password-input, command, tabs, alert-dialog, switch, etc.). Ship-of-Theseus rule: they still serve the RDP/SSH dialogs and xterm.js chrome that user DOES see when she uses RDP/SSH, and preserving them preserves upstream Skynet rebase-ability.
...
- Backend routes — Phase 13 was originally scoped as backend route cleanup, and user called that off in the 2026-07-23 mid-purge discussion (kept for rebase-ability, zero user impact). This Phase 13 is the RENAMED phase — the backend-routes-cleanup phase is dead.

**The mock is the source of truth.** `~/.claude/identities/tina/bounties/skynet-transformation/prototype.html` (mock v4, user signed off 2026-07-23 07:20Z). The v4 lock is: Full bubble intensity + Normal density + active-set/ambient recession + ONE dot with ONE meaning (row is in active set AND agent is idle). "Reduced" and "Selection-only" intensity variants in the mock are exploratory — Full is what ships. Density variants (tight/normal/cozy) — Normal is what ships. Do NOT re-litigate v4 lock — it's the target.
...
- Row.active-set (default = row is in user's active set, i.e. full bubble): base treatment applies as-is.
...
- Row.working: displays no dot (only ambient rows display no dot, and dot appears only on `active-set:not(.working)` rows per user 2026-07-23 v4 lock — "one meaning: this row is in the active set AND its agent is idle").
...
- `src/ui/features/terminal/Terminal.tsx` — `isIdle` starts at null, flips on backend WS ticker. Check whether ticker has fired for user's active session post-recreate.
...
<specifics>
## Specific requirements from user (this session, 2026-07-23)
...
## Meta-lesson pinned this session (user 2026-07-23 verbatim)
...
- Backend routes serving now-dead UI — deliberately deferred forever per user 2026-07-23 (zero user impact + upstream rebase-ability preservation).
