...
dependency_graph:
  requires:
    ...
    - `~/.claude/identities/tina/bounties/skynet-transformation/` (MASTER bounty — closes on user UAT sign-off)
    - `~/.claude/identities/tina/bounties/skynet-transformation/prototype.html` (LOCKED mock v4, user signed off 2026-07-23)
  ...
  provides:
    ...
    - 13-UAT-CHECKLIST.md — user's post-deploy verification walkthrough (Desktop 1-12 + Mobile 13-20 + Cross-viewport 21-26 + Failure route-back table + Post-UAT deploy runbook + Master bounty closeout note)
  ...
  affects:
    - Master bounty `~/.claude/identities/tina/bounties/skynet-transformation/` — closes on user UAT sign-off, marking Ship-of-Theseus movement (Phases 11+12+13) complete
    - The three-patch batch (#138 + #139 + #140) queues for user's greenlight per fleet-standing "batch patches into meaningful deploys" rule
...
decisions:
  ...
  - "Referenced ONLY the master bounty `~/.claude/identities/tina/bounties/skynet-transformation/` throughout all three deliverables — no sibling bounty names, no sibling bounty references. Per CONTEXT.md § meta-lesson (user 2026-07-23 verbatim: 'I fragmented the Ship-of-Theseus movement into sibling bounties instead of tracking it all inside the master bounty')."
  - "The UAT checklist includes an explicit 'Master bounty closeout note' section binding user's UAT sign-off to the master bounty's `/close skynet-transformation` action. This makes the phase-13-completes-the-Ship-of-Theseus-movement narrative traceable in the fork docs, not just in Tina's identity file."
...
**Phase 13 docs closeout: authored 3 deliverables (13-BUILD-VERIFY-LOG.md + 13-UAT-CHECKLIST.md + 13-PATCHES-MD-ENTRY.md) capturing the phase-boundary code-complete-clean state, user's post-deploy verification walkthrough, and the patch #140 write-up ready for Tina's `skynet-patches.md`. Ran the full build-verify sweep (tsc + full vitest + npm run build) + phase-boundary grep hygiene gates — all PASS at the phase tip. Ship-of-Theseus movement (Phases 11+12+13) is code-complete-pending-user-UAT-and-deploy-greenlight.**

## One-Liner

Phase 13 closes as **Skynet-SHAPE-COMPLETE-pending-user-UAT-and-deploy-greenlight**. The three-patch batch (#138 + #139 + #140) queues for user's greenlight; her UAT sign-off closes the master `skynet-transformation` bounty and completes the full Ship-of-Theseus movement.
...
- **Task 1: 13-BUILD-VERIFY-LOG.md authored** (5-section structure per Phase 11 + Phase 12 precedent):
  ...
  - Section 5: requirement traceability mapping SHAPE-01 through SHAPE-07 to specific commits + verification evidence (SHAPE-05 deferred to user's live UAT per the plan's design)
- **Task 2: 13-UAT-CHECKLIST.md authored** (5-section structure):
  ...
  - Master bounty closeout note tying user's UAT sign-off to `/close skynet-transformation`
...
2. **This SUMMARY:**
   - `[Plan-05 summary SHA — filled after commit]` — `docs(13-05): summary — Ship-of-Theseus movement complete pending user UAT and deploy greenlight`
...
- `.planning/phases/13-skynet-transformation-conversation-list-lift-from-mock/13-UAT-CHECKLIST.md` (~330 lines)
  ...
  - Every UAT item is an OBSERVABLE assertion user can walk with side-by-side mock comparison
...
- `.planning/phases/13-skynet-transformation-conversation-list-lift-from-mock/13-PATCHES-MD-ENTRY.md` (~500 lines)
  ...
  - Post-paste bookkeeping section binds user's UAT sign-off to `/close skynet-transformation` master bounty action
...
- **UAT checklist includes explicit Master bounty closeout note** binding user's UAT sign-off to the master bounty's `/close skynet-transformation` action. This makes the phase-13-completes-the-Ship-of-Theseus-movement narrative traceable in the fork docs, not just in Tina's identity file.
...
| Every UAT item is expressed as an OBSERVABLE assertion user can check | PASS |
...
| user has a concrete UAT checklist covering SHAPE-01..SHAPE-07 for post-deploy verification | PASS |
| user has an authoritative deploy runbook citation that will not misdirect | PASS |
| Tina has a paste-ready patch entry for skynet-patches.md that references the master bounty (not a sibling) | PASS |
| The Ship-of-Theseus movement (Phases 11+12+13) is one user UAT away from complete | PASS |
...
- **T-13-05-01 (Tampering — false PASS in build-verify):** Task 1 ran tsc/vitest/build LIVE and captured verbatim output (Sections 1-3). All 21 grep gates run against HEAD (`ee4de1e`) with observed values recorded. No false PASS possible — user + future-me can re-run any gate and reproduce the observed values.
...
- **user's live UAT session:** `13-UAT-CHECKLIST.md` gives her a structured 26-item observation walk-through (12 Desktop + 8 Mobile + 6 Cross-viewport) with side-by-side mock comparison + DevTools snippets for SHAPE-05 primary verification.
- **user's `13-04-UAT-DIAG-LOG.md § Section 2` fill-in:** UAT checklist item 15 (Mobile SHAPE-05) + item 16 (mobile scroll) + item 17 (safe-area) explicitly reference the 13-04 diag log's Section 2C/D/E for verdict copying. Route-back matrix in Section 3 of the diag log translates her verdicts to follow-up actions.
- **Deploy handoff:** Post-UAT deploy runbook cites `deploy-runbook.md` verbatim; batching rule + check-before-recreate one-liner + fork CLAUDE.md 15-min deadman stale callout all captured. user (or Tina) can run the deploy from the checklist without needing to lookup runbook fragments.
...
- **Master `skynet-transformation` bounty closure:** user's UAT sign-off is what triggers `/close skynet-transformation`. The bounty's timeline+todos will reflect Phase 13 patch #140 completing the last item; no follow-up bounty needs to be spawned for this movement (per meta-lesson).

## Follow-up Candidates for Master Bounty

Enumerated in `13-04-UAT-DIAG-LOG.md § Section 3` route-back matrix. Highlights (contingent on user's UAT outcome):

- **Candidate B.1 fix (fresh-terminal path key mismatch):** Update `Tab.targetTmuxSession` from Terminal.tsx's `onTmuxSessionChange` callback so the row's key resolves to the actual backend session name. Estimated: 1-file fix in AppShell.tsx (openTabs management layer). Owner: master bounty follow-up patch (if user UATs FAIL_PARTIAL and this candidate is confirmed).
- **Candidate A.1 fix (Terminal.tsx isIdle mount race):** Add client-side `setIsIdle(true)` fallback on WS-attach or change Panel-side null-handling. Terminal.tsx edit is FORBIDDEN per Phase 13's SHAPE-06 scope-fence; Panel-side fix is possible but semantics-changing. Owner: master bounty follow-up patch (Terminal.tsx path) OR follow-up plan (Panel-side path, if user approves the semantics change).
- **Mobile scroll `touch-action: pan-y`:** single-line CSS addition to `.pv-row` in pretty-conversations.css. Trivial. Owner: master bounty quick-task if user confirms freeze on iPhone PWA UAT.
...
Phase 13 (patch #140) is code-complete on `feat/tab-title-from-tmux` at commit `058c362` (Plan 05 docs) + the SUMMARY commit landing this file. The three-patch batch (#138 + #139 + #140) queues for user's greenlight per fleet-standing "batch patches into meaningful deploys" rule. user's live UAT of the deployed batch (via `13-UAT-CHECKLIST.md` — 26 non-negotiable items) is the terminal handoff; her PASS sign-off closes the master `skynet-transformation` bounty and completes the full Ship-of-Theseus movement (Phases 11+12+13). The Skynet SHAPE — Telegram-mobile-app-of-Skynet — is done.

user's next set of Skynet bounties will be NEW-FEATURE work (pretty-view enhancements, message-queue improvements, translation asides, tool-use bubble upgrades) — not further Ship-of-Theseus purge.
