...
status: pre-UAT complete; blocked on user live UAT of deployed Waves 1-3 + shell chrome
tags: [ui, uat, diagnostics, ready-dot, mobile-scroll, safe-area, phase-13]
generated: 2026-07-23T15:22:08Z
generated_by: executor (autonomous pre-UAT static-analysis pass, per user's "go all the way through" directive)
---
...
- **Section 2** — user's UAT observations (TEMPLATE — user fills after
...
- user's UAT observation sections in Section 2 are TEMPLATES — the executor
  does NOT invent user's observations, only provides the scaffolding
...
plan requires source-read verdicts on all 4 before user UATs.
...
- **A.1 (Startup null-start):** If user clicks a row, Terminal.tsx mounts,
  ...
  emits an initial `{type:"idle", idle:true}` frame. Question for user:
...
- **A.3 (No cleanup effect):** The publishSessionWorking useEffect at line
  ...
  frame." — GOOD. This means when Terminal.tsx unmounts (e.g. user
...
**Recommended user runtime verification:**
...
3. **Value alignment (the tricky part):**
   ...
   - **Potential mismatch:** If a Tab is opened with `targetTmuxSession=null`
     ...
     **KEY MISMATCH POSSIBLE in the "opened without pre-picked tmux session"
     path.**
4. **In the fleet-derived synthetic rows path** (rows user sees for
...
- **B.1 (Fresh-terminal path):** user opens a NewSessionDialog, picks a
  ...
  shows on this row until user reloads and the row is re-derived from
  the updated Tab.**
...
- **B.2 (Session-picked path):** user opens a NewSessionDialog and PICKS
...
- **B.3 (Fleet-discovery path):** Row was auto-synthesized from a
  FleetSession, `targetTmuxSession: session.sessionName`. If user
...
4. **Fresh browser session, first page load, no sessionStorage entry yet**:
   activeSet starts as empty Set. user clicks Row A → `selectConversation`
...
5. **Page reload after user has clicked 3 conversations**: sessionStorage
...
user's iPhone Safari PWA, sessionStorage is standard and available.
...
freeze may still reproduce. If user confirms scroll works, no action
...
- **1B.1 (Angle tolerance too tight):** If `PC_SWIPE_ANGLE_TOLERANCE` is
  ...
  threshold before user has moved enough to feel scroll-lock. Value
  should be checked (Wave 1 preserved it from pre-Wave-1). Need user's
...
- **1B.3 (Safari PWA quirk):** iOS Safari PWA has known quirks with
  ...
  user perceives as freeze. user's DevTools trace would confirm
  or reject this.
...
user closes a Terminal.tsx-mounted pane, the last-known `isWorking` value
...
Rows synthesized from a FleetSession (user sees these for tmux sessions on
...
user hasn't opened (Terminal.tsx isn't mounted). So `useSessionWorking` returns
`null` for these rows → the row's condition `isWorking === false` is false →
dot never renders. **This is CORRECT semantics per user's v4 lock: dot
means "in user's active-set AND agent is idle." Fleet-only rows are NOT in
...
## Section 2: user's UAT Observations (TEMPLATE — user fills after UAT)

### 2.0 Deployment preflight (user checks BEFORE UAT)

- [ ] Waves 1-3 shipped to term.example.com (or local `npm run dev` if
      running against a workstation build)
- [ ] user freshly reloads the page (Cmd+R desktop / iOS reload PWA) to
...
**Deployment status:** _(user: fill after deploy)_
...
**user's overall verdict:** _PASS / DIFFERENCE_NOTED (with specific delta)_
...
2. Click into Row A (any conversation, ideally one with an idle Claude
   pane at the prompt). user Prime or similar known-idle identity.
...
**user's overall SHAPE-05 verdict:**
...
**If FAIL, user's DevTools verification (paste into console):**
```js
document.querySelectorAll('[data-pv-conv-ready-dot="true"]').length
```
Expected: 3 (one per clicked+idle row). Actual: _(user fill)_
...
- Notes: _(user fill; e.g., "last row's bottom edge is ~4px above home indicator, comfortable")_
...
- If FAIL, specific element that changed: _(user fill; ROUTES BACK as
...
- If FAIL, which surface changed: _(user fill; ROUTES BACK as SHAPE-06
  scope violation — hard fail)_

### 2G. Freeform observations (user's optional additional notes)

_user: any observation not captured above — visual quirks, unexpected
...
row states: what user observed, what the follow-up work is, whether the
...
| user's finding | Root cause hypothesis | Route-back action | Owner |
|-----|-----|-----|-----|
...
| FAIL_NO_DOTS: `data-pv-conv-ready-dot` querySelectorAll returns 3 but user sees 0 | CSS `display: none` is winning somehow (unlikely given inline `style="display: block"` at Row.tsx:418) OR the dot is rendered but visually hidden by a z-index / clip-path / opacity issue elsewhere | user inspects the DOM element in devtools: `document.querySelector('[data-pv-conv-ready-dot]')` → check computed style for `display`, `visibility`, `opacity`, and any parent with `overflow: hidden` clipping it. | phase-13 (diagnostic session) |
...
| FAIL_PARTIAL: only fleet-derived synthetic rows fail | **Candidate C ambiguity: fleet-only rows are never in activeSet on click (per intent) — they should be routed by `onDetachedRowClick` and NOT show dot until user attaches** | Verify user's expectation: dot on fleet-only rows is EXPLICITLY NOT part of SHAPE-05 (per Panel.tsx:53 detached-row plumbing). If user thinks it should — that's a Rule 4 architectural request, needs discussion. If not — closed. | closed (SHAPE-05 is active-set only) |
| FAIL_WRONG_ROWS: dots on non-clicked rows | activeSet incorrectly populated OR a shared sessionStorage entry from a prior session leaked in | user clears sessionStorage: `sessionStorage.removeItem('pv-conv-active-set')` in devtools console, then reloads. If dots reappear on non-clicked rows immediately → NEW bug in addToActiveSet call path (not previously suspected). If they don't reappear → stale sessionStorage cleared correctly. | phase-13 (13-05 or 13-04-follow-up) if reproducible after clear |
| FAIL: dot doesn't appear until user switches away from the pane and back | **Candidate A confirmed: no client-side "assume idle at connect" fallback; backend's initial-state `{type:"idle"}` frame lag** | Options: (1) Add client-side default `setIsIdle(true)` in Terminal.tsx after WS-attach and before backend's first frame arrives — but this is a **Terminal.tsx edit, forbidden in this plan** per scope boundary. Route-back is: **fold into master bounty as a follow-up** or into a Phase 14 plan that owns Terminal.tsx. (2) Change Panel-side null-handling: treat null-store-state as "assume idle" for rows already in activeSet — riskier semantics change, needs user's Rule 4 approval. | master (Terminal.tsx edit forbidden here) OR phase-13 (13-04-follow-up if Panel-side null-handling change is preferred) |
| PASS but with a delay: dot appears 1-2 seconds after clicking | Same as above (Candidate A) — backend's initial-state frame lag is real but tolerable | Close SHAPE-05 as PASS with a note in the master bounty about the initial-frame lag for future optimization. | closed (user likely accepts this delay per verbatim "just so I know they've engaged") |
...
| FAIL: last row behind home indicator | Workaround INSUFFICIENT — `pb-[env(safe-area-inset-bottom)]` isn't being applied OR is being overridden by an ancestor `overflow: hidden` | user checks in Safari devtools whether the padding is computed. If yes but visually failing — deeper layout issue (parent `min-h-0` may be collapsing the padding). If padding is 0 — Tailwind's arbitrary value class isn't compiling; fix is to move to inline style. | phase-13 (13-05 or 13-04-follow-up) |
| PASS but user wants the architectural fix now | Not phase-blocking; nice-to-have | Add a follow-up plan to move safe-area compensation to AppShell.tsx:1400 outer wrapper and revert PrettyConversationsPanel.tsx:233's workaround. | master (SHAPE-04 explicitly limited to chevron; broader AppShell change is a Ship-of-Theseus master-bounty patch) |
...
## Section 4: Deferred Route-Backs Awaiting user's UAT
...
- user's UAT sections (2A-2G) are TEMPLATES ready for her live observation
- Route-back matrix (3A-3F) is EXHAUSTIVE per plan requirement

**Executor's provisional recommendation to user (based purely on static
analysis):**
...
If user's UAT shows this exact partial-failure pattern (rows work for
...
in static analysis. user's UAT observation of "dot appears with a delay"
vs "dot never appears" will disambiguate.

**Candidates C and D are PASS by static analysis and should not be
suspected first.** If user's UAT shows total FAIL_NO_DOTS but the
...
**user: proceed to live UAT of Waves 1-3 + shell chrome. Fill in
...
- **Section 2 (user UAT template) exists with placeholder verdicts:** yes
...
- **Executor did NOT invent user's observations:** confirmed; all Section 2
