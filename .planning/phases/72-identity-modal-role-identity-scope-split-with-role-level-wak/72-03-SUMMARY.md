...
key-decisions:
  ...
  - "First-tab labels moved from 'Role' / 'Identity' (position-0 in the pre-72 flat NAV_SECTIONS) to 'Role file' / 'Identity file' — the scope switch owns scope, so the tab label describes the artifact. user's shape file variant D mockup matches this treatment"
