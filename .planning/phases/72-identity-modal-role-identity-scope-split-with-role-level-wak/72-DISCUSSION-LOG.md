...
1. `/build "update the identity modal to support both role-level and identity-level wakeups"` — user pitched the mismatch: current modal shows only identity-scope wakeups, so a coordinator (who only has role-scope wakeups) sees "no scheduled wakeups" — technically true, entirely unhelpful. Also raised broader dissatisfaction with the flat 6-tab strip mixing role-scope and identity-scope content in one flat list.
2. `/open identity-modal-scope-split` — pitched shape back to user (scope-legibility redesign, coordinator case as the strongest argument for doing this at all, history-tab-scope surfaced as a possible confusion — turned out user had misspoken and it IS role-scope per id-skill canonical text, no change needed).
3. user asked for a "tasting" (visual variant comparison) after her direction was "hard to say" — pushed to `/gsd:sketch` to produce 5 layout variants (grouped strip, stacked sections, left rail, top scope switch, dashboard cards), each showing actor-identity AND coordinator-identity states side-by-side.
4. Sketch served on tailnet `http://100.99.149.8:8899/001-identity-modal-role-vs-identity-split/`. user picked **variant D — Top Scope Switch** with "definitely D". Winner marked in README + MANIFEST + ★ on the tab. Sketch commit HEAD `f5c8f459`.
5. Returned to `/open` to write shape file. Vehicle proposed: GSD phase (backed by fleet standing directive that phase-shaped work gets a phase, plus real backend surface, plus 4 modal test files in blast radius). user greenlit vehicle with thumbs-up.
6. Shape file written at `.planning/shapes/shape-identity-modal-scope-split.md`. user greenlit running `/gsd:phase` with thumbs-up.
...
All items below are LOCKED with defaults grounded in the existing codebase patterns above. Each is a decision that the shape file did not lock but that the planner needs to know before drafting plans. user can override any of them in one line before `/gsd:plan-phase` runs.
...
- Alternative to rendering the Role file tab under coordinator Role view (currently shows "not loaded" empty state; could hide entirely — deferred until user sees shipped version).
