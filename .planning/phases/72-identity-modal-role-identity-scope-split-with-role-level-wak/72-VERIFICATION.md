...
Remaining is user's real-browser walk-through (5 items in `human_verification`), which is the human-verify checkpoint that Phase 72 explicitly deferred to post-ship. Pre-existing PrettyView test failures are documented + flagged as outside Phase 72 scope.
