...
**One-liner:** Phase 4 code-side ready for deploy — `npm run build` clean, all Phase 4 tokens survived Vite + Tailwind v4 tree-shaking, Terminal.tsx / backend / docker / nginx / dependencies all untouched, and two new artifact files (`04-UAT-CHECKLIST.md` + `04-AGENTS-MD-ENTRY.md`) prepared for user's separate deploy green-light.
...
**Coverage**: all 10 VISUAL-01 through VISUAL-10 requirements represented as walk-through sections with concrete in-app actions + expected outcomes + failure indicators. user walks the checklist post-deploy, checks boxes, signs off.
...
1. user reviews `04-UAT-CHECKLIST.md` and gives explicit per-deploy green light (blanket pre-authorization ≠ per-deploy green light per tina.md rule).
...
5. user walks `04-UAT-CHECKLIST.md` to verify visual-01 through visual-10 all pass on the live deploy.
...
If any UAT step fails: the 15-min deadman auto-reverts, or user can trigger immediate revert via `sudo bash /opt/skynet/.tmp-revert.sh`.
