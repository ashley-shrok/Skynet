...
- **Expected observable:** The message reaches the agent. It appears as an user turn in the conversation stream (or the queued message drains to the agent). The send path (pvSendInputRef / pvSendInterruptRef, now owned by IdentitySessionPane wrapper instead of Terminal.tsx) works correctly.
