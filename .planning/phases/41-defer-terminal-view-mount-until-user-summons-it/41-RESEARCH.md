...
1. **Aside-arm trigger** (L1997): `if (prev === false && isIdle === true && pvIdentity != null)` — triggers `{type:"aside_arm"}` on PV WS. NOTE: aside auto-fire is currently DISABLED (L69-73 comment: "user 2026-07-27: automatic aside triggering DISABLED"). The machinery exists but is not active.
