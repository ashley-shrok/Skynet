...
provides:
  ...
  - "41-UAT-CHECKLIST.md: 10-item Nyquist-style checklist for user's post-deploy walk"
...
affects:
  ...
  - user's UAT walk (consumes checklist)
...
Per `workflow.human_verify_mode=end-of-phase`, Task 2 cannot be closed by the executor. This is the hold-point for orchestrator-driven deploy + user's UAT walk.

**Current status:** Awaiting orchestrator deploy motion (docker build + docker compose up --force-recreate + HTTPS 200 + coord-room BEFORE/AFTER) and user's sign-off on 41-UAT-CHECKLIST.md items 1-10.

**Resolution path:** user walks 41-UAT-CHECKLIST.md items 1-10 against the deployed build and signals "UAT green" (or per-item resolution) → /gsd-verify-phase marks Task 2 done and Phase 41 complete. If items fail → gap plans (41-04+) via `/gsd-plan-phase 41 --gaps`.
...
*Task 2 (UAT gate): HELD OPEN — awaiting orchestrator deploy + user sign-off*
