...
**Portal note**: PrettyConversationsPanel does NOT contain portaled content. The conv-list rows and header are all normal React children. React synthetic listeners are safe here — Phase 58 shipped and user UAT confirmed the drop mechanics work end-to-end. No native-DOM-listener migration needed.
...
3. **Structured logging** (per box-maintainer standing directive user 2026-08-11). Two new prefixes, both zone-change-gated (mirror Phase 57's `prevZoneRef` pattern to avoid drag-hover log spam):
...
- Any changes to the coral palette values themselves — reuse Phase 57's `rgba(255, 184, 150, 0.22)` fill + `0.60` border verbatim. Any palette change would be a design decision that should go through user separately.
...
- **1 plan option (recommended)**: 59-01 covers both gaps in one plan — the pattern is identical, the code is small, and shipping both in one atomic patch #517 matches user's expressed intent (bounty is one thing, both gaps land together).
