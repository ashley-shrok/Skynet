...
**Threshold parameters:** `-40dB` noise floor is appropriate for clean microphone input. `-35dB` may be safer if user's recordings have more ambient noise. Duration `0.2` (200 ms minimum gap) is the CONTEXT.md choice; gaps shorter than this are treated as speech.
