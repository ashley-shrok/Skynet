...
**UAT scenario:** Start `/id-reset` on identity X while PrettyView for identity X is NOT currently mounted (e.g., user is viewing a different identity's panel and identity X's tab is closed).
