...
- **D-09:** **The pulse MAY fill in appearance; appearance-writing stays ADDITIVE, never wholesale-replacing.** An answer that knows less must never blank out one that knew more, or the list would UNDRESS itself on a later tick — the same symptom this phase fixes, arriving from the opposite direction. Exactly ONE place appearance can be written; both paths go through it.
  - ⚠️ **Reconciliation with D-10 (pattern-mapper finding, 2026-09-16):** that single door CANNOT be `setIdentities` as it stands. `setIdentities` (`identities-store.ts:87-92`) sets `loaded: true` **unconditionally, with no branch**, and all four existing writers funnel through it — so "reuse the existing door" is simultaneously a D-10 violation. The single door must therefore be a **NEW additive-merge entry point** that writes appearance and notifies subscribers WITHOUT touching `loaded`. `patchIdentityFlag` (`:426-448`) is the right model for everything except its final `setIdentities` call. When D-09 and D-10 appear to conflict, **D-10 wins** — it is protecting against a reproduced bug (terminal-flash + listener leak), whereas D-09 is protecting against drift that has not happened yet.
- **D-10:** **⚠️ The `loaded` flag stays owned by the fuller request — the pulse must NOT set it.** This is the decision with a real bug behind it. `identities-store` answers TWO questions with one piece of data: "what does this look like?" AND "is this an agent at all?" The second drives the pane discriminator in `tabUtils.tsx:282-284` (`identitiesByKey.has(identityKey) || !identitiesLoaded`) — note `:205` is only the `useIdentities()` hook call, NOT the discriminator, and answering it wrongly-early already caused a Terminal to boot an xterm + real SSH WS and then unmount, leaking listeners (see the 2026-09-08 comment block at `identities-store.ts:258-273`, and the deliberate empty-map skip-guard at `:274-277` that exists solely to prevent it; bounty `terminal-first-flash-on-reload-plus-listener-leak`). Rule: **the pulse makes rows pretty; it never makes the app conclude an agent does not exist.** Single-authority for appearance is preserved; the existing hydration-race guard stays exactly as load-bearing as it is now.
...
- `src/ui/shell/tabUtils.tsx:282-284` — the `isIdentityPane` discriminator (`identitiesByKey.has(identityKey) || !identitiesLoaded`). The consumer of the `loaded` flag D-10 protects. ⚠️ `:205` is the `useIdentities()` hook call only — an earlier draft of this file mis-cited it as the discriminator.
