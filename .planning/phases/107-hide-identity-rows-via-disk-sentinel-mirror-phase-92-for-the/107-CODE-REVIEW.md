...
**Why it matters:** Real deploy scenario — one of user's fleet hosts is momentarily unreachable during a "reorganize pins and hides" PUT. Pin writes on reachable hosts succeed; hidden probe against the unreachable host throws (openConnForHost line 128 `connectOneShot(host, 5_000)` timeout); everything unwinds with a 500. user retries; disk still holds the earlier pin writes; frontend refreshIdentities re-projects. Confusing but not corrupting. Client-visible symptom: a "failed" PUT actually partially succeeded.
