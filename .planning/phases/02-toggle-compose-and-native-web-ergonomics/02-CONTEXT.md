...
### RENDER-04 and RENDER-05 are verify-only, not implementation (user clarified at /id reset 2026-07-17)
...
- Chord choice: **Ctrl+Shift+O** is the recommended default (unbound in Chrome + VS Code + user's OS binding table per patch #37 lesson-learned; adjacent-alphabet to Ctrl+Shift+I devtools but not colliding). Planner should confirm with user before committing.
...
- The chord chosen MUST be verified against user's live OS bindings before ship. Patch #37 shipped Ctrl+Shift+K and had to be resworn to Ctrl+Shift+L because user's OS-level speech-to-text hotkey ate K. Add a UAT step: "user confirms the chord flips modes on all her active OS-level binding profiles."
...
### Claude's Discretion
- **Chord choice** — Ctrl+Shift+O recommended, planner should propose 1–2 alternates in case user has a conflict.
...
### The project scope
- `.planning/PROJECT.md` — Skynet fork context; Core Value = user never loses access.
...
- The fork's mandatory 15-min deadman rollback timer (`/opt/skynet/.tmp-revert.sh`) fires on every deploy per user 2026-07-03; no exceptions.
...
- Fleet directive (user 2026-07-12): pre-authorized code work does NOT authorize the deploy — every build → deploy transition is a distinct ask. Planner should note this so the executor doesn't stumble.
...
**Out of Phase 2, in a follow-up patch if user wants it:**
- Multi-line preservation on send (COMPOSE-05 note): compose textarea preserves newlines visually today per browser default; whether the send path preserves them as `line + \r + line + \r ...` chunks or collapses to spaces is Claude's Discretion in Phase 2. If user later wants literal multi-line end-to-end, it's a small follow-up.
