...
type: context
source: ROADMAP.md § Phase 32 (LOCKED via design conversation with user 2026-08-12)
...
**Consumer context (from ROADMAP, verbatim):** "wake bubble message history (from the just-shipped `dormancy-bubble-in-flow` quick task) is the first and currently only consumer. user 2026-08-12: *'the bubble looks good, but unfortunately, the rest of the messages that would be in that session are not showing up.'*"
...
Their first user turn isn't `/id <name>` (user opens a shell, launches claude with no `/id` invocation), so they never match — the helper is identity-only BY CONSTRUCTION. **No explicit throwaway-pane filter needed.** This falls out of D-02 + D-01.
...
Wake-bubble message history (from the just-shipped `dormancy-bubble-in-flow` quick task, patch #422, 2026-08-12) is the first and currently only consumer. The predecessor quick-task SUMMARY is at `.planning/quick/260812-ma8-dormancy-bubble-in-flow/260812-ma8-SUMMARY.md`. user's verbatim direction after that ship: *"the bubble looks good, but unfortunately, the rest of the messages that would be in that session are not showing up."* Phase 32 closes that gap.
