...
**Out of scope (user's manual post-close pass — NOT part of this build):**
...
- **D-20:** Convention is going-forward only. Existing runbooks on user's manual post-close pass. Do NOT add a "for existing runbooks, main file is `<slug>.md`" migration-note in the id skill — the manual pass renames them all to `runbook.md`, so no grandfather rule needs to be codified.

### Ship discipline (D-21, D-22)
- **D-21:** Campaign constraint holds — commits + push + scoped tests only. No `docker build`, no `docker cp` fast-path, no `docker compose up --force-recreate`, no full-suite test run until entire UX-pass campaign lands AND user greenlights ship gate.
...
- **Skills-parity is the design bar.** Any UX detail in the runbook editor that has a corresponding detail in the skills editor should match unless there's a specific reason to diverge. user explicitly asked to "steal from skills modal" during /open.
