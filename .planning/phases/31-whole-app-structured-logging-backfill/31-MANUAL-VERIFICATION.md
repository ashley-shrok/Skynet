...
**Purpose:** Phase 31 shipped instrumentation only (D-22 — no bugs fixed). The two deferred symptom bounties (`ws-pause-gate-stuck-connect-cycling`, `speak-button-broken-on-cellular`) are unblocked once user reproduces on the instrumented app and confirms the new log trail lands as expected. This document is the post-ship verification procedure.
...
Per box-maintainer.md convention: if user doesn't reproduce and report back within a reasonable window (several days post-deploy), Phase 31 is assumed verified. This runbook is not a blocking gate — it is a preferred path that unlocks faster diagnosis of the deferred bounties.
