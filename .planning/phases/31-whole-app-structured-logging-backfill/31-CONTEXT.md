...
- **D-09: My call, optimized for grepability and diagnosis speed.** Alice 2026-08-11 verbatim: *"the conventions should be whatever makes it easiest for you to find and figure out issues, because I won't be looking at the logs, you will."*
...
- **D-20: Light testing only.** Smoke tests for the MOST critical log emissions — WS `close` event structured fields, pause-gate transitions, TTS play-attempt result — asserting the line fires with the expected shape. NOT contract tests on every log line. Alice 2026-08-11 verbatim: *"testing is potentially helpful but probably not totally necessary."* Non-blocking: a missing smoke test doesn't fail the phase; a wrong-shape one does.

### Follow-on scope (D-21..D-22)

- **D-21: The two symptom bounties are DEFERRED behind Phase 31.** `ws-pause-gate-stuck-connect-cycling` and `speak-button-broken-on-cellular` don't get diagnosed or fixed IN this phase. After Phase 31 ships and Alice reproduces, we open follow-up phase(s) informed by the log data.
- **D-22: Instrumentation-only phase scope.** Alice 2026-08-11 locked (a) — Phase 31 does NOT bundle fixes for either symptom bounty, even if the root cause becomes visually obvious while adding logs. Discipline: if I notice a bug during the backfill, capture it as a bounty and file it — don't fix inline.
...
- **iOS Safari as PRIMARY runtime** — every log-line design decision must survive Alice's iPhone PWA workflow. Instrumentation cost is real if it slows the phone; batched-post amortizes that. UA is captured in DIAG-REPORT and useful for cross-device correlation.
...
- **Dedup follows syslog** — "last message repeated N times" is Alice's asked-for pattern verbatim, from her "I'm sure there are standards" comment.
...
- **Log-shipping to structured store / dashboards** — nothing currently consumes the console-forward log except me grepping. If Alice ever wants Grafana/Loki-style dashboards, that's its own phase. Not this one.
...
- **Fix for `ws-pause-gate-stuck-connect-cycling`** — deferred, becomes a phase after Phase 31 ships and Alice reproduces on the instrumented app.
