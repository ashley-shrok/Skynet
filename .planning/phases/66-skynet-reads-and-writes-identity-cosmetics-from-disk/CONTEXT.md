...
- **3 orphans, all adjudicated:**
  ...
  - `commander-zoey` — folder on ZoeyBattlestation is `commander zoey` (space, not hyphen), violates the id-skill invariant. user greenlit "accept the ugly render post-Phase-B" as a scoped edge case; no rename in this phase, no fallback code.
...
- **Per-push greenlight** — every git push requires a fresh "may I?" from user (deploy-window boundary). Multi-step pre-authorizations authorize the CODE motion only, never the push.
