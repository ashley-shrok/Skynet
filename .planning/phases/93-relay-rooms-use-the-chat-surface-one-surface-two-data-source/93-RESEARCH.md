...
- **Executor doesn't ship** (fleet rule, user 2026-08-08). Plans MUST NOT include a "ship" task at executor scope. Executor's remit stops at code + commit + scoped tests green. Deploy motion (push, docker build, docker compose up, verify, coord-post) is orchestrator-owned.
- **Scoped tests during dev; full suite at deploy gate** (fleet rule, user 2026-08-20 + 2026-09-07). Executor's green-gate is `npx vitest run --related <changed-files>` OR targeted paths (e.g. `src/ui/features/pretty-view/`), NOT full suite.
- **No message streaming anywhere** (fleet rule). Do NOT design any streaming state, do NOT add streaming affordances.
- **Regression floor: harness case unchanged.** Every slice ships with user's daily chat surface looking + behaving EXACTLY as today. If a slice can't clear that gate, stop and rework. This is the strongest "what would make it wrong" from the shape.
...
**What goes wrong:** The multi-badge extension or source-prop plumbing accidentally shifts harness-case badge position by 1px, changes hue calculation, or introduces a stray z-index conflict. user notices in daily use; regression escalates.
...
Not applicable — this is a refactor of internal code, not adoption of a new library or ecosystem shift. Slice D shipped ~2 days ago (Phase 90, 2026-09-08 planning; Phase 91 UAT fix 2026-09-09). Phase 93 reverses Slice D's architectural choice within the same week based on user's UAT reformulation.
