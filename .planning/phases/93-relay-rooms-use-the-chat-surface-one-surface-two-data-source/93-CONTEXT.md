...
- **D-18: Badge-click in relay rooms — no-op v1.** In the harness case, badge-click continues to open the agent's bounties (unchanged from today). In the relay case, badge-click does nothing. Revisit later if a real interaction emerges. user 2026-09-09 verbatim in grill: *"clicking on badges in relay rooms just will do nothing right now. We might come back to it later."*
...
- **The harness case is the regression floor.** user's daily chat surface must look and behave EXACTLY as it does today after every slice lands. This is the strongest "what would make it wrong" from the shape. Every slice ships with a "harness case unchanged" as an acceptance gate. If a slice can't clear that gate, stop and rework.
