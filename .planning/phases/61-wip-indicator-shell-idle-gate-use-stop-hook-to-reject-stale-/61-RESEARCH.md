...
- **57-04** — Deploy checkpoint + UAT verification (lazy rollout — user walks through Poppy/aqua/wilma, confirms next turn-end flips the indicator off correctly).
...
**Warning signs:** Indicator flickers back on every time user types a character in the compose box.
...
**How to avoid:** Accept it (documented tradeoff). At <1KB per file, 1000 sessions/year on a busy box is ~1MB/year — safely below any noticeable disk-usage threshold. user can `rm -rf ~/.claude/fleet-status/stop-*.json` manually if it ever becomes an issue. If a future phase wants cleanup, hook it into whatever session-lifecycle cleanup already exists on the managed box.
...
1. **Managed-box `bash` vs `sh` for the Stop hook's inner subshell.**
   ...
   - Recommendation: change the inner subshell to `timeout 2 bash -c "..."` (bash is present on every non-embedded Linux distro — it is a dependency of the harness itself since Claude Code requires it). Verify with `which bash` on user's target fleet before finalizing the script.
...
**Missing dependencies with fallback:** BSD `stat` if any managed box uses it. Fleet is presumed Linux-only per prior phases; verify with user if any macOS or *BSD boxes are in the fleet.
