...
The design is LOCKED from Wave 3's user UAT; this plan's job is
...
sites (three open partitions + archive). user UAT (Task 3) walks
...
Handle the archived bounty case: SCRATCH-REPORT.md may specify that some editors are disabled for archived bounties (user's decision). Honor that gate — if the report says "title is read-only on archived cards", do not render the title editor when `archived === true`.
  </action>
  <verify>
...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: user UAT — bounty-field editors work end-to-end plus IDMEDIT-07 non-regression walkthrough</name>
  <what-built>
...
| T-18-25 | Repudiation | user edits meeting_question then blames agent | accept | Per IDMEDIT-08 semantics + Wave 3 SCRATCH-REPORT.md — meeting_questions user-authoring is a UI convention. user is the only user; audit trail is bounty.json timeline entries (each meeting_questions edit generates one `<ISO-Z> meeting_questions updated via identity modal` line). Repudiation surface accepted. |
...
- user UAT Task 3 approved on all 13 steps
