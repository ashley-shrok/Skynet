...
Wave 2 lands as ONE atomic commit at user's request (all three touches ship together as a coherent unit; the type-only change on its own is unusable, and the banner component on its own is unmounted dead code):
...
- **`session-file-parser.ts` NOT modified** — user wants `<command-name>` / `<local-command-stdout>` visible in pretty view (CONTEXT.md HARD REJECTED).
...
- Ready to batch-deploy with pending patches #61/#62/#63 at user's greenlight per bounty `pending-patch-batch-post-60`.
