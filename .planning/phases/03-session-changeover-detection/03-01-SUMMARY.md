...
key-decisions:
  ...
  - "Layer 1 /exit scan FALLS THROUGH (no return after transitionToHolding) so the parser still emits the /exit turn as a chat bubble — user HARD LOCK: slash commands must remain visible in pretty view"
