...
**Recommendation:** Recommend (b) as a small addition to the phase scope — the fix is one line in Auth.tsx (`t("auth.loginTitle")` → `t("common.login")` or similar) plus a locale-key deletion. But this is a scope-expansion decision for user, not for research to lock. Flag it, don't decide it.
...
1. **Should `t("auth.loginTitle")` = "Login to SKYNET" (Auth.tsx L1314) be part of Phase 70 scope?**
   ...
   - What's unclear: Whether user considers this in-scope or deferred.
   - Recommendation: Flag to user in planning phase; if in-scope, add a single-line change: replace with a stack-neutral phrase (e.g., `t("common.login")` = "Login") OR wire through branding config (`Login to ${brandingConfig.appName.toUpperCase()}`).
...
2. **Should the bundled-default `iconPath` unify the conversation-header and login-screen icons?**
   ...
   - What's unclear: Which one is the "canonical" Skynet icon per user — or whether the two are BOTH intentional (icon.png is a photo/detailed version; SVG is a stylized simplified version).
