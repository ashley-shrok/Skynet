...
5. **Structured logging discipline** (box-maintainer standing directive, user 2026-08-11). New handler-level logs from AppShell:
...
**Executor scope**: code + commit + scoped tests green. Full-suite + docker build + deploy are orchestrator-only per box-maintainer role directive (user 2026-08-08).
...
**Parent bounty**: none (this is a shape-file-driven /build phase, not attached to a bounty). If user wants a bounty tracker, plan-phase can create one — but the shape file already captures the trail.
...
**No worktrees** (user 2026-07-31, fleet rule).
