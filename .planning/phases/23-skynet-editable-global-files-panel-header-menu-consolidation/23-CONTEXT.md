...
Give user a UI-driven way to edit configurable per-host files that live OUTSIDE
...
- Per-user gating (anyone with the admin cookie can read/write — matches user's
...
- **Cookie auth.** Anyone with the admin cookie can read/write — no per-user
  gating (matches user's `anyone could edit` decision 2026-08-04).
...
- **Per-user gating** — anyone with the admin cookie can read/write. If user
...
- **Windows-host support** — omitted from bootstrap (GEFM-06 explicitly).
  If user later wants to edit files on Windows hosts, that's a separate
