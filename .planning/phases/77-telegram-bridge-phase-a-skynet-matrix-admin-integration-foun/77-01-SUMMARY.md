...
- `users.mxid TEXT` column lands via `addColumnIfNotExists` — nullable, ready to accept externally-created mxids from the Plan 03 endpoint (POST /users/:id/mxid) for the three pre-existing hand-made accounts (user, Zoe, Laura) and any future users.
