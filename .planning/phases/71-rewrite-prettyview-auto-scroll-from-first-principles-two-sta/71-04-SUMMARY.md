...
dependency_graph:
  ...
  affects:
    - user real-browser verification (Task 2 — awaiting orchestrator ship)
...
decisions:
  ...
  - "Task 2 (human-verify) returns checkpoint — awaiting orchestrator ship to production + user real-browser walk"
...
**One-liner:** Whole-repo grep-gate sweep confirmed all 11 structural invariants GREEN after two inline fixes (redundant test assertions removed, `pinnedRef` in conversations panel renamed to `pinnedRowsRef`); Task 2 returns checkpoint for user real-browser verification post-ship.
...
### Task 2: Human-Verify (CHECKPOINT — awaiting orchestrator ship + user)

Task 2 is a `checkpoint:human-verify` gate. The orchestrator must ship the code to production
before user can walk the four bounty scenarios in a real browser. This plan does NOT attempt
to simulate or substitute for that verification.

**Bounties awaiting user's real-browser sign-off:**
...
user's verification protocol is in the plan's Task 2 `<how-to-verify>` block (8 numbered
...
**AWAITING ORCHESTRATOR SHIP + USER REAL-BROWSER VERIFICATION.**
...
3. Ask user to walk the 8 scenario groups (Task 2 `<how-to-verify>`)
