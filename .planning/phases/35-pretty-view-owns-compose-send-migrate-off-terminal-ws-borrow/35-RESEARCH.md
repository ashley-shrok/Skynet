...
- **patch #111:** bumped to 250ms ("50ms was too tight — user UAT'd... Ink stays in paste-detection past 50ms for long bodies")
...
a watchdog unless user asks for it.
...
| Split-send timing | Different delay | 250ms matching terminal.ts:842 | Empirically validated by user UAT (patches #111, #118) |
...
**What goes wrong:** Messages arrive in Claude Code's composer but don't submit. user's
