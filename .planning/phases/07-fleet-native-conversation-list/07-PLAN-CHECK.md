...
| TG-17 | Snapshot-on-load, no polling | ✓ (empty-dep useEffect + NOT wired to skynet:hosts-changed + grep gates on setInterval/setTimeout) | | ✓ (UAT — DevTools Network tab shows single request) | COVERED |
...
- ✅ **Snapshot on page-load, no polling.** Plan 07-01 hard_constraint #3 explicit: "NO polling. The fleet-discovery `getSessionList()` fires ONCE on mount (empty dep array useEffect). No setInterval, no setTimeout retry loop, no window focus/visibility listener, no periodic refetch. The `skynet:hosts-changed` event listener that already exists for realHostTree is NOT extended to re-fetch sessions." Grep gates enforce absence of `setInterval|setTimeout` near `getSessionList()` and NOT wired to `skynet:hosts-changed`.
- ✅ **No attached/detached visual distinction (TG-13).** Plan 07-01 hard_constraint #4 + must_haves.truths[1] + explicit fleetOnly is INTERNAL routing marker only. Plan 07-01 Task 1 Step 5: "user-facing: `fleetOnly` is INTERNAL — ConversationRow renders identically regardless (Plan 07-02 renders it identically visually)."
...
- ✅ **Deploy behind mandatory 15-min deadman.** Plan 07-03 Task 4 how-to-verify block references `~/.claude/identities/tina/deploy-runbook.md` explicitly, includes sentinel-cleanup-before-arm (`sudo rm -f /tmp/skynet-keep-patched`), the exact `nohup sudo -b bash -c 'sleep 900; [ ! -f /tmp/skynet-keep-patched ] && bash /opt/skynet/.tmp-revert.sh'` pattern, and the narrow-pkill disarm `sudo pkill -f 'sleep 900; \[ ! -f /tmp/skynet-keep-patched'`.
- ✅ **Deploy is user-gated.** Task 4 is `checkpoint:human-verify gate="blocking"`. Resume-signal enumerates all four states including explicit refusal path: `deploy without deadman → executor REFUSES — deadman is mandatory per CLAUDE.md, no exceptions; escalate`.
...
2. ✅ **No real-time polling / push mechanism on fleet-discovery input.** Plan 07-01 hard_constraint #3 + T-07-01-02 threat mitigation + explicit grep gates (`setInterval|setTimeout` count near `getSessionList` = 0; NOT wired to `skynet:hosts-changed`).
...
- ✅ **One-shot fetch on page-load, no polling anywhere.** Plan 07-01 Task 2 AppShell effect: `useEffect(() => { ...await getSessionList()...updateFleetSessions(...) }, []);` — empty dep array. Silent try/catch. Cancelled-guard for unmount race. Not wired into `skynet:hosts-changed` (explicit grep gate: `grep -B2 -A5 "skynet:hosts-changed" src/ui/AppShell.tsx | grep -c "getSessionList\|updateFleetSessions") -eq 0`).
...
|------|-------------------|---------|
| 07-01 | T-07-01-01 (dedup key collision), 02 (accidental polling regression), 03 (fleet-session list disclosure), 04 (malformed getSessionList response), 05 (detached-row-click bypass), 06 (race between hostsFlat update and fleet-row click), SC (supply chain) | T-07-01-02 is the load-bearing one for TG-17 — grep gates on `setInterval|setTimeout` + NOT wired to `skynet:hosts-changed` are the concrete defenses. Directly addresses shape-file "even free polling that manifests as visible list mutations violates the shape." |
...
| 07-03 | T-07-03-01 (bad build wedges Skynet container), 02 (deadman not armed before docker compose up), 03 (deploy without Alice's approval), 04 (deploy exposes Phase 7 code), SC | T-07-03-02 the critical one — sentinel-cleanup-before-arm step (`sudo rm -f /tmp/skynet-keep-patched`) is the belt-and-braces defense against a stale sentinel from a prior deploy neutralizing the new deadman. |
...
**Analysis:**
- ConversationsPanel gains an `onDetachedRowClick?` prop AND wires `handleRowSelect(row)` to route through it based on `row.fleetOnly`. Without the prop being wired at the AppShell mount site, `row.fleetOnly` rows would silently fall through to `selectConversation(row.id)` — which would then be rejected by the T-06-01-01 stale-guard (the fleet id `fleet::1::work` is not in openTabs), silently doing nothing when Alice clicks a detached row. This is invisible correctness rot — no error, no test failure at the ConversationsPanel level, just Alice's click doing nothing.
...
- ✅ **Mandatory 15-min deadman.** Explicit `nohup sudo -b bash -c 'sleep 900; [ ! -f /tmp/skynet-keep-patched ] && bash /opt/skynet/.tmp-revert.sh'` pattern. Wall-clock time noted.
- ✅ **Sentinel cleanup BEFORE deadman arm.** Step c: `sudo rm -f /tmp/skynet-keep-patched` — belt-and-braces defense per T-07-03-02.
- ✅ **Narrow pkill disarm.** Step h: `sudo pkill -f 'sleep 900; \[ ! -f /tmp/skynet-keep-patched'` — narrow pattern-match on the deadman-specific sleep loop, not a broad `pkill sleep`.
- ✅ **Check-before-recreate compose file safety.** Not explicit as a "check-before" step, but the plan does specify container health check (`docker compose ps skynet` → healthy) AFTER the recreate — the sentinel-cleanup-before-arm handles the state hygiene concern. Not a blocker.
- ✅ **user-gated resume-signal enumeration.** Task 4 resume-signal enumerates all four states: `approved`, `approved but I'll UAT tomorrow`, `wait, [issue]`, `deploy without deadman → REFUSE`. Matches the fork DEPLOY DISCIPLINE + CLAUDE.md 2026-07-03 lock.

**NOTE-D (non-blocking):** Task 4 step (a) says "Push the branch to the deploy remote (`git push origin feat/tab-title-from-tmux`) OR sync via the fork's existing deploy path (whichever is fresh in Alice's box-map.md)." The "OR" leaves the executor a choice; the deploy-runbook.md canonical path should be preferred (deployer must consult it at Task 4 execution time). Not a blocker; the checkpoint's blocking nature and Alice's oversight will catch any deviation.
...
**Analysis:**
- ✅ **Skynet-patches.md verified — patch #105 is the highest.** `grep -in "105\|106" ~/.claude/identities/tina/skynet-patches.md` shows patch #105 at line 7213 as the most recent entry (`feat(navigation): telegram-like interface — sidebar conversation list, tab strip removed, mobile list-vs-view flow, session persistence across switches — Phase 6, shipped 2026-07-21`).
- ✅ **Patches-md entry format follows patch #105 precedent.** Plan 07-03 Task 3 explicit: "structured for post-UAT paste into `~/.claude/identities/tina/skynet-patches.md`" with the sections What-it-does / motivating-gap / changed-files / constraints-preserved / deploy / follow-ups. Multi-commit format under one PIN (matches patch #105's "Multi-commit patch (9 code commits landing across 4 waves + 1 verify wave under this one patch number)" precedent).
...
Phase 7 plans are ready for Alice's execution green-light.

## Action items (optional, non-blocking)

If Alice wants the planner to strengthen the plans before executing, here are the small revisions worth batching:
