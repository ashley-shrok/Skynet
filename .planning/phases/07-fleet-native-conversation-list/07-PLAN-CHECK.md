...
| 07-03 | T-07-03-01 (bad build wedges Skynet container), 02 (deadman not armed before docker compose up), 03 (deploy without Ashley's approval), 04 (deploy exposes Phase 7 code), SC | T-07-03-02 the critical one — sentinel-cleanup-before-arm step (`sudo rm -f /tmp/skynet-keep-patched`) is the belt-and-braces defense against a stale sentinel from a prior deploy neutralizing the new deadman. |
