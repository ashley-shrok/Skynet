...
| 07-03 | T-07-03-01 (bad build wedges Skynet container), 02 (deadman not armed before docker compose up), 03 (deploy without user's approval), 04 (deploy exposes Phase 7 code), SC | T-07-03-02 the critical one — sentinel-cleanup-before-arm step (`sudo rm -f /tmp/skynet-keep-patched`) is the belt-and-braces defense against a stale sentinel from a prior deploy neutralizing the new deadman. |
...
**Analysis:**
- ConversationsPanel gains an `onDetachedRowClick?` prop AND wires `handleRowSelect(row)` to route through it based on `row.fleetOnly`. Without the prop being wired at the AppShell mount site, `row.fleetOnly` rows would silently fall through to `selectConversation(row.id)` — which would then be rejected by the T-06-01-01 stale-guard (the fleet id `fleet::1::work` is not in openTabs), silently doing nothing when user clicks a detached row. This is invisible correctness rot — no error, no test failure at the ConversationsPanel level, just user's click doing nothing.
...
**NOTE-D (non-blocking):** Task 4 step (a) says "Push the branch to the deploy remote (`git push origin feat/tab-title-from-tmux`) OR sync via the fork's existing deploy path (whichever is fresh in user's box-map.md)." The "OR" leaves the executor a choice; the deploy-runbook.md canonical path should be preferred (deployer must consult it at Task 4 execution time). Not a blocker; the checkpoint's blocking nature and user's oversight will catch any deviation.
...
Phase 7 plans are ready for user's execution green-light.

## Action items (optional, non-blocking)

If user wants the planner to strengthen the plans before executing, here are the small revisions worth batching:
