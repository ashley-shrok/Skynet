...
# Plan 07-03's UAT walk on user's browser after the deploy. Mirrors
...
- **T-07-02-02 (host name disclosure):** accepted — same host names user already sees in HostsPanel, SessionsPanel, SidebarTree, double-shift menu. No new disclosure.
- **T-07-02-03 (many RDP hosts flooding the list):** accepted — user's fleet scale (~20 hosts) is well within a handful of RDP rows at the bottom. No virtualization needed.
...
- **T-07-02-05 (mobile viewport race with useIsTouchDevice):** mitigated — the hook is deterministic within a page-load (media query detection). user resizing on desktop across breakpoints causes the gear to vanish when useIsTouchDevice flips true — acceptable behavior matching Plan 06-03's mobile flow gate on the same signal. No SSR concern (Skynet is client-only React).
...
- With user's fleet containing at least one RDP-enabled host, the conversation list shows a row for that host at the BOTTOM of the ConversationsPanel scroller with:
...
- **Toggling `enableRdp` off** on a host via HostEditor + refresh → RDP row VANISHES. **Toggling back on** + refresh → RDP row RETURNS. (Note: NO auto-update per TG-17 shape lock — user refreshes.)
...
- Deploy at user's discretion — the visible UAT surface is now complete (fleet-native list + RDP rows + pencil + gear-dedup). user can UAT the DEV/staging build first if desired.
...
- Patch #106 pins after user's UAT sign-off + `/close telegram-like-interface` bounty closure.
