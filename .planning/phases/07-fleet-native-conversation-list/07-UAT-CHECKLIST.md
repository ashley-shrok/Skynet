...
- [ ] 🚨 **TG-12 mobile fresh page-load** Open Skynet in fresh incognito on phone (or DevTools with `pointer: coarse` + narrow viewport). Same expectation: full-screen ConversationsPanel shows running tmux sessions. **This is THE gap patch #105 shipped with** — Phase 6 UAT found "no active conversations" on a fresh mobile page-load even when user had active sessions on hosts. Phase 7 fix must show them.
