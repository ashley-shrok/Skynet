...
- **T-06-02-01 mount-lifecycle contract preserved byte-for-byte.** `createPortal(` count in AppShell.tsx still exactly 1; MountManager scaffold Tests 1-3 in AppShell.persistence.test.tsx unchanged; tabNodesRef DOM-move mechanism unchanged; the SAME `state.selectedId` still drives `effectiveSelectedTabId` (a detached row that becomes attached uses the same visibility signal path user expects).
...
- **One-shot fetch, empty dep array, silent try/catch, cancelled-guard.** TG-17 hard shape lock enforced. NOT wired to `skynet:hosts-changed`. Cross-device / cross-session staleness deliberately acceptable per user's confirmation in the shape file.
...
- **T-07-01-03 (fleet-session list disclosure):** accepted — same list user already sees in SessionsPanel; no new disclosure surface.
