...
**Paste timing:** Only after user greenlights the batched Phase 11 + Phase 12+ purge cluster deploy AND UAT passes on the 22 non-negotiable items in `11-UAT-CHECKLIST.md`. Post-deadman-retirement flow per current `~/.claude/identities/tina/deploy-runbook.md` (the 15-min deadman regime was retired 2026-07-21).
**Batch context:** Patch #138 is the FIRST Phase 11 patch. Per the fleet-standing "batch patches into meaningful deploys" rule (user 2026-07-23), it does NOT ship standalone. Batches with subsequent Phase 12+ purge patches (dashboard/panel-file deletion, backend-route deletion, dead-locale-string sweep) into a single grouped-semantic-unit deploy ("the visible-surface purge cluster") unless user explicitly greenlights an early Phase 11 standalone.
...
## Draft (paste-ready — matches the multi-commit-under-one-pin convention from patches #104, #105, and #128)
   ...
   138. `feat(app-shell,sidebar): patch #138 — Skynet transformation: purge dead
        ...
        with subsequent Phase 12+ purge patches per user 2026-07-23 fleet-
        standing "batch patches into meaningful deploys" rule; not deployed
        standalone unless user explicitly greenlights).

        * **Motivating gap** (user's direct call-out, Phase 10 UAT 2026-07-23):
          "I really feel like we need to get away from this skynet front end
          stuff before any of this is worth quibbling over." Long-term user
        ...
        * **Fix summary — landing swap** (PURGE-01). Replaced the desktop
          ...
          disclosure guardrails per user's motion-quiet lock. The
        ...
        * **Fix summary — SettingsRow retirement** (user's total-not-partial
          ...
          prop dropped in Phase 11 (user's "no settings" lock)`. Test count:
        ...
        * **Bundle-size headline — Ship-of-Theseus purge landed in bytes.**
          ...
          user's Skynet client now downloads 373 kB less code on first-load
        ...
        * **Files touched** (all on `feat/tab-title-from-tmux`):
          ...
          - Deleted:
            ...
            - `src/ui/sidebar/SettingsRow.tsx` (198 LOC — user's "no
              settings" lock)
        ...
        * **Design source-of-truth** (LOCKED, no re-litigation per CONTEXT.md):
          ...
          - `~/.claude/identities/tina/tina.md` § Skynet direction — Ship
            ...
            "conversation list + pretty view is all user sees" heuristic,
          ...
          - `~/.claude/identities/tina/bounties/skynet-transformation-purge-dead-surfaces/`
            — the bounty premise, user's UAT quote, the todo set
        ...
        * **Commits** (all on `feat/tab-title-from-tmux`):
          ...
          - **Plan 03 (retirement — post B-3 split into 5 atomic tasks)**:
            ...
            - `c3c84be` `chore(sidebar): delete retired SettingsRow.tsx
              (Phase 11 PURGE-03 — user's "no settings" lock)`
              (198 lines deleted)
        ...
        * **Deploy status**. Code-complete on `feat/tab-title-from-tmux` at
          ...
          12+ purge patches per user 2026-07-23 fleet-standing "batch
