...
- Fixed Bug 1 of bounty `claude-code-2-1-214-pretty-view-compat`: modern (v2.1.150+) async `Agent` invocations now show up in the BG-agents panel (user's original observation of taylor's `gsd-executor` sub-agent's Task tool_use not rendering will resolve on next deploy).
...
- **Rendering sub-agent CONVERSATION content in pretty view** (user's follow-on interest) also remains deferred — requires reading `subagents/agent-*.jsonl` files, distinct code path from the panel admission fix.
