...
CSS keys off `.pv-row.spinner-on .pv-avatar::before` at pretty-conversations.css line 557 — single class match, NO `:is(.working, .recycling)` narrowing, NO `.active-set` scoping on the CSS side. All 4 inputs live in JS, CSS matches on the single className the JS gate emits. user's verbatim inversion rule is intact.
...
Both tests pass in the run below. These guard against regression to the pre-revision CSS-only gate `.pv-row.active-set:is(.working, .recycling)` that dropped 2 of user's 4 inputs.
