...
- No level meter, waveform, or recording-time counter (user explicitly declined — the three-button record state IS the recording indicator).
- No swipe-to-cancel gesture (user chose a plain cancel BUTTON).
...
### Locked design pins (user 2026-07-27, prototype UAT)
...
- iOS Safari behavior is not unit-testable — flag as a manual-verification step in the plan's verification section (user already verified on her PWA in the prototype UAT).
...
- **Update `~/.claude/identities/tina/skynet-patches.md` in the same commits as the code** — new fleet directive from user 2026-07-27: docs/catalog updates happen inline with work, never queued, never awaiting greenlight. The planner should include a task at the end of each PLAN.md to update this catalog file.
...
- `~/.claude/identities/tina/bounties/add-voice-input/prototype.html` — 340-line self-contained HTML with the exact button state machine, MediaRecorder wiring, and fetch flow that user UAT-passed. Planner should MIRROR the state machine and error-handling shape here. In particular the `startRecording` / `stopRecording` / `transcribe` functions and the `setState('idle'|'recording'|'transcribing', msg)` pattern.
...
- **Waveform / level meter / recording timer** — user explicitly declined.
- **Swipe-to-cancel gesture** — user chose a plain cancel BUTTON.
