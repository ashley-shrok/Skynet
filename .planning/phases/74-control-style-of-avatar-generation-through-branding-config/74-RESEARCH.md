 ...
 * log + process.exit(1) so the container's supervisor restarts and user
...
**How to avoid:** Plan phase should draft the actual seed text at planning time (not defer to execution). Show user/discuss-phase the exact string. The seed's palette instruction should read like:
...
1. **Where exactly does the aesthetic palette prose land inside the seeded director spec?**
   ...
   - Recommendation: The seed is the current `ARCHETYPE_SYSTEM_PROMPT` text with an inline "PALETTE INSTRUCTION:" paragraph inserted near the palette bullet, phrased so the mechanical hue fact from `paletteHueLine()` at request time makes sense as its concrete instance. user should eyeball the drafted seed at plan-phase discussion.
