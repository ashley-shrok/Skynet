...
decisions:
  ...
  - "Gate does NOT read avatarGammaDefault at all — per user resolution #5 (74-CONTEXT.md), gamma is optional-with-code-default. The loader's shape guard already enforces `typeof number + isFinite`; the request-time consumer trusts whatever finite value it reads. Wrapping gamma in the boot gate would add zero safety and would violate the plan's Rule-4 architectural line"
...
| 5 | Structured log fires with `operation: "branding_config_boot_gate"` | Structured error surface — user sees WHY on failed boot |
| 6 | Does NOT gate on `avatarGammaDefault` | user resolution #5 — includes sane (0.7) and nonsense-but-finite (-999) gamma sub-cases; both pass |
...
- AFTER `initializeDatabase()` so the DB-backed logger stream is live to carry the fatal log line out to user.
...
- `assert-boot.ts` contains ZERO `avatarGammaDefault` references — the gate reads the spec field ONLY. user resolution #5 is enforced by absence, not by comment.
