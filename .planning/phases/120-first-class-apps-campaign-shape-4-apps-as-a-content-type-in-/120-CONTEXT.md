...
- The exact resolution of D-11 (path-prefix + app absolute-URL handling). Options ranked: (i) response-body HTML `<base>` tag injection at the proxy, (ii) full response-body URL rewriting, (iii) app-side relative-base config in the starter template (SCOPE AMENDMENT — needs the user greenlight). Researcher must recommend after investigating the SvelteKit runtime path handling + `http-proxy-middleware` response transforms.
...
- `~/fleet/roles/box-maintainer/box-maintainer.md` § "Standing directives" — container-mutation serialization (the user 2026-09-12), test discipline scoped-during-dev (the user 2026-09-07), deploy-boundary-at-push (the user 2026-08-29), no-worktrees (the user 2026-07-31), no message streaming ever (the user 2026-08-29). Planner + executor must honour these.
...
- **Container-mutation serialization** (the user 2026-09-12) — applies to the eventual deploy motion at campaign close, NOT to Phase 120's planning or executor phases.
