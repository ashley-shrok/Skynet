...
- First browser smoke by user: reported "no active Claude session"
...
Redeployed via same build+deadman flow. user's second browser smoke
...
- Old deadman kept inert by touching sentinel; v2 deadman armed with
  distinct `/tmp/skynet-keep-patched-v2` sentinel to give user a
...
- Also validated end-to-end in production:
  ...
  - **BACKEND-03** — tail streaming works (user saw the conversation
    render)
...
- **Executor swap:** the plan block for Task 3 was written for a
  ...
  bypass GSD. user enforced this mid-run. Tasks 1 + 2 were also handled
