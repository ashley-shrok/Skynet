...
- **The `/rooms/{id}` endpoint DOES also return `joined_local_members` count** (CONTEXT § 3.4 suggested it as an alternative). We chose `/rooms/{id}/members` instead because:
  ...
  2. For user + agent DM rooms specifically (single homeserver, no federation from thenasty), the two counts would be identical. But defensive programming: use the endpoint that isn't semantically ambiguous.
