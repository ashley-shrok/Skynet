...
**Why this matters:** The Chatterbox demo Nelly showed used `Adrian.wav`; the plan explicitly locked `Elena.wav` as the Skynet default (user: "we're not gonna switch our default voice just because nelly picked a random one to do the demo with"). The backend's `predefined_voice_id: voice ?? "Elena.wav"` fallback should produce Elena on any message where no identity voice override is set.
...
**What to do:**
1. On user's iPhone PWA, repeat Item 1 (streaming latency on a long message).
