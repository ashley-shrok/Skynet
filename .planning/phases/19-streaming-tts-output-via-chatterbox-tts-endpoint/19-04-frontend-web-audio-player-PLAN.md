...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    3. Instantiate `audioContext = new AudioContext();` (or `new (window.AudioContext || (window as any).webkitAudioContext)()` for iOS Safari — patch #211 lesson; but modern iOS 14.5+ Safari has standard AudioContext, and the CONTEXT.md iOS spike already passed on user's iPhone PWA, so plain `new AudioContext()` is sufficient unless typecheck complains).
