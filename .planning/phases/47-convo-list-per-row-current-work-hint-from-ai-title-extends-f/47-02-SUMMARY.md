...
requires:
  - Phase 47 Plan 01 (wire-type surface — SessionState + RemoteTmuxSession + FleetSession all carry optional nullable aiTitle; consumed as-is here)
...
provides:
  ...
  - "New scanTailForLatestAiTitle helper in BOTH backend files (hand-mirrored per 47-CONTEXT.md 'no new shared module' scope decision inherited from Phase 43/44)"
...
affects:
  ...
  - src/backend/database/routes/sessions.test.ts (jsonlAiTitleLine fixture + 7 new tests in Phase 47 Plan 02 describe + aiTitle assertions on 10 pre-existing tests)
  ...
  - src/backend/fleet-status/ssh-poll-orchestrator.test.ts (jsonlAiTitleLine fixture + 6 new tests in Phase 47 Plan 02 describe + tail-n-200 → tail-c-262144 in 5 assertions + aiTitle assertions on pre-existing Test G/I/J/K)
tech-stack:
  added: []
  patterns:
...
**`src/backend/database/routes/sessions.test.ts`:**
- New fixture helper `jsonlAiTitleLine(sessionId: string, aiTitle: string): string` emits the `{"type":"ai-title","aiTitle":"…","sessionId":"…"}` line shape (47-CONTEXT.md § Backend scraper mechanics).
- New `describe("GET /sessions/list — aiTitle derivation (Phase 47 Plan 02)", ...)` block with 7 tests:
...
- In `processPid`:
  - Tail command bumped from `tail -n 200 ${jsonlPath} ...` to `tail -c 262144 ${jsonlPath} ...`. Docblock at the swap site cites 47-CONTEXT.md § Backend scraper mechanics.
...
- `SessionState` composition gained `aiTitle: derivedAiTitle,` after `lastMessageAt: derivedLastMessageAt,`. Comment above cites Phase 47 Plan 02 + last-wins semantics.
...
- New `describe("Phase 47 Plan 02 — aiTitle derivation and publish", ...)` block with 6 tests:
...
- `npx vitest run src/backend/database/routes/sessions.test.ts` — **17/17 pass** (10 pre-existing + 7 new Phase 47 Plan 02 tests).
- `npx vitest run src/backend/fleet-status/ssh-poll-orchestrator.test.ts` — **32/32 pass** (26 pre-existing (5 updated for tail-c-262144 + 3 for aiTitle-null) + 6 new Phase 47 Plan 02 tests).
...
| `grep -c 'describe.*Phase 47 Plan 02' src/backend/database/routes/sessions.test.ts` | >= 1 | 1 ✓ |
...
| `grep -c 'describe.*Phase 47 Plan 02' src/backend/fleet-status/ssh-poll-orchestrator.test.ts` | >= 1 | 1 ✓ |
...
1. **Old `tail -n 200` references in docblocks scrubbed** — three comments in ssh-poll-orchestrator.ts still referenced the historical shape in a purely descriptive context. The acceptance criterion `grep -Fc 'tail -n 200' == 0` was strict, so all three comments were rephrased to reference the new `tail -c 262144` shape (e.g. "Phase 47 Plan 02: `tail -c 262144`" instead of "`tail -n 200` bumped to `tail -c 262144`"). No semantic change, just satisfies the grep spec strictly.
...
| Task | Hash | Message |
|---|---|---|
| 1 | `041b7d27` | `feat(47-02): extend /sessions/list route with per-session aiTitle derivation` |
| 2 | `9f792710` | `feat(47-02): publish aiTitle on every SessionState frame via ssh-poll-orchestrator` |

## Known Stubs

None. Both backend read paths now fully derive `aiTitle` end-to-end — the /sessions/list REST response carries it per row, and the fleet-status WS `SessionState` frame carries it per publish. The wire types accepted the field via Phase 47 Plan 01 (already landed). Downstream consumer wiring lives in Plans 47-03 (working-store third axis) and 47-04 (AppShell seed).
...
- **Plan 47-03** (working-store third axis): can consume `SessionState.aiTitle` from the fleet-status WS payload with confidence that it's derived symmetrically from the same JSONL as `lastMessageAt`.
- **Plan 47-04** (AppShell seed): can consume `RemoteTmuxSession.aiTitle` from `/sessions/list` response with confidence that every row emits the field (server-always-emits contract).
- **Plan 47-05** (row redesign): downstream — consumes the working-store hook exposed by Plan 47-03.
...
Per-task git-log gate sequence (each task combines TDD RED+GREEN in one commit per the plan's `tdd="true"` scope, matching the Phase 47 Plan 01 pattern):
- Task 1 commit `041b7d27`: `feat(47-02): extend /sessions/list route with per-session aiTitle derivation`.
- Task 2 commit `9f792710`: `feat(47-02): publish aiTitle on every SessionState frame via ssh-poll-orchestrator`.
...
- **Files present:**
  ...
  - `src/backend/database/routes/sessions.test.ts` — modified, present (jsonlAiTitleLine fixture; 7 new tests in Phase 47 Plan 02 describe; aiTitle assertions on 10 pre-existing tests).
  ...
  - `src/backend/fleet-status/ssh-poll-orchestrator.test.ts` — modified, present (jsonlAiTitleLine fixture; 6 new tests in Phase 47 Plan 02 describe; 5 tail-n-200 → tail-c-262144 updates; aiTitle assertions on pre-existing Tests G/I/J/K).
  - `.planning/phases/47-convo-list-per-row-current-work-hint-from-ai-title-extends-f/47-02-SUMMARY.md` — created, present.
...
- **Scope fence honored:** only 4 files modified (matches plan's `files_modified` list exactly). No edits to wire-protocol.ts, fleet-status-types.ts, conversation-store.ts, session-working-store.ts, AppShell.tsx, or any pretty-conversations file — Plan 47-01 owns the wire types; Plans 47-03 through 47-05 own downstream consumer wiring.
