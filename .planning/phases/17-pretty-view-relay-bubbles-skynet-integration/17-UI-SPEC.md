...
user's 2026-07-23 lock on pretty-view interior applies: the chat surface
...
- New settings, new preferences, new toggles for relay bubble on/off — the
  detection is always-on, invisible when nothing matches, per user's
  no-settings rule.
...
- **Design lock**: user UAT'd 6/6 in the prototype 2026-07-28. Do not re-litigate the shape.
...
- **Post-ship**: expect one round of "make it look nicer" polish after user sees it in-flow (per prior pretty-view phase patterns — the ugly-but-reliable intermediate state was explicitly greenlit in the prototype bounty premise).
