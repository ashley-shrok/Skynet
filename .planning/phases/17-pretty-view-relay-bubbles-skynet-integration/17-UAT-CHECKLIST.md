...
**Design source-of-truth:** `~/.claude/identities/tina/bounties/pretty-view-relay-bubble-prototype/prototype.html` (6/6 acceptance battery passed with user 2026-07-28) + `.planning/phases/17-pretty-view-relay-bubbles-skynet-integration/17-UI-SPEC.md` (LOCKED).
...
2. Have a fleet-connected tmux session with **a real Matrix relay send ready** — either a `curl -X PUT` to a room pre-staged in history, or trigger one live (user's standard `relay-send.sh` or equivalent).
...
5. Have the user identity's `colorHue` value handy for RELAYBUB-03 (check `~/.claude/identities/user/identity.json` or the Skynet identity registry).

---

## Item 1 — RELAYBUB-01: Outbound blue bubble

**Action:** On a tina-identity pretty-view tab, trigger a real Matrix relay send (user's `curl -X PUT` to a room, the same 3-way command format the prototype detects).
...
- Body shows the extracted message text (e.g. "Hey @user, the deploy is green")
...
- Header line reads `● user · !room-alias:server` (or the room ID — the dot is the identity avatar-dot)
...
**Expected:**
- The avatar-dot is rendered in **user's identity colorHue** (warm orange/amber — whatever `colorHue` is set to in the user identity registry, typically in the 20-40 hsl-degree range)
- It is NOT the neutral grey fallback (`hsl(210, 8%, 50%)`)
- Cross-check: if the bubble sender's mxid resolves to the user identity in the registry, the hue should match `hsl(colorHue, 70%, 65%)` or similar per the hue-chain convention
...
**Action:** Ask user to send a long message (>500 characters, or whatever the recv.sh threshold is for writing to a temp file instead of inline body) from Element. The recv.sh event body will read `body written to /tmp/relay-msg-XXXX.txt` instead of inline text.
