...
**Paste timing:** Only after user greenlights the Phase 17 deploy AND UAT passes on all
...
## Draft (paste-ready)
   ...
   169. `feat(pretty-view,backend): patch #169 — pretty-view relay bubbles
        ...
        * **Motivating gap** (user 2026-07-28 design session, prototype
          validated 6/6 in acceptance battery):
          user's fleet identity sessions frequently interleave with Matrix
          ...
          work, buried in the conversation stream. user wanted them
          ...
          acceptance battery passed with user 2026-07-28 — outbound blue,
        ...
        * **Requirements closed.** RELAYBUB-01 (outbound blue bubble),
          ...
          on normal pretty-view surfaces). All six closed per user's
        ...
        * **15-min deadman deploy discipline observed.** Per fork DEPLOY
          ...
          skynet` runs behind user's 15-min deadman rollback timer, no
          exceptions. Claude does not execute the deploy. user's
        ...
        * **Bounty reference.** `~/.claude/identities/tina/bounties/
          pretty-view-relay-bubble-prototype/` — closes on user UAT
