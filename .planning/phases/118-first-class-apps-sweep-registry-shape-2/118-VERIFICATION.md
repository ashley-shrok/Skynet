...
| D-03 | Backend authors the healthMessage ("not running — ask an agent to check on it") | VERIFIED | `fleet-status-sweep.py:1229` literal string matches. Shell case 4 asserts the exact string round-trips through the JSONL. Register matches the user's steer. |
...
**Status:** Standard fleet rule. Coordinate with the user before any `docker compose up -d --force-recreate skynet`.
