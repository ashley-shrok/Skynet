...
### Shape + R&D
- `.planning/shapes/shape-skynet-passthrough-urls.md` — the two-phase shape agreed with user 2026-09-05. Governs philosophy (agents-aren't-lied-to, Skynet-as-delivery-surface, no-writes-from-Skynet), scope edges, failure modes. Applies to both phase 75 (file) and the phase-2 serve URL work.
...
- URL grammar reads like GitHub file URLs (`github.com/owner/repo/blob/main/src/foo.ts`) — same "literal path segments" feel user reaches for naturally.
