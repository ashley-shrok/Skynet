...
A bubble with a muted red border (`hsla(0, 60%, 55%, 0.4)`) and a barely-visible reddish tint (`hsla(0, 40%, 50%, 0.08)`) over the standard blue-gray gradient. user confirmed this looks like "just a red border."
...
1. In the `arm` log (line 1240), add `pane_state=${paneState ?? 'null'} dormant_signal=${dormant}` — this lets user (and any post-deploy analysis) see BOTH signal values at arm time, distinguishing which one drove the authoritative result.
