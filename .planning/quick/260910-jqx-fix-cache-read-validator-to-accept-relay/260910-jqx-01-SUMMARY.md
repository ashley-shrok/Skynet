...
Kind-aware `isFleetSession` validator so relay-room cache rows survive `readFleetSessionsCache()` — restores instant paint of relay rooms on cold-boot instead of user's observed ~10s wait for the `/sessions/list` round-trip.
...
Every relay-row cache entry hit that gate on read and got silently filtered out. `readFleetSessionsCache()` returned only the harness rows. On cold-boot, AppShell painted the harness rooms instantly from cache but had to wait for the fresh `/sessions/list` fetch (~10s under user's load) before relay rooms materialized in the sidebar.
...
- Ship gate: this fix belongs with the broader UAT / Phase 97 polish arc currently held for greenlight. user owns the push decision.
