...
user eyeballed the deployed pretty-view (tina's session) and confirmed the relay bubbles read inverted from the surrounding chat convention:

- Regular `ChatMessage.tsx` uses `isUser ? justify-end : justify-start` — user (user's compose-box input) = RIGHT, assistant (agent's own reply) = LEFT.
...
user wants relay bubbles to read like the rest of the chat — same "who's speaking → which side, which color" convention. Small attribution headers/footers (`▸ relay send → {room}` / `via curl`; avatar-dot + `{name} · {room}` / `via recv.sh`) STAY per user's A-vs-B answer — they preserve the "this went through Matrix, not native Skynet" semantic without needing a color difference to carry that signal.
...
### 2. `RelayInboundBubble.tsx` (Matrix send from user → blue-gray, RIGHT)
...
- No deploy — patch batches on `feat/tab-title-from-tmux` with #198+#199, waits for user's ship greenlight (fleet rule).
