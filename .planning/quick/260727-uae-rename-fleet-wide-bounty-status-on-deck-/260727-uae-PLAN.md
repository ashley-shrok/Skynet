...
- Keep `status: "waiting_on_someone_else"` — the bounty is not done (it's blocked on user's deploy authorization for the tb1 badge itself), and this rename doesn't change that status axis. Do NOT flip it to `pinned` or `done` unless explicitly told to.
...
Nothing is pushed. Nothing is docker-built for deploy. The tree is on the branch, green, and awaiting user's explicit deploy authorization.
</verification>
