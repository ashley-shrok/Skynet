...
- `bounty.json`:
  ...
  - `premise`: **unchanged** (historical record of the original design conversation; the `on-deck` phrasing there is what user said at the time)
...
- **Commit:** none.
- **Follow-up:** if user wants a scripted split, adding `"test:backend": "vitest run src/backend"` + `"test:frontend": "vitest run src/ui"` would take one line each in `package.json` — flagged for a separate cleanup patch if desired, not scoped here.
