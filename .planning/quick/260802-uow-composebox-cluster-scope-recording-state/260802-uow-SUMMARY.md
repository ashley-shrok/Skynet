...
decisions:
  ...
  - "MicButton disabled prop uses `voice.state !== 'idle'` (blocks second concurrent recording) instead of the narrower `micTarget !== <this-source>` — matches user's verbatim intent ('mic stays visible but disabled')."
  ...
  - "Row 1 bottom margin uses the arbitrary bracket `mb-[3px]` (not a Tailwind numeric class) to preserve user's DevTools-measured 3px exactly."
metrics:
  duration_minutes: ~10
...
- **Primary block (~L1246-1272):**
  ...
  - `showPrimaryArmButton` — removed the `voice.state === "idle"` gate entirely. Send-when-idle is orthogonal to recording elsewhere (user: valid workflow).
...
- **Row 1 container (~L1406):** Added `mb-[3px]` (arbitrary bracket — not a Tailwind numeric class, per user's DevTools-measured 3px). Combined with the outer `gap-1` (4px), total gap Row 1 → next block = 7px, regardless of whether the queued-slot stack is present.
...
- [x] All 4 bounties resolved per user's verbatim descriptions.
