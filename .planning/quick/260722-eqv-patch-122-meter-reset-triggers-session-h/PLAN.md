...
description: >
  ...
  holding, and turn the overlay's bubble RED after a 120s timeout as user's
...
- Add `isHolding?: boolean` in the props block. Slot near `canSend`
  ...
  unlit state (well glow, border, and background stay intact). user
...
- Add:
  ...
  // idempotent — no double-fire. If user clicks reset a second
...
- Change to:
  ...
  case "inactive": {
    ...
    // user sees the failure explicitly, not drop back to the
