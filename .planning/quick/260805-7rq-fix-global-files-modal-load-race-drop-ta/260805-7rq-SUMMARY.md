...
Deploy NOT run — awaiting user greenlight. No `docker build`, no `docker compose up`, no `git push`. Backend build intentionally skipped (frontend-only change per plan explicit non-goal).
