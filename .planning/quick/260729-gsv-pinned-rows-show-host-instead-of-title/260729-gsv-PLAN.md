...
<success_criteria>
Pinned rows visually match the grouped block's sublabel treatment: they render `identity.title ?? identity.displayName` when identity resolves, and cleanly fall back to Server + hostname when it doesn't. The full pretty-conversations vitest folder + `tsc --noEmit` + `npm run build` all pass. Strict scope maintained: only the three files listed in `files_modified` were touched, and no adjacent behaviors regressed. A single patch-#184 commit lands on `feat/tab-title-from-tmux`, ready for user's deploy greenlight.
</success_criteria>
