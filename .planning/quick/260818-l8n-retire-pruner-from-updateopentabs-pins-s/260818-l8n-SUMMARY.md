...
One-liner: Deleted the pin pruner block from `updateOpenTabs` in `src/ui/state/conversation-store.ts` — pins are now sticky across openTabs / fleetSessions churn, fixing user's deploy-race where WS-reconnect transients nuked legitimate pins that then got persisted server-side on her next pin/unpin.
...
- Test 5b/5c describe renamed from
  ...
  user's deploy-race (openTab pin survives when its tab leaves the
