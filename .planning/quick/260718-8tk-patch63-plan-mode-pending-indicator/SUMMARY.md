...
waiting on user's `1` (accept) / `2` (keep planning) reply in Plan
...
It stays unmatched until user replies — at which point a subsequent
...
- **Plan body rendering.** user's explicit ask was
...
- **Auto-accept/decline buttons.** Plan Mode replies belong to
  user + Claude Code; pretty view is the observer surface.
...
**Deploy pending user's explicit go-ahead.** Per fork DEPLOY
...
(post-deploy, after user confirms the deploy holds), not at commit.
