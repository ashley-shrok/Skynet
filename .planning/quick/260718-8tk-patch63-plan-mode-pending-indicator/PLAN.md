...
user replies — at which point a subsequent user turn's `content[]`
...
`2`" contract. NO plan content is displayed (user's explicit ask —
...
#60 (already holds #61 and #62). Deploy happens later when user
...
`let stopped = false;`). Insert this block:
    ...
    // ExitPlanMode tool_use blocks (Claude asking user to accept /
...
Full contents:
    ...
    // object. Unmounted when the session returns pending: null (user
    ...
    // either (Plan Mode is between user and Claude Code; the pretty
...
(line 104), insert a new state hook with a matching comment style:
    ...
    // tracked but not displayed (Plan Mode is between user and
...
5. **Do NOT deploy.** Per user's DEPLOY DISCIPLINE (tina.md):
