...
decisions:
  - Splitting Case 2 into two effects (new-message signal keyed on messageCount + pill-only RO) is the structural fix user greenlit over a narrow MEASUREMENT_DELTA_IGNORE_PX threshold-bump — it eliminates the conflation at the source rather than papering over the delta filter.
...
- **Pre-fix hook**: mount at scrollTop=5000, sticky=true; simulate tall-bubble re-measure by bumping scrollHeight to 5800 and firing RO callbacks; RO callback checks `stickyRef.current` (true) and `!shrunk` (5800 > 5000) → calls `jumpToBottom(scrollEl)` → scrollTop yanks to 5800 → **test FAILS** (matches user's report).
