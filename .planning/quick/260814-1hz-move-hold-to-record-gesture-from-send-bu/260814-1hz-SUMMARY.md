...
Deferred to user's next mobile session (per plan step 6): hold the mic button on iOS Safari, confirm the mic-permission prompt fires and the record → release-inside-bounds → send flow works.
...
Remaining validation: manual on-device iOS Safari smoke test (deferred to user's next mobile session).
