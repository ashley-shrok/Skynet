...
decisions:
  - "Debounce delay is 60_000ms (60s) per user proposal — gives in-flight nav-away work time to finish"
...
Debounced `ws.close()` at both hidden-pane pause-effect sites (Terminal.tsx and PrettyView.tsx) with a 60s timer, cancellation on isVisible=true, and unmount cleanup. Fixes user's voice-record + send + immediate nav-away regression.
