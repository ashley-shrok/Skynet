...
- The comment header at L1183-1191 (`Patch #149 B+C: active-set rows overtake pinned per user 2026-07-24. ...` through the `subtitleMode="identityTitle"` block-comment) — replace with a one-line comment: `// Phase 42 UAT amendment 2026-08-17 (user verbatim): active-set top zone retired — active-set rows now flow through to pinned (if pinned) or middle (by recency).`
...
**ORCHESTRATOR-ONLY EXCLUSIONS (do NOT do in this task, per user 2026-08-08 fleet rule):** no `git push`, no `docker build`, no `docker compose up`, no coord-room posts, no skynet-patches.md entry, no fast-path vite-watch launch. This task ends at "code changed, tests green, builds green, ready for commit." The orchestrator picks up the deploy motion.
...
9. **Human verify (post-deploy, orchestrator's problem — not this plan's):** user loads the app, confirms active sessions no longer surface above pinned area and the "Pinned" section header no longer renders.
</verification>
