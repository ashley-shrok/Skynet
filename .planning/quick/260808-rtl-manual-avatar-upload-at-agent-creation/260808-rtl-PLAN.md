...
Purpose: user wants to hand-pick avatars for some identities without waiting for the LLM+gpt-image-1 pipeline. Reuses the existing candidate cache + auth + scope guard so the birth/clone contracts don't need touching.
