...
- **Aesthetic parity by verbatim token copying** — ImageBubble does not import ChatMessage's className builder or share styling helpers. The gradient/border/shadow tokens are literally copied so any future divergence (e.g. user tunes ChatMessage's saturation) is a deliberate choice, not accidental drift. Justified by the aesthetic-parity constraint being a design contract, not a code-DRY concern.
...
- Live-run hand-verify is deferred to user's next deploy (per constraints: no build/docker/deploy for this patch; user is still stacking patches on `feat/tab-title-from-tmux`).
...
- Aesthetic acceptance criterion is structural: ImageBubble's gradient/border/shadow tokens are byte-identical to ChatMessage's assistant branch. user's per-pane identity hue will color-shift the bubble automatically via `hsla(var(--pv-id-hue),...)`.
