...
Purpose: When Claude reads a PNG (avatar review, screenshot, etc.) the tool_result currently drops silently at the parser (RENDER-01 hard-lock, "aggressive minimalism"). user (in pretty view) then can't see the image the agent just read. This patch lifts that restriction FOR IMAGES ONLY — non-image tool_results still drop as before.
...
<task type="auto" tdd="false">
  ...
  <action>
    ...
    2. Aesthetic rule (per scope_detail user 2026-07-19 design read): ALWAYS use the assistant identity-hue treatment, regardless of `role`. Semantically the Read was invoked by the assistant so the image lives on the assistant side. Left-aligned row wrapper (`flex justify-start`).
    ...
    10. Add a new `case "image":` in the `ws.onmessage` switch (parallel to the existing `case "message":`), appending the parsed image event to the same `messages` array via `appendDedup(prev, parsed)`. Do NOT create a separate state channel — image bubbles interleave with text messages in strict wire order (this is the whole point of user seeing "the agent read this image" at the correct chronological position in the conversation).
    ...
    - Hand-verify NOT gated (requires live dev server + user's fleet SSH), but the CLAUDE.md GSD Workflow / Nginx caveat is not relevant here (no new backend HTTP routes — WS-only).
  </action>
  <verify>
