...
Purpose: eliminate the ~1s freeze on Deactivate tap. user's fleet-management flow depends on list mutations feeling instant.
...
Trade-off (accepted by user, must be documented in the code comment): for a fraction of a second between "list settled" and "new pane mounted," the right pane still shows the just-deactivated pretty view. Acceptable because user isn't actually waiting on the session to be unloaded — she's waiting on the list to acknowledge her tap.
</objective>
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    ADD ABOVE the startTransition block, a 3-5 line block comment. Suggested content (adapt to file's comment style — the file already uses `// Patch #NNN:` block comments as shown at line 1159, so match that voice):
      ...
      - Line 4: TRADE-OFF — for a fraction of a second the right pane may still show the just-deactivated view while the list updates. Accepted: user isn't waiting on the session unload, she's waiting on the list to acknowledge her tap.
...
- Any push / build / deploy — stop at commit boundary per fleet rule (user 2026-07-27).
</verification>
...
- Commit SHA.
- Explicit note: "NO push, NO build, NO deploy per quick-task constraints. Tina bundles this with the pinned-slate batch when user greenlights ship."
</output>
