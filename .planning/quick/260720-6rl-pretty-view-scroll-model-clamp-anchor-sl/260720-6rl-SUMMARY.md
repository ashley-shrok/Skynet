...
**Pin-time reminder:** bump `skynet-patches.md` header 95 → 96, add per-patch entry for the four files above, update the "Patch drift caveat" file list, and follow `~/.claude/identities/tina/deploy-runbook.md` (mandatory 15-min deadman) at deploy time — NOT now (user handles deploy separately per fleet rule).
