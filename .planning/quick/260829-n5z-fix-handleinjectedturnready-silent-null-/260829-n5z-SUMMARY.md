...
user hit this 1-in-8 UAT sends (tina, 496KB PNG, 2026-08-29 16:20:52) — compose cleared
...
Similar pattern exists in `MessageQueueDrawer`'s `onSend` prop in `IdentitySessionPane.tsx` (line ~243): `if (!send) return false` — the drawer send also silently drops if `pvSendInputRef.current` is null. Less likely to be hit (drawer sends are user-initiated, WS usually stable by then) but worth the same queue-and-replay treatment if user observes drawer sends fizzling.
