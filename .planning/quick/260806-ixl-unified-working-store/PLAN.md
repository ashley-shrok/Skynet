...
- **No worktrees.** Work in `/home/ubuntu/skynet` on branch `feat/tab-title-from-tmux`.
  Any Agent tool spawn is main-tree, never `isolation: worktree`. (user fleet
...
<task type="auto" tdd="false">
  ...
  <action>
    ...
    **Full-suite gate — MANDATORY, this is the recovery point:**
    ...
    commit (user's fleet rule — never leave tests failing). Note the fix in
    the commit body.
  ...
  <done>
    ...
    - `npx vitest run` exits 0 with zero failures. Any pre-existing failures on
      the branch were fixed in this same commit (per user's rule) and the fix
...
- Real-world verify user will run herself post-ship:
