...
must_haves:
  truths:
    ...
    - "user can right-click any non-RDP conversation row and see Hide (or Show, when already hidden) between Pin/Unpin and Deactivate"
...
Purpose: user wants to declutter the pretty-conversations panel per Skynet account without permanently deactivating agents. Hidden rows should stay recoverable via the collapsible section — same durability model as pinning (server-backed, cross-device via the account's row in `user_preferences`).
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    (3) `src/ui/features/pretty-conversations/PrettyConversationsPanel.tsx`:
        ...
        - Add local `const [hiddenExpanded, setHiddenExpanded] = useState(false);` — collapsed by default per user's lock
