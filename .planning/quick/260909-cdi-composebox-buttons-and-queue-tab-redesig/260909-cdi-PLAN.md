...
Purpose: Locality of interaction — clicking a plus on top of a stack should feel like adding to the top. The old button-in-a-row didn't carry that gesture. Recap phrasing tweak improves how often user reaches for it (her own usage found the new phrasing works better).
...
<success_criteria>
- user clicking Recap now sends `/explain what has gone on since my last message` verbatim.
