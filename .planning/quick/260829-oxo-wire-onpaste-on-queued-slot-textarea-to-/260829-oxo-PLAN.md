...
must_haves:
  truths:
    - "user can paste a screenshot (or any file-shaped clipboard payload) into any queued-slot Textarea and the file is staged under target `queued:${slot.id}` via onAttachFilesForTarget — a chip renders in that slot's chip strip"
...
<objective>
Close the queued-slot paste gap user live-reported. The primary composebox
...
#     bounty if user wants the primary path instrumented too.
#
