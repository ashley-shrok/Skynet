...
tech-stack:
  ...
  patterns:
    ...
    - "Tailwind arbitrary-value inline styling for user's console-locked color/position values"
...
One-liner: user's console-locked subtle inside-textarea Send button (bare `<button>` inside the compose textarea wrapper, absolute right-3 bottom-2.5, quiet 30%→90%→15% currentColor states) shipped as one atomic commit routing entirely through the existing handleSend() — no branching duplication.
