...
**Paste timing:** After user greenlights the batched #123-#129 deploy AND UAT passes. Post-deadman-retirement flow per current `deploy-runbook.md`.
...
## Draft (paste-ready)
   ...
   129. `feat(pretty-view/ComposeBox): patch #129 — subtle inside-textarea
        ...
        at `37986b2`; deploy batched with #123-#128 pending user
        greenlight).
        ...
        * **Motivating gap.** Post-patch-#121 (vestigial-Send trim) the
          ...
          contract. user wanted the ChatGPT/iMessage/Slack pattern:
        ...
        * **Design lock via console-snippet iteration** (not prototype.html,
          ...
          low-opacity rest / high-opacity hover. user then tweaked
        ...
        * **Fix.** One-file substantive change (plus test extensions):
          ...
          - Insert bare `<button type="button">` sibling to `<Textarea>`
            ...
            bottom — user's locked values). `p-2` gives 40×40 hit target
          ...
          - Colors locked verbatim per user: rest
        ...
        * **VISUAL-08 hard-lock comments (lines 1134/1164/1194/1242)
          ...
          later date), that's a separate patch — user's directive was
