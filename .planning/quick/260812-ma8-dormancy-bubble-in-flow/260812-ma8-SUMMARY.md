...
Converted `DormancyOverlay` from a full-surface scrim (blurred + blocked the chat region) into an assistant-aligned in-flow bubble at the bottom of the message list (mirrors PlanPendingBubble's Phase 4 Glass treatment), so user can read the tail of the conversation while a session is dormant and decide whether to wake it. Three states + STATIC Moon guardrail preserved verbatim; only outer container geometry and mount site changed.
...
Awaiting user's iOS PWA visual confirmation for:
