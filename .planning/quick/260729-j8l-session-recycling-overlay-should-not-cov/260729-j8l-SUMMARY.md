...
is gated off by a new `recycleActive` prop — user can now pre-draft the next
...
- **PrettyView:** the `{showOverlay && <SessionHoldingOverlay error={...} />}`
  ...
  user can pre-draft during recycle" rationale.
...
user can pre-draft the next message. The autosave path (patches #57 / #119)
