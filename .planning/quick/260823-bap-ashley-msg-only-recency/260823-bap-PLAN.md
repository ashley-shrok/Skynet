...
ordering reflects ONLY user's real messages going TO agents (typed prose +
...
injections. user 2026-08-23 verbatim: "only my real messages going to them" —
...
user's own outbound cadence to each agent, not agent-side activity or
...
<task type="auto" tdd="true">
  ...
  <behavior>
    New describe block per test file — `describe("isRealUserTurn — user 2026-08-23 lock predicate matrix")`. Same 7 cases per site (byte-parallel):
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    (d) REPLACE the docblock at ~L229-246 verbatim with:
        ```
        /**
         * user 2026-08-23 lock: "only my real messages going to them" —
    ...
    (h) Update the docblock at ~L54-68 to reflect the new lock — retain the "canonical copy in ssh-poll-orchestrator.ts" cross-reference (with the current line number of the helper — grep to confirm before writing), just invert the meaning-carrying paragraph to cite user 2026-08-23 verbatim.
  ...
  <done>
    ...
    Docblock at site #1 contains the exact string "user 2026-08-23 lock" AND "INVERTS the 2026-08-14 lock".
    Docblock at site #2 contains "user 2026-08-23 lock".
  </done>
</task>
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    When updating a test's assertion, ALSO update its inline comment (if any) so future maintainers see the lock inversion documented at the point of test. Cite "user 2026-08-23 lock" in the comment.
    ...
    After all inversions, run the full scoped suite and confirm zero failures across both files. Then commit as `test(quick-260823-bap): invert existing tests to user 2026-08-23 msg-only-recency lock`.

    Executor scope explicitly STOPS here — do NOT run the full-repo test suite (that is the orchestrator's ship-gate per fleet directive user 2026-08-20), do NOT deploy, do NOT `git push`, do NOT edit `~/.claude/roles/box-maintainer/box-maintainer.md`, do NOT edit `skynet-patches.md`.
  </action>
  <verify>
  ...
  <done>
    ...
    `grep -c 'user 2026-08-23 lock' src/backend/fleet-status/ssh-poll-orchestrator.test.ts` returns ≥ 1 (at least one inline comment cites the lock, plus the new describe block from Task 1).
...
Both docblocks cite the new lock:
  grep -c 'user 2026-08-23 lock' src/backend/fleet-status/ssh-poll-orchestrator.ts   # ≥ 1
  grep -c 'user 2026-08-23 lock' src/backend/database/routes/sessions.ts             # ≥ 1
...
- Docblocks at both sites cite the user 2026-08-23 lock verbatim (with cross-reference to canonical copy site — line numbers grep-refreshed).
...
<output>
Create `.planning/quick/260823-bap-user-msg-only-recency/260823-bap-SUMMARY.md` when done — record:
  ...
  - Confirmation that both docblocks cite "user 2026-08-23 lock"
