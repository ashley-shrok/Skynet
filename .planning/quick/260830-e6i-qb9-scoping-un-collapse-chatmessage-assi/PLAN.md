...
Three surgical follow-ups after this morning's ship (HEAD `20c8ad33`). user's UAT feedback:
...
- **Part C**: r9i's `goodbye_echo` skip only catches `"<local-command-stdout>Goodbye!</local-command-stdout>"` — user's exit routine emits three more variants that also need to skip: `"Catch you later!"`, `"See ya!"`, `"Bye!"`. Widen the predicate to match any of the four literal strings.
...
Only catches `Goodbye!`. user's session-end routine also emits three other variants — all of them equally session-lifecycle noise:
...
- **Do NOT deploy.** Commit only. Orchestrator (Tina) handles ship motion after user greenlights push.
...
which was over-broad — user wanted only the REMOTE-source ones
...
user: "the whole point of that change was supposed to be that we
...
user's computed-styles measurement of the qb9 collapsed relay bubbles
...
literal "<local-command-stdout>Goodbye!</local-command-stdout>". user's
