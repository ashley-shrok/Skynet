...
1. **Close pinned bounty `sidebar-header-left-gap-after-collapse-button-moved`** (parked 2026-07-30 in user's tina identity). Root cause: patch #193 (commit `1e14cba`) moved the persistent sidebar-toggle chevron from viewport-left to `left: sidebarWidth + 8px` on desktop-open, but the 48px padding-left clearance in `.pv-panel-header` (patch #142 Fix 5) was still firing on desktop-open — reserving space for a chevron that had already moved elsewhere. The prop-flip (change #1) stops the clearance from firing when the chevron isn't at viewport-left. The attached-tab treatment (change #2) makes the chevron read as visually part of the sidebar so no gap even looks needed.
...
Verified via `git diff` that all four working-tree changes were present as spec'd (tina-authored via docker cp fast-path, eyeballed live by user — executor did NOT reimplement):
...
- Post-commit branch still `feat/tab-title-from-tmux`; deploy queue extends to #198→#214, still HELD per user's not-shipping-until-shape-lock rule.
