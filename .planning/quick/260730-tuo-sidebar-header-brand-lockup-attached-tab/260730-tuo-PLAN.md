...
Purpose:
1. Close pinned bounty `sidebar-header-left-gap-after-collapse-button-moved` (parked 2026-07-30 in user's tina identity). Root cause: patch #193 (commit `1e14cba`) moved the persistent sidebar-toggle chevron from viewport-left to `left: sidebarWidth + 8px` on desktop-open, but the 48px padding-left clearance in `.pv-panel-header` (patch #142 Fix 5) was still firing on desktop-open — reserving space for a chevron that had already moved elsewhere. Change #1 (prop flip) stops the clearance from firing when the chevron is not at viewport-left. Change #2 (attached-tab treatment) makes the chevron read as visually part of the sidebar so no gap even looks needed.
...
into the running skynet container and eyeballed live by user. DO NOT reimplement.
...
user's feedback: "chevron should look like part of the sidebar, not a floating mystery button."
...
- DO NOT PUSH, DO NOT DEPLOY, DO NOT touch `docker compose`. Deploy queue is HELD per user's
...
<task type="auto">
  ...
  <action>
    ...
    silently re-apply — user eyeballed the current diff live and greenlit it as-is.
...
<task type="auto">
  ...
  <action>
    ...
    2. AppShell.tsx L1506-1555: persistent chevron gets an attached-tab
       ...
       floating-pill treatment. user: "chevron should look like part of the
