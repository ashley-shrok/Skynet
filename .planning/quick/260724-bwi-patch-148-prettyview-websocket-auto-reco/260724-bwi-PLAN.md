...
must_haves:
  truths:
    ...
    - "visibilitychange:visible resets reconnectAttemptsRef to 0 and opens a fresh WS when the current WS is not OPEN and status is not 'inactive' (the direct user iOS PWA repro fix)"
...
PrettyView's claude-session bridge WebSocket to fix user's iOS PWA repro:
...
cleanly (3 end-to-end cycles, 0 failures). The "connection closed" text user
...
- visibilitychange handler resets counter to 0 (user iOS PWA fix)
  ...
  <action>
    ...
    (c) MODIFY the existing WS-setup useEffect (currently L280-468):
        ...
        - Update / rewrite the comment block that used to say "Do NOT
          ...
          path (which is where user's iOS PWA case lands).
...
6. **Commit (single atomic, fork convention):**
   ...
   - Body: brief mention of Terminal.tsx pattern being mirrored, the
     ...
     fresh-budget path (user iOS PWA fix), and the 5-attempt/8s cap.
...
7. **DO NOT update `~/.claude/identities/tina/skynet-patches.md`** —
   deferred to deploy-recommendation time per user's 2026-07-23
