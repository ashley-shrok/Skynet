...
Commit message: `feat(pretty-conversations): patch #136 — full pretty-view bubble+badge restyle for every row (locked from user's mock)`
</execution_context>
...
- `@media (prefers-reduced-motion: reduce)` wrapping a selector that targets the dot: use an attribute selector `[data-pv-conv-wip-dot="true"]` so the `.tsx` render slot in Task 2 can opt in via a data attribute (avoids coupling to a global class name user hasn't sanctioned).
...
- Commit lands (single commit) with the message shape:
  `feat(pretty-conversations): patch #136 — full pretty-view bubble+badge restyle for every row (locked from user's mock)`
