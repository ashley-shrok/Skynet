...
Five `type:"user"` events currently render as user-role bubbles in PrettyView that user wants hidden — they're all session-lifecycle noise injected by Claude Code or agent-supervisor ("hide how the sausage is made"). Add skip rules in `session-file-parser.ts` alongside the existing `harness_wrapper` skip.
...
3. **`goodbye_echo`** — `content.trim() === "<local-command-stdout>Goodbye!</local-command-stdout>"`
   - Narrow: only this literal string; other `<local-command-stdout>...</local-command-stdout>` blocks stay (user uses other slash-commands intentionally and their stdout should render)
...
- Commit with message:
  ...
  items 1, 4, 5. /id is NOT excluded there (used as "user present" signal)
  ...
  because user intentionally invokes other slash-commands whose output is
  useful context.
  ```

## Constraints

- **DO NOT deploy.** Executor's remit stops at code + commit + scoped-tests-green. Orchestrator picks up ship motion after user greenlights push.
