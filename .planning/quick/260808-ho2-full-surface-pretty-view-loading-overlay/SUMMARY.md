...
design_decisions_locked:
  ...
  timeout: 10000ms silent auto-dismiss (no error variant per user's ask)
...
A new `PrettyViewLoadingOverlay` component and its wiring into `PrettyView.tsx` that covers the ~5s window between a fresh pane mount and the first user-visible WS frame arriving. Fixes user's silent-window UX bug (row lights up but pretty-view sits blank for 5s → she re-taps → double-fires).

Component: full-surface scrim + centered glass card with `Loader2` spinner + "Loading…" copy. Mounted inside the chat-region wrapper as a sibling of SessionHoldingOverlay and DormancyOverlay — ComposeBox (peer sibling below the wrapper) stays typeable so user can pre-draft during the boot window (patch #275 posture preserved).
...
- No error variant of the loading overlay (per user — 10s silent dismiss + underlying `status="inactive"`/`"error"` render branches).
