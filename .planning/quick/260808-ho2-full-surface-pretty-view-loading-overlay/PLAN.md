...
must_haves:
  truths:
    ...
    - "The loading overlay covers messages / tasks / shells but does NOT cover ComposeBox (user can pre-draft during the boot window; patch #275 anchor)."
    ...
    - "The loading overlay dismisses automatically after ~10s if no frame arrives (stuck-state fallback; silent dismiss, no error variant per user)."
...
Purpose: eliminate the silent-window UX bug — user taps a conversation row, the row lights up but pretty-view sits blank for 5s while it mounts + fetches + does its WS handshake, so she re-taps and double-fires. user verbatim ask: "just doing like a full screen overlay with like a spinner to block everything else from being touched and let you know that it's going would be good."
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    TIMEOUT — new useEffect placed adjacent to the existing overlay-related useEffects (currently lines 1223-1276). Pattern mirrors patch #122's holding_timeout watchdog (lines 1257-1265):
      ...
      // no error variant (per user's ask — no cancel button, no error card).
    ...
    DERIVATION + MOUNT — at the mount site (currently lines 1556-1568, immediately after `{dormant && <DormancyOverlay ... />}`):
      ...
      {/* quick 260808-ho2: full-surface loading overlay for the ~5s window
          ...
          ComposeBox (peer sibling below the wrapper) stays typeable — user
  ...
  <action>
    ...
    Step 3 — Commit (single atomic commit per fleet convention; commit_docs=true means PLAN.md ships with the code):
      ...
      Commit message (HEREDOC form to preserve newlines):
        ...
        Fixes user's silent-window UX bug (row lights up but pretty-view sits
        ...
        Design (all six gate decisions locked in PLAN.md § Objective):
          ...
          3. No minimum hold time — arm instantly. Flash on fast mount is fine
             (user's complaint is silence, not overlong loading UX).
          4. 10s timeout auto-dismiss, silent (no error variant per user's ask).
...
| T-260808-ho2-04 | Repudiation | 10s timeout dismiss hides a genuine stuck state | accept | `console.info` log emitted on timeout dismiss (see Task 2 action). Stuck-state fallback intentionally silent (no error card per user); underlying `status === "inactive"` / `"error"` render branches show through immediately after dismiss. |
