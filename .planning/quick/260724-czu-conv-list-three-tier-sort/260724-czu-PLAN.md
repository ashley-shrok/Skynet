...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    5. **DO NOT modify** the following (out of scope for this slice):
       ...
       - The `updateOpenTabs` pruning behavior for pinnedIds. Note: this DOES still prune pinnedIds for ids not in openTabs; that means a fleet-only pinned id (fleet::1::work) will NOT survive an updateOpenTabs call that lacks it. user's post-#149-A design accepts this as a known limitation (fleet rows can be pinned per-session; page refresh drops the pin). Do NOT try to fix here; leave the pruning code alone.
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - No visual separator between the three tiers — the existing `.pv-panel-group` gap CSS gives adequate visual coherence per user's Telegram-shape flat-list lock.
  ...
  <action>
    ...
    3. **Add the new active-set group render block** immediately BEFORE the existing `<div className="pv-panel-group" data-pinned-group="true">` block (~line 331). Structure mirrors the pinned block VERBATIM except:
       ...
       - Comment above the block: `{/* Patch #149 B+C: active-set rows overtake pinned per user 2026-07-24. Rows here get pinned={pinnedIds.has(row.id)} so a row that IS pinned AND active still shows the pin glyph. */}`
...
<task type="auto">
  ...
  <action>
    ...
    3. Do NOT run:
       ...
       - Docker build or docker compose commands (deploy is batched with Slice A, deferred to user greenlight).
       - Fork-specific patches.md updates (deferred to deploy-recommendation time per user 2026-07-23 batch-writeups-until-deploy rule).
...
<success_criteria>
- user's ask satisfied: active-set rows overtake pinned rows at the top of the panel.
...
- No deploy — bundled with Slice A (cf624a4) for one deploy at user greenlight.
</success_criteria>
