...
Swap ComposeBox send-button icon from static paper-plane to spinning `Loader2` while `voice.state === "transcribing"` so user gets in-button feedback that her Send-transcript tap registered during the 1-3s `/voice/transcribe` round-trip.
