...
<objective>
Swap the ComposeBox send button icon from the static paper-plane to a spinning `Loader2` while `voice.state === "transcribing"`, so user gets visible in-button feedback that STT is in flight. The button already renders disabled during transcribing (via `showTranscribingSend` on line 1779); this change adds the visual signal that "something is happening" during the 1-3s round-trip to `/voice/transcribe`.

Purpose: Close the perceptual gap where the send button today just dims (disabled tint) with no motion — user can't tell whether her Send-transcript tap registered or the app is stuck. A spinner in the same slot resolves that ambiguity without moving any layout.
