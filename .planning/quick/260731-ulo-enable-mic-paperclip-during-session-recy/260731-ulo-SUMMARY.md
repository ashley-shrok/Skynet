...
- **No push** to origin — user greenlights ship separately per fleet push→build→recreate rule.
- **No `docker build`** — frontend-only edit; skipped `npm run build:backend` per constraints.
- **No `docker compose up -d --force-recreate skynet`** — deploy motion is the orchestrator/user's decision.
