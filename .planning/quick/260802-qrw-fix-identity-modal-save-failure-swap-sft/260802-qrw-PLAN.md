...
Root cause (confirmed on skynet-ec2, root-caused by @stacy on ceo-skynet 2026-08-02, full handoff at ~/pretty-view-uploads/2026-08-02/190204-TINA-HANDOFF.md):
  ...
  user's "sometimes it works, sometimes it doesn't" — she confirmed all her
...
top-of-list production bug for user today.
...
- Do NOT add a fallback for sshd versions that lack posix-rename@openssh.com — OpenSSH ≥5.1 (2008+) supports it universally and every host in user's fleet is well beyond that; a fallback would add a code path we cannot exercise.
...
4. Manual smoke (out of scope for automation but noted for the orchestrator's post-plan verification): after this plan ships and skynet redeploys, user should be able to open the IdentityModal on any existing identity (e.g. `tina`), edit any of the four writable fields (main markdown, history, handoff, bounty fields), click Save, and see the save succeed without the generic "Error: Failure" toast. The orchestrator owns the redeploy step (not this plan; per scope, no `docker compose up`).
</verification>
