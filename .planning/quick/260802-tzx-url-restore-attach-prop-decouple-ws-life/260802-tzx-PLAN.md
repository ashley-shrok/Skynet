...
Purpose: Fix bounty `url-restore-loads-only-selected-session-not-full-active-set`. user's dot-semantics lock (ready-dot must mean "idle AND connected") is being violated because ambient URL-restored tabs light up their dot without an underlying WS.
...
<task type="auto" tdd="false">
  ...
  <action>
    ...
    5. Build the commit message via HEREDOC. Include the TESTING-GAP trailer paragraph verbatim from `/tmp/testing-gap.txt` if that file exists; otherwise omit that trailer. Do NOT include a Claude co-author trailer (user's fleet convention — solo commits on this repo).
...
having no live connection — violating user's lock that the dot means
"idle AND connected."
