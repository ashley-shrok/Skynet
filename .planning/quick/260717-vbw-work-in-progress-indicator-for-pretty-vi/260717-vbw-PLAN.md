...
Purpose: today, when Claude Code takes 30-60s to finish a tool call, user
...
Do NOT include a Claude co-author line — user's convention is bare
...
4. Do NOT accept props for now; hardcode the visual. user's design is
...
8. SMOKE CHECK (run all three; each must succeed):
   ...
   b. Verify the local build succeeds:
      ...
      commit. user's convention preserves atomic commits; prefer a
...
9. DO NOT deploy. DO NOT run `docker compose`. DO NOT touch /opt/skynet.
   DO NOT arm a deadman. DO NOT update AGENTS.md. user has explicitly
...
4. No changes to `session-file-parser.ts` (hands off per user).
...
- Explicit note: NOT deployed. Requires future deploy behind the mandatory
  15-min deadman when user authorizes.
</output>
