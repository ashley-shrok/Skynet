...
If Claude Code crashes or is killed mid-tool-call, the last JSONL event will be `tool_use` or `tool_result` and WIP will remain `true` until new user input arrives. Accepted per user's design brief. No fix in this patch.
...
NOT DEPLOYED. Requires a future `build-skynet.sh` + `docker compose up -d --force-recreate skynet` behind the mandatory 15-min deadman when user authorizes.
