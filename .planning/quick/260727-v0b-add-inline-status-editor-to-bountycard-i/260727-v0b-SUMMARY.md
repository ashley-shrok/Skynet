...
decisions:
  ...
  - "Status pills use their STATUS_CLASSES color for the INACTIVE state (per user's ask: 'no glyph — just label + color'); ACTIVE pill still gets the pressed white-ring treatment so it reads as selected against the colored row."
metrics:
  duration: ~10min
...
Adds a StatusRow inline editor to BountyCard's expanded body — the parallel of patch #154's PriorityRow, applied to `bounty.status`. user can now click a pill in the identity modal to flip a bounty between the 5 status values (pinned / in_progress / waiting_on_someone_else / done / dropped) without hand-editing bounty.json, and can resurrect a done/dropped/archived bounty by clicking `pinned` or `in_progress` on it. Server patches bounty.json in place — folder is NOT moved.
...
**Post-commit report: committed on branch feat/tab-title-from-tmux at 4de8f14; NOT pushed, NOT built (image), NOT deployed. Awaiting user's ship signal.**
