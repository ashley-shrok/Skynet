...
**Deferred to human-verify checkpoint (Task 2, skipped per prompt constraint):** browser eyeball for cohesive-instrument reading, drain animation, hover state, null-contextPct case. user will run this after deploy.
...
- Task 2 (checkpoint:human-verify) — SKIPPED per prompt constraint (user handles browser verification post-deploy)
