...
**Ashley's iPhone use case:** Tap "Add to Home Screen" on term.example.com → launches as standalone app (no Safari chrome), Skynet whole-mesh-energized network graph icon, notch-safe padding, no pinch-zoom + no tap-into-input auto-zoom, "Skynet" everywhere she looks in the UI.
