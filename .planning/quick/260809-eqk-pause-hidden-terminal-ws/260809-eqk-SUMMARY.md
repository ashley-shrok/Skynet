...
- **Ship PV diag fix in the same commit:** dishonest diag emit would obscure the iter-2 win — user + tanya need reliable numbers post-ship to declare the empirical rotation done.
...
- **NOT pushed, NOT built, NOT deployed** per plan constraints — orchestrator (tiffany) motions post-return:
  ...
  - Retire bounty `pause-hidden-terminal-ws-iter-2` (or whatever the exact bounty JSON name is) once user UATs the diag delta on iPhone 16 Pro Max.
