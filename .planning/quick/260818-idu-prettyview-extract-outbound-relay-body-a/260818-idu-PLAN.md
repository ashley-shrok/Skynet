...
Purpose: pretty view currently shows a wall of curl args for every relay send — Alice reads through the noise to find the actual message. Extracting the body and floating it above the raw command as a pretty text bubble (with rawCommand behind an expand toggle) turns 96.4% of relay sends into readable one-liners, while the 3.6% fallback path (cross-turn file refs, python heredocs) safely degrades to today's always-expanded mono block.
