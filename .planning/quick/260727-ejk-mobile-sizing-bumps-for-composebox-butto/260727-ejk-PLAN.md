...
<objective>
user finds ComposeBox controls and the PrettyView jump-to-latest button cramped on her mobile viewport. Apply purely-additive Tailwind `max-md:` className variants (viewport <768px, same threshold as `useIsMobile()`) so every ComposeBox button roughly doubles in tap-target size and the scroll-to-bottom button roughly quadruples on mobile — with zero desktop visual changes and zero JS/logic touched.

Purpose: user-requested mobile ergonomics batch. Stacks on top of cdccd4f (this session's earlier ComposeBox amendment) and is held for a single combined deploy after user's UAT greenlight — do NOT deploy inside this task.
...
**Batch-deploy rule**: Do NOT run `docker compose up -d --force-recreate skynet` inside this task. This change stacks on cdccd4f and awaits user's combined-batch UAT greenlight before deploy (per the 15-min deadman rollback constraint — one deploy, not two).
  </action>
  <verify>
  ...
  <done>
    ...
    - NOT deployed — stacks with cdccd4f pending user's combined-batch UAT greenlight
  </done>
</task>
...
- Human verification (user on mobile) is the definitive UAT signal — deferred to post-deploy alongside cdccd4f.
</verification>
...
- Change is held (NOT deployed) until user greenlights the combined batch with cdccd4f.
</success_criteria>
...
- Deploy status: HELD (batched with cdccd4f, awaiting user UAT)
</output>
