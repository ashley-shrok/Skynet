...
decisions:
  ...
  - "Archive button threaded to OPEN BountyCard render sites only (the single OPEN_STATUS_ORDER map covers in_progress + rest + other with one addition). Deliberately NOT threaded to sortedArchive.map — cards already under archive/ don't get the button; unarchive is a separate follow-up quick per user's UX call."
...
**Post-commit report: committed on branch feat/tab-title-from-tmux at 3362d6f; NOT pushed, NOT built (image), NOT deployed. Awaiting user's ship signal.**
