...
The naked-spinner treatment prevents the first-glance ambiguity where a bubble containing only a spinner reads as "assistant is composing a message" — instead, the spinner-alone shape reads as "session state indicator" instantly, matching user's operator model.
