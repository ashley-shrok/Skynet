...
**Commit:** 3d28512
**Deploy status:** DEFERRED — user bundles with prior UAT commits (558749a, 68e4f62, b749bf1, 536d224) in a single grouped deploy.
...
NOT deployed. NOT pushed. user owns the deploy timing and bundles this with:
- 558749a
- 68e4f62
- b749bf1
- 536d224
...
iOS caches PWA manifest theme_color + background_color at install time. user WILL NOT see the safe-area seam disappearance (Skynet-gray top/bottom bars instead of black) until she:
...
- Bounty todo #7 ("Persistent top-left chevron stays, safe-area architecture stays post-#131") — untouched per user's directive: this patch is pure REMOVAL + color rebase, no positive UI structural changes.
- Bounty todos #1 ("side-by-side survey") — implicitly completed via prototype.html grep during planning; explicit visual diff enumeration deferred to user's UAT walkthrough.
- Bounty todo #8 ("deploy") + #9 ("patches-md #133 pin") — DEFERRED to user's greenlight, tracked as follow-up in bounty timeline.
...
- If user later wants a Skynet settings surface, build fresh — do NOT reintroduce shadcn DropdownMenu. Radix headless primitives + custom styling per the tina.md § Learned preferences shadcn `!` important lesson (patch #81); this patch REMOVED the wrapper, not just its usage.
