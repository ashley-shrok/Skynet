...
Fix (LOCKED by user — not up for re-debate): fold role resolution into the per-host session
    ...
    **Frontend — src/ui/state/conversation-store.ts:**
    ...
    5. Update `rowFromTab` at line 283-294: LEAVE the identitiesByKey lookup as-is here.
       ...
       **Correction: with identities.ts returning role: null for all rows, `rowFromTab`'s
       ...
       user's fleet); acceptable and mirrors the `openTabsSessionKeys` dedup pattern already
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    user: after committing (do NOT push/build/deploy — per your standing rule, "ship it"
