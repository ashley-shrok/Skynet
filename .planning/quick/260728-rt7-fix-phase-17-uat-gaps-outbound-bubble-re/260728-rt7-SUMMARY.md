...
decisions:
  - Option D (user 2026-07-28): delete body-extractor entirely; rawCommand IS the bubble body
...
**Fix (Option D, user 2026-07-28):** Delete the body-extraction block (~L181-197) from `detectRelayOutbound`. `rawCommand` IS the body — rendered faithfully as a scrollable `<pre>` mono block.
...
- Follow-up bounty comment added: opportunistic MSG=/BODY=/TEXT= grep deferred per user's explicit decision.
...
- Test 1: Replaced `/tmp/` positives with user's exact reproducer path (`_j14UxhqP0NpJXLReeXBR0qPGh04JwNXDGneCrEyarWw.txt`) + alt identity path — both MUST match.
...
- NEW Test 6: "detectFilePointer matches recv.sh preview line format with em-dash boundaries" — feeds the exact user reproducer string; asserts `pointerPath` equals the identity-dir path.
...
- user's exact reproducer path (`_j14UxhqP0NpJXLReeXBR0qPGh04JwNXDGneCrEyarWw.txt`) matches WHITELIST_REGEX and POINTER_REGEX — verified by Test 1 (backend) and Test 6 (frontend).
