...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Forensic logging (Alice called this out — next iOS UAT needs branch-level evidence in the forwarded console stream):
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    - `npx vitest run src/ui/features/pretty-view/` — expect no new regressions vs the pre-change baseline (Alice's UAT baseline: 583 pass / 6 skip / 1 todo per SUMMARY.md L86).
...
7. Manual iPhone spot-check (deferred to Alice's next UAT window — not blocking executor exit):
...
8. Bounty update (deferred to Alice's discretion):
   - Update `~/.claude/roles/box-maintainer/bounties/hold-to-record-move-to-mic-button/` — append timeline entries per task; mark iPhone UAT regressions closed once the manual spot-check confirms fix. Do NOT close the bounty from executor context — the manual verification is Alice's call.
...
- Ship intent `#442` (verify next available at ship time; do NOT ship from executor context — tina orchestrator handles ship motion after Alice greenlight).
</output>
