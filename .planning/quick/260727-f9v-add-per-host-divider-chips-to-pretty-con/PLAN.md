...
<objective>
Two coupled UI changes in the pretty-conversations list, greenlit by user:
...
Panel context:
  ...
  - Regular host groups: panel lines 424-448, currently FLAT (comment "FLAT per user/prototype lock" — that lock is INTENTIONALLY reversed by this task)
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test A (identityTitle + identity.title set): render row with `subtitleMode="identityTitle"` and a resolved identity having `title="user Ops"` and `displayName="user"`. Assert the sublabel text is exactly "user Ops"; assert NO `svg` corresponding to the Server icon exists inside the `.pv-host` span (query by parent tag or by lucide's rendered class marker — the existing Server render uses width=11 height=11, so `.pv-host svg[width="11"]` should return null).
    - Test B (identityTitle + identity.title null, displayName set): render with `subtitleMode="identityTitle"` and identity `title=null`, `displayName="user"`. Assert sublabel text is "user"; assert no Server icon in `.pv-host`.
...
<verification>
Manual smoke check (optional, executor's discretion — this is a visual change; no smoke required for correctness but user may want a pretty-preview screenshot before deploy):
...
<risk_notes>
- **Per Tina's patch #149 lesson ("known limitation, inert ≠ inert"):** the row's fallback chain in the "identityTitle" branch MUST have the terminal keep-hostname safety net — if it's silently missing, unresolved-identity rows in host groups will ship with sublabel "" or "undefined" on user's very next click. Test C in Task 1 exists specifically to guard this — do NOT skip it.
- **Deploy discipline:** this change alone does NOT warrant an immediate deploy. Leave the deploy call to the executor/Tina after the commit lands, unless user explicitly asks for an out-of-band push. If a deploy does happen, the executor's deploy notification MUST pre-warn user about first-hard-refresh HTTP2_PROTOCOL_ERROR (Tina's learned pref on any container recreate).
