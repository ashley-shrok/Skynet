...
// but with a treatment tweak per user's ask: status has no glyph (just
      ...
      >
        <span className="flex flex-col min-w-0 flex-1 text-left">
          ...
          {/* Patch #109: slug line — folder basename, monospace + muted.
              user refers to bounties by slug in conversation, so making
      ...
      {expanded && (
        <>
          {/* Quick 260727-v0b: inline status editor mirroring patch #154
              priority editor. Placed ABOVE Priority per user's ask —
          ...
          {/* Patch #154: inline priority editor. Sits above the premise so
              ...
              frequency user re-prioritizes vs. reads the premise. */}
          {onPriorityChange && (
            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium text-[var(--color-pv-fg-muted)] uppercase">
                Priority
