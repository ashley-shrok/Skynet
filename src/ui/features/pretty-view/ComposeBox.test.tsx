...
describe("ComposeBox — Phase 05 upload wiring", () => {
  ...
  // timeout). user hit this four times on 2026-08-23 across wanda + nelly.
  //
...
describe("ComposeBox — Quick B: queued-row per-slot paperclip + top-left delete tab + padding parity + overlay chip + one-line armed header", () => {
  ...
  it("QB-3: queued row renders a delete × in the top-left corner OUTSIDE the textarea wrapper (tab protrudes via -top-* + -left-*)", async () => {
    ...
    // textarea's border (user moved from -right-2 to -left-2 on 2026-08-12).
  ...
  it("QB-13: 'click to cancel' literal lowercase copy is present in the armed overlay (regression guard for user's exact phrase)", async () => {
    render(<ComposeBox {...baseProps()} />);
    await flushMountEffect();
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: /queue a message/i }));
    ...
    // Case-sensitive matcher — user asked for the literal lowercase phrase.
...
describe("ComposeBox — optimistic bubble seeding (Phase 50 Plan 03 Task 2)", () => {
  ...
  it("Test 3: onSend returning false triggers onOptimisticSend with immediateFailure:true and clears textarea", async () => {
    ...
    // no draft-preservation for retry (user 2026-09-02).
    expect(textarea.value).toBe("");
  });
