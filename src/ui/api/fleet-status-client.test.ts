...
describe("fleet-status-client: Test 1 — opens WS and sends subscribe on open", () => {
  it("sends { schemaVersion: 1, type: 'subscribe' } when WS opens", () => {
    ...
    const ws = latestWs();
    // Phase 111 SKEW-09: client appends ?build=<CLIENT_BUILD_ID> to the WS URL
    // so the backend handshake gate (Plan 05) can refuse mismatched clients
    // with 4409. In vitest env, CLIENT_BUILD_ID falls back to "dev-unknown"
    // (import.meta.env.VITE_BUILD_ID is not substituted under jsdom).
    expect(ws.url).toBe("ws://localhost/fleet-status/ws?build=dev-unknown");
...
describe("fleet-status-client: app-frame dispatch (Phase 117 Plan 117-01)", () => {
  ...
  const sampleApp = {
    ...
    description: "Test app for Phase 117 UAT",
    port: 8080,
    hasIcon: true,
  ...
  it("does NOT throw when app frames arrive with all three callbacks omitted (they are optional)", () => {
    ...
    // Phase 116-era AppShell will still wire the client before 117-02 lands.
    createFleetStatusClient({
      url: "ws://localhost/fleet-status/ws",
      onSnapshot: vi.fn(),
      onUpdate: vi.fn(),
