...
export function createFleetStatusClient(
  ...
  // handleFrontendConnection) can refuse mismatched clients with 4409.
  // Preserves any existing query params by picking `?` vs `&` correctly.
  const wsUrl = url.includes("?")
    ? `${url}&build=${encodeURIComponent(CLIENT_BUILD_ID)}`
    : `${url}?build=${encodeURIComponent(CLIENT_BUILD_ID)}`;
  ...
  function connect(): void {
    if (disposed) return;
    // Phase 111 SKEW-09: if the skew-lock has already been tripped by
    // another lane (axios interceptor, another WS client), don't reopen —
    // the shell is about to reload.
    ...
    ws.onmessage = (evt: MessageEvent) => {
      ...
      // Backend piggybacks `build: <SERVER_BUILD_ID>` on every outbound
      // envelope (Plan 05 Task 3). If mismatched, fire the shell-level lock
      // and drop this frame — the session is skew-locked.
      const parsedBuild = (parsed as { build?: unknown }).build;
      if (typeof parsedBuild === "string" && parsedBuild !== CLIENT_BUILD_ID) {
        lockSkewedSession({
          reason: "ws_message_tag_mismatch",
          clientBuild: CLIENT_BUILD_ID,
          serverBuild: parsedBuild,
        });
    ...
    ws.onclose = (evt: CloseEvent) => {
      ...
      // CloseEvent (circular refs, per role-file directive + Pitfall 5).
      const isSkew = evt.code === 4409;
      ...
      if (isSkew) {
        lockSkewedSession({
          ...
          clientBuild: CLIENT_BUILD_ID,
          // Server closes before revealing its build on the wire — pull from
          // close reason if present, otherwise sentinel.
          serverBuild: evt.reason || "unknown",
        });
