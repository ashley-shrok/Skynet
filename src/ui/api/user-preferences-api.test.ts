...
// Phase 115 Plan 115-02: the prior "Phase 107 Plan 04 Task 1 —
// user-preferences-api hidden surface" describe block (API-107-01 through
// API-107-04) was retired per D-21 alongside the source-code deletion of
// putHiddenIds. The archive gesture ships in 115-06 via a dedicated POST
// endpoint, not a PUT array shape — so no direct pin-side mirror test is
// added here to replace it.
describe("Phase 115 Plan 115-02 — putHiddenIds retired", () => {
  it("putHiddenIds is not exported from @/api/user-preferences-api", async () => {
    const module = await import("@/api/user-preferences-api");
    const surface = module as Record<string, unknown>;
    expect(surface.putHiddenIds).toBeUndefined();
