...
// Phase 111 SKEW-07/SKEW-08: WS handshake gate + sendFrame piggyback for
// the frontend (browser) path. Watcher path (box-side, Tailscale-trusted,
// non-browser) is intentionally NOT gated — non-browser callers legitimately
// omit the ?build= query, matching D-06 spirit for that specific surface.
// SERVER_BUILD_ID is captured once at module load per D-16.
...
async function handleFrontendConnection(
  ...
  // Phase 111 SKEW-07: WS handshake gate. Frontend path only (browser
  // clients). Version tag arrives as `?build=<sha>` on the upgrade URL.
  // Absence AND mismatch both close with 4409 (stricter than HTTP D-06 —
  // browsers are the only legitimate frontend WS clients). Watcher path
  // is untouched: those callers are box-side, Tailscale-trusted, and
  // legitimately do NOT carry a build tag.
  const clientBuild = extractSkewTag(req);
  if (!clientBuild || clientBuild !== SERVER_BUILD_ID) {
    systemLogger.warn("Fleet-status frontend skew refused", {
      operation: "fleet_status_skew_refused",
      remoteIp,
      hasClientBuild: clientBuild !== null,
    });
