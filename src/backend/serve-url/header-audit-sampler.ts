 ...
 *   Gives user + downstream dashboards a full look at every outbound
 ...
 *   the second-layer-of-defense signal user will watch.
 *
 ...
 * structured logs so user can filter anomalies by which surface caught
 * them.
 */
...
export function emitHeaderAudit(
  ...
  // it. Warn-level so it surfaces on user's dashboard via the standard
