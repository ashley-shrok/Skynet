  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        // #321's auto-route callback (formerly the clone dialog's
        // onCreateSession — the dedicated clone dialog was removed in
        // Phase 80; the same route is now driven by the unified new-session
        // dialog's onCreate) fires on a bare login shell and pretty-view
        // renders "no active Claude session" for the newly-cloned identity's
        // tab. Same helper birth uses for its own steps 3-5. Latency added: ~25s (2s post-launch
