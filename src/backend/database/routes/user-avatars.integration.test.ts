...
const authControl = { userId: "user-placeholder", pass: true };
...
function bootstrapDb(): void {
  ...
  // Pre-seed one admin user so user (created during the test) is NOT the first
...
describe("Phase 85 end-to-end lifecycle", () => {
  ...
  beforeEach(async () => {
    ...
    authControl.userId = "user-placeholder";
    pendingWhereVal = null;
  ...
  it("create → serve → change → serve → delete → verify-clean", async () => {
    ...
    const createRes = await postMultipartCreate(globalServer, {
      textFields: { username: "user", password: "s3cret" },
      file: {
        filename: "avatar.png",
    ...
    // Inspect DB row to get user's real id (nanoid-generated by users.ts).
    ...
    expect(rowAfterCreate!.username).toBe("user");
    expect(rowAfterCreate!.avatar_path).toBeTruthy();
    // Filename convention: {userId}.{ext} — should be {newUserId}.png
    expect(rowAfterCreate!.avatar_path).toMatch(/\.png$/);

    const newUserId = rowAfterCreate!.id;
    ...
    // -------------------------------------------------------------------------
    // STEP 2: "Login" — set authControl.userId = newUserId
    // (The mock auth middleware reads authControl.userId at request time, so
    //  setting it here ensures all subsequent authenticated requests identify as user.)
    // Any non-empty string works as the JWT since the mock ignores it.
    // -------------------------------------------------------------------------

    authControl.userId = newUserId;
    const userJwt = "user-phase85-integration-jwt";

    // -------------------------------------------------------------------------
    // STEP 3: GET /users/:id/avatar → PNG bytes round-trip
    // -------------------------------------------------------------------------

    const getRes1 = await getAvatar(globalServer, newUserId, userJwt);
    ...
    const putRes = await putAvatar(globalServer, newUserId, userJwt, {
    ...
    expect(putBody.id).toBe(newUserId);
    ...
    const rowAfterChange = sqliteDb
      ...
      .get(newUserId) as { avatar_path: string | null } | undefined;
    ...
    const getRes2 = await getAvatar(globalServer, newUserId, userJwt);
    ...
    const delRes = await deleteAccount(globalServer, userJwt, "s3cret");
