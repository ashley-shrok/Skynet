 ...
 * Fixup M-2 shape) is what produced user's ~20s worst-case cold-load
 ...
 * defense.
 *
 * Thundering-herd math anchored to user's fleet size (~100 concurrent
...
export function createObservationLoop(
  ...
  function start(users: Array<{ userId: string; userMxid: string }>): void {
    ...
    for (const { userId, userMxid } of users) {
      ...
      // (user cold-load fix + fleet-size thundering-herd math).
