...
//
// Test corpus mirrors the 6/6 acceptance battery user walked through on
...
// Nelly's session where user typed normally in pretty view and hit enter;
...
describe("parseSessionLine — queue-operation enqueue as kind:message (Phase 50 Plan 01 Task 1)", () => {
  ...
  it("Test QO-3c (negative — /id slash-command skipped): enqueue whose content starts with /id does NOT emit message", () => {
    // user 2026-09-04: when Claude Code is busy and a slash-command is
...
describe("parseSessionLine — session-lifecycle noise skips (quick-260829-r9i)", () => {
  ...
  // JSONL shape observed in the wild that user wants hidden from PrettyView
  // bubbles.
  ...
  // user's session-end routine emits three additional literals beyond Goodbye!
  ...
  // user 2026-09-02: relay-hygiene closer phrase — a bare "No response
