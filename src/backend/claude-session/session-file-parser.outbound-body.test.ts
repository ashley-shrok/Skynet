...
curl -sS -X PUT "$BASE/rooms/$ROOM_ENC/send/m.room.message/$TXN" \\
...

  {
    ...
    // Phase 49 sanitize pass fix — pre-Phase-49 this returned "Relaying user"
    name: `NELLY-SHAPE — BODY-sq with '"'"' apostrophe escape (bash close-sq/quote/open-sq)`,
    cmd: `TOK=$(jq -r .access_token ~/fleet/identities/nelly/relay.json); BASE=$(jq -r .base ~/fleet/identities/nelly/relay.json); ROOM='!wNhqmNRUNlHesCshwg:thenasty.taild9b663.ts.net'; BODY='Relaying user'"'"'s reply: hi'; curl -sS -X PUT "$BASE/rooms/$ROOM/send/m.room.message/$TXID" -d "$(jq -nc --arg b "$BODY" '{msgtype:"m.text", body:$b}')"`,
...
TXN=$(date +%s%N)
BODY=$(cat <<'EOF'
Nelly — user asked me to loop you in. WindowsPc (100.80.122.111) is offline on tailscale ("last seen 2h ago, tx 11076 rx 0", ping times out, tcp/8000 unreachable). The faster-whisper STT on port 8000 is failing all requests: /voice/transcribe hits are returning 502 to her PWA after ~10.7s (undici ConnectTimeoutError at 10s). user's voice-in workflow is dead until it's back.
...
curl -fsS -X PUT \\
  -H "Authorization: Bearer $TOK" \\
  -H "Content-Type: application/json" \\
  -d '{"msgtype":"m.text","body":"[pretty-view relay-bubble feasibility test] hi user — this is a test send from tina. reply here on the relay whenever and i will inspect my session file to see the round-trip shape."}' \\
  "$BASE/rooms/$ROOM/send/m.room.message/$TXN" | jq -r '.event_id // .errcode // .' `,
    expectedBody:
      "[pretty-view relay-bubble feasibility test] hi user — this is a test send from tina. reply here on the relay whenever and i will inspect my session file to see the round-trip shape.",
  },
...
# Update user via Tina
ROOM='!FHdIfqtmSWcGYUfyVp:thenasty.taild9b663.ts.net'
CREDS=~/fleet/identities/tiffany/relay.json
BASE=$(jq -r .base "$CREDS")
TOK=$(jq -r .access_token "$CREDS")
BODY='@tina got the mechanism from the [wsdiag] tape. Please relay to user: FOUND IT (probably).'
curl -sS -X PUT -H "Authorization: Bearer $TOK" -H "Content-Type: application/json" \\
  ...
  -d "$(jq -nc --arg b "$BODY" '{msgtype:"m.text", body:$b}')" | jq -r '.event_id // .error'`,
    expectedBody:
      "@tina got the mechanism from the [wsdiag] tape. Please relay to user: FOUND IT (probably).",
  },
];
...
Hey — the extractor's BODY='relaying user' bug matched inside my heredoc content instead of the real body.
EOF
)
...
curl -sS -X PUT "$BASE/rooms/$ROOM/send/m.room.message/$TXN" \\
  ...
  it("F: APOSTROPHE round-trip — Phase 49 sanitize + shell-var substitution compose", () => {
    ...
    const cmd = `BODY_ALT='user'"'"'s note'
curl -sS -X PUT "$BASE/rooms/$ROOM/send/m.room.message/$TXN" \\
  -H "Authorization: Bearer $TOK" -H 'Content-Type: application/json' \\
  -d "$(jq -nc --arg b "$BODY_ALT" '{msgtype:"m.text", body:$b}')"`;
    expect(extractOutboundBody(cmd)).toBe("user's note");
  });
});
...
you and user, and stand by. Ping if that changes.
EOF
)
...
describe("extractOutboundBody — latent bug regressions", () => {
  it("LB-1: primary BODY=$(cat <<EOF …) beats secondary --arg body 'label' (tabitha→nelly shape)", () => {
    ...
    // "relaying user" turn (2026-08-20, room !wNhqmNRUNlHesCshwg…) — nelly
