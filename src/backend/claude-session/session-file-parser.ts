...
// quick-260830-e6i (widen r9i goodbye_echo): user's session-end routine
...
export type ParsedLine =
  ...
  // byte-count placeholder tells user something was lost).
 ...
 * now SKIPS all `/id` user turns as session-lifecycle noise (user
...
export function parseSessionLine(line: string, sessionId?: string): ParsedLine {
  ...
  // "queued_command" — user typed normally in pretty view and hit enter,
  ...
  // because user intentionally invokes other slash-commands whose output
