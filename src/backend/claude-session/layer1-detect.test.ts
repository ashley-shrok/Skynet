...
describe("applyLineToLayer1State — tool_result during holding must not clear (2026-08-08 regression)", () => {
  ...
  it("state {isIdReset:true} + tool_result + REAL follow-up user typing + holding → clear only on the real typing", () => {
    // Full-realism: after tool_results, if user DOES type a new
...
describe("applyLineToLayer1State — user-bug regression guard (historical /id reset + later regular user turn)", () => {
  it("replaying: user(regular) → assistant → user(/id reset) → assistant → user(regular) with changeoverState always 'active' produces zero arm_holding actions overall", () => {
    // This is user's bug in its purest form. Under the OLD /exit
  ...
  it("replaying a session with 2 historical /exit user turns + no /id reset produces ZERO arms (the direct user bug fix)", () => {
    // The exact scenario from user's bug report: JSONL has 2
...
describe("applyLineToLayer1State — harness synthetics do not supersede /id reset (2026-08-12 regression)", () => {
  ...
  it("Test I — real user typing after /id reset STILL supersedes: [/id reset → real freeform typing] fires arm + clear (user cancels reset by typing)", () => {
    // Preserves the "supersede" semantics: user explicitly cancels the
