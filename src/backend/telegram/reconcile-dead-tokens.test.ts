 ...
 *   Test 2: happy path — user.token-dead exists, user has mxid,
...
describe("reconcile-dead-tokens", () => {
  ...
  it("Test 2: happy path — user.token-dead + mxid known + mint ok → sentinel unlinked, counts {1,1,0}", async () => {
    const { mintAndWriteHumanToken } = await import("./human-token-writer.js");
    ...
    const sentinelPath = path.join(tempDir, "user.token-dead");
